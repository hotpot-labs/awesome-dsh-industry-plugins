...
- **校验对象**: https://github.com/Phant0Meow/dsh-meow-cachebilling
- **校验时间**: 2026-08-25 01:24:02
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-03-17 (3083 天前); 公开仓库数: 18; 粉丝数: 5 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 212; 包名: meow-cachebilling; 描述: 喵账单：输入框下方一行实时读数——本轮请求的缓存命中/未命中/输出各花多少钱，峰谷自动计价，专治『缓存命中率 100% 却越用越贵』 — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web"}}; Cordis 配置文件: cordis.patch.yml<br>  - lib/index.js:2987 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 60 个, 比例: 0.0% — 未检测到任何 try/catch |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/client.ts; 发现清理逻辑: lib/client.js |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:7 → 深层目录穿越 (80 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:29 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:16 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:32 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:2562 → 探测到 HTTP(S) 网络请求 (8 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - lib/index.js:6158 → 挖矿相关 (2 处) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 4.1 | 无混淆代码 | 必查 | ❌ 不通过 | 检测到潜在混淆代码:<br>  - lib/index.js:8359 → 连续十六进制转义 (混淆) (4 处) |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 939, 注释行: 123 (13.1%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-meow-cachebilling |
| 校验 URL | https://github.com/Phant0Meow/dsh-meow-cachebilling |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
