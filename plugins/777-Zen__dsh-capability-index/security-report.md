...
- **校验对象**: https://github.com/777-Zen/dsh-capability-index
- **校验时间**: 2026-08-23 14:19:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-04-21 (124 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 548; 包名: dsh-capability-index; 描述: Pre-flight plugin-library check for DSH agents: per-step trigger-table hints inj — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml, cordis.patch.yml |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - examples/dsh-tool-demo-cap/README.md:15 → 深层目录穿越 (1 处) — 需人工判断是否有业务必要性 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - eval-results/simulate.mjs:47 → 进程控制 (1 处)<br>  - test/trigger-table.test.mjs:147 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1029, 注释行: 160 (15.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-capability-index |
| 校验 URL | https://github.com/777-Zen/dsh-capability-index |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
