...
- **校验对象**: https://github.com/Freakz2z/dsh-catgirl-plugin
- **校验时间**: 2026-09-01 04:09:38
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-04-03 (151 天前); 公开仓库数: 15; 粉丝数: 4 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 687; 包名: dsh-catgirl-plugin; 描述: Token-efficient catgirl persona runtime for DeepSeek Harness — README 包含功能/使用说明 |
...
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: renderer-test.mjs; 发现清理逻辑: composition-test.mjs |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - run-quality-test.mjs:37 → 读取环境变量配置 (1 处)<br>  - client/tsdown.config.ts:31 → 读取环境变量配置 (6 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - neko-renderer.js:26 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:22 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:20 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - client/package-lock.json:21 → 探测到 HTTP(S) 网络请求 (89 处)<br>  - assets/logo.png:4 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - assets/readme/mechanism.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, opencollective.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - client/tsdown.config.ts:31 → 读取环境变量 API Key/凭证 (4 处) — 需人工判断是否读取的是当前会话上下文 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - composition-test.mjs:26 → 写入配置文件 (1 处)<br>  - run-quality-test.mjs:39 → 写入配置文件 (2 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README_en.md:23 → 临时文件操作 (2 处)<br>  - smoke-test.mjs:67 → 进程控制 (1 处)<br>  - renderer-test.mjs:39 → 进程控制 (1 处)<br>  - usage-meter.js:10 → 临时文件操作 (1 处)<br>  - economy-test.mjs:62 → 进程控制 (1 处)<br>  ... 共 14 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 798, 注释行: 95 (11.9%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 10 天; 提交总数: 15 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-catgirl-plugin |
| 校验 URL | https://github.com/Freakz2z/dsh-catgirl-plugin |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
