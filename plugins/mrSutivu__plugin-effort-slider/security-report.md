...
- **校验对象**: https://github.com/mrSutivu/plugin-effort-slider
- **校验时间**: 2026-09-14 03:59:56
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-06-06 (464 天前); 公开仓库数: 7; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 680; 包名: plugin-effort-slider; 描述: Notched reasoning-effort slider grouped with the model picker for DeepSeek Harne — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"engines": {"dsh": ">=0.1.2-alpha"}, "bundle": {"patch": "./cordis.patch.yml"}, "client": {"platfor; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 54 处, 函数: 76 个, 比例: 71.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:17 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (8 处)<br>涉及的域名: github.com, img.shields.io, ko-fi.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - CHANGELOG.md:18 → 敏感凭证读取 (1 处)<br>  - README.md:35 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 481, 注释行: 32 (6.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | plugin-effort-slider |
| 校验 URL | https://github.com/mrSutivu/plugin-effort-slider |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
