...
- **校验对象**: https://github.com/CLASSLU/dsh-telegram-bridge
- **校验时间**: 2026-08-26 01:25:08
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-11-23 (3197 天前); 公开仓库数: 15; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 534; 包名: dsh-telegram-bridge; 描述: Telegram bot bridge for DeepSeek Harness (dsh): chat with your DSH agent from Te — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-credentials: >=0.1.0; @deepseek-ai/dsh-llm: >=0.1.0; @deepseek-ai/dsh-settings: >=0.1.0; engines: {"node": ">=18"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["@deepseek-ai/d; Cordis/DSH 依赖: @deepseek-ai/dsh-credentials, @deepseek-ai/dsh-llm, @deepseek-ai/dsh-settings; Cordis 配置文件: cordis.patch.yml |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:10 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - cordis.patch.yml:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:49 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: 127.0.0.1, github.com, user<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - .gitignore:12 → 敏感凭证读取 (2 处)<br>  - package.json:52 → 敏感凭证读取 (1 处)<br>  - README.md:34 → 敏感凭证读取 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - README.md:37 → 硬编码敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-telegram-bridge |
| 校验 URL | https://github.com/CLASSLU/dsh-telegram-bridge |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
