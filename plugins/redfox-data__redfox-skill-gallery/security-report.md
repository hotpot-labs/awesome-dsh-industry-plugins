...
- **校验对象**: https://github.com/redfox-data/redfox-skill-gallery
- **校验时间**: 2026-09-14 04:00:46
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-04-14 (153 天前); 公开仓库数: 6; 粉丝数: 31 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 632; 包名: redfox-skill-gallery; 描述: 技能画廊：在侧边栏可视化全部 skill，点击即用（通过 /name 手势直接调用 DeepSeek Harness）。 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ❌ 不通过 | 检测到潜在违规模式:<br>  - lib/skill-categories.json:144 → 非法爬取 (1 处) |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CHANGELOG.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:33 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:4 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - client/client.js:179 → fetch 网络请求 (3 处)<br>涉及的域名: github.com, keepachangelog.com, localhost, semver.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:139 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 847, 注释行: 54 (6.4%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 17 天; 提交总数: 10 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | redfox-skill-gallery |
| 校验 URL | https://github.com/redfox-data/redfox-skill-gallery |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
