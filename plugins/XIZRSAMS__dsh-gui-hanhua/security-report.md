...
- **校验对象**: https://github.com/XIZRSAMS/dsh-gui-hanhua
- **校验时间**: 2026-08-30 04:27:16
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ⚠️  需人工复核 | 账号注册: 2026-08-11 (18 天前); 公开仓库数: 2; 粉丝数: 0; ⚠️ 账号注册不足 30 天 (18 天) |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 510; 包名: dsh-gui-hanhua; 描述: GUI可视信息汉化 V1.1 —— DeepSeek Harness 界面信息汉化插件（常驻版） — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"client": {"platform": "web"}}; Cordis 配置文件: cordis.patch.yml<br>  - client.js:688 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 118 处, 函数: 200 个, 比例: 59.0% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:11 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.md:11 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - client.js:919 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - index.js:372 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - index.js:398 → fetch 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, raw.githubusercontent.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - client.js:275 → 敏感凭证读取 (2 处)<br>  - index.js:159 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - index.js:413 → 修改设置 (6 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client.js:1282 → 定时任务 (2 处)<br>  - index.js:490 → 文件读取 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2113, 注释行: 190 (9.0%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 10 天; 提交总数: 44 |
...
| :--- | :--- |
| 插件名称 | dsh-gui-hanhua |
| 校验 URL | https://github.com/XIZRSAMS/dsh-gui-hanhua |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
