...
- **校验对象**: https://github.com/nicecx/dsh-reset-handoff
- **校验时间**: 2026-09-03 03:34:02
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1045; 包名: dsh-reset-handoff; 描述: Standard handoff protocol so DSH never restarts itself: a host plugin that files — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 40 个, 比例: 20.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - scripts/test_review_submit.py:17 → 读取环境变量配置 (2 处)<br>  - scripts/review_submit.py:24 → 读取环境变量配置 (1 处)<br>  - hermes/reset_agent.py:33 → 读取环境变量配置 (1 处)<br>  - src/protocol.js:26 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/protocol.js:27 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - scripts/test_review_submit.py:22 → Python subprocess (2 处)<br>  - hermes/reset_agent.py:133 → Python subprocess (2 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:20 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:26 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:26 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - hermes/reset_agent.py:241 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: 127.0.0.1, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/index.test.js:67 → 临时文件操作 (4 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1315, 注释行: 112 (8.5%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - CONTRIBUTING.md:36 → 创建开机自启/系统服务 (1 处)<br>  - README.md:125 → 创建开机自启/系统服务 (2 处)<br>  - README.zh.md:124 → 创建开机自启/系统服务 (1 处)<br>  - scripts/review_submit.py:1 → 修改系统目录 (1 处)<br>  - docs/PROTOCOL.md:56 → 修改系统目录 (1 处)<br>  ... 共 8 处 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-reset-handoff |
| 校验 URL | https://github.com/nicecx/dsh-reset-handoff |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
