...
- **校验对象**: https://github.com/MannixHu/dsh-statusbar-config
- **校验时间**: 2026-09-04 03:41:09
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2015-11-26 (3934 天前); 公开仓库数: 34; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 270; 包名: dsh-statusbar-config; 描述: Template-driven conversation statistics row for DeepSeek Harness (${variable} pl — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 4 处, 函数: 108 个, 比例: 3.7% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:5 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package.json:78 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1420, 注释行: 134 (9.4%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:104 → 硬编码敏感信息 (1 处)<br>  - src/client/SettingsCard.tsx:69 → 硬编码敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-statusbar-config |
| 校验 URL | https://github.com/MannixHu/dsh-statusbar-config |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
