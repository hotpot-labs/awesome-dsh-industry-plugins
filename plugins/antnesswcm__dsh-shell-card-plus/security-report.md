...
- **校验对象**: https://github.com/antnesswcm/dsh-shell-card-plus
- **校验时间**: 2026-09-02 03:37:02
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-11-29 (2468 天前); 公开仓库数: 44; 粉丝数: 8 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 165; 包名: dsh-shell-card-plus; 描述: Shell command cards (bash / pwsh) for the DSH web UI: fully custom React-rendere — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 5 处, 函数: 63 个, 比例: 7.9% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CLAUDE.md:211 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.en.md:5 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - package.json:64 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:25 → 探测到 HTTP(S) 网络请求 (54 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com, img.shields.io, opencollective.com, registry.npmmirror.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client/ShellCard.tsx:94 → 定时任务 (2 处)<br>  - scripts/build.mjs:176 → 定时任务 (1 处)<br>  - lib/client.js:245 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 0 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 812, 注释行: 101 (12.4%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-shell-card-plus |
| 校验 URL | https://github.com/antnesswcm/dsh-shell-card-plus |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
