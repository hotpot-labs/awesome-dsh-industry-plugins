...
- **校验对象**: https://github.com/GooDAnDReaDY/dsh-dsml-artifact-guard
- **校验时间**: 2026-09-13 03:51:38
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 901; 包名: @goodandready/dsh-dsml-artifact-guard; 描述: Sanitizes leaked DeepSeek DSML closing tags from model text streams — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (3): v0.1.5, v0.1.4, v0.1.1; package.json version: 0.1.5; GitHub Releases (3): v0.1.5, v0.1.4, v0.1.1 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:9 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package-lock.json:21 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.md:8 → 探测到 HTTP(S) 网络请求 (10 处)<br>  - README.md:52 → WebSocket 连接 (1 处)<br>  - README.md:52 → 浏览器网络 API (1 处)<br>  - docs/README.ru.md:8 → 探测到 HTTP(S) 网络请求 (10 处)<br>涉及的域名: github.com, goodandready.app, img.shields.io, nodejs.org, registry.npmjs.org, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - .gitignore:8 → 敏感凭证读取 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 243, 注释行: 1 (0.4%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-dsml-artifact-guard |
| 校验 URL | https://github.com/GooDAnDReaDY/dsh-dsml-artifact-guard |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
