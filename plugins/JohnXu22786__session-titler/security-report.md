...
- **校验对象**: https://github.com/JohnXu22786/session-titler
- **校验时间**: 2026-08-23 16:35:05
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1522; 包名: dsh-session-caption; 描述: Two-phase session captioning for the DeepSeek Harness: instant keyword captions  — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 14 处, 函数: 355 个, 比例: 3.9% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/pacemaker.ts; 发现清理逻辑: src/index.ts; 发现清理逻辑: src/flow.ts |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - dev/pkgs/dsh-session/index.d.ts:1 → 深层目录穿越 (1 处)<br>  - dev/pkgs/dsh-session/types.d.ts:1 → 深层目录穿越 (1 处)<br>  - dev/pkgs/dsh-session/index.js:1 → 深层目录穿越 (1 处)<br>  - dev/pkgs/dsh-session/types.js:1 → 深层目录穿越 (1 处)<br>  - dev/pkgs/dsh-session-title/index.d.ts:1 → 深层目录穿越 (1 处)<br>  ... 共 8 处 — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package-lock.json:93 → 探测到 HTTP(S) 网络请求 (69 处)<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - tests/keywords.test.ts:89 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: example.com, github.com, keepachangelog.com, opencollective.com, registry.npmjs.org, tidelift.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - tests/flow.test.ts:122 → 敏感凭证读取 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/pacemaker.ts:84 → 定时任务 (1 处)<br>  - lib/src/pacemaker.js:56 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 8. repo: 0 deps + 8 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 4765, 注释行: 875 (18.4%) |
...
| :--- | :--- |
| 插件名称 | session-titler |
| 校验 URL | https://github.com/JohnXu22786/session-titler |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
