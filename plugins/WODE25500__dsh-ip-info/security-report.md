...
- **校验对象**: https://github.com/WODE25500/dsh-ip-info
- **校验时间**: 2026-09-02 03:36:16
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 162; 包名: dsh-ip-info; 描述: IP/domain intelligence for DeepSeek Harness: ip_lookup, dns_lookup, whois_lookup — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ❌ 不通过 | 检测到潜在违规模式:<br>  - src/tools/whois.js:5 → 盗版/侵权 (2 处) |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 2 处, 函数: 11 个, 比例: 18.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:11 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.zh.md:11 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - src/index.js:19 → fetch 网络请求 (1 处)<br>  - src/tools/dns.js:14 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/tools/whois.js:12 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: developers.google.com, dns.google, ipwho.is, rdap.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/check-tools.mjs:41 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 2 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 172, 注释行: 15 (8.7%) |
...
| :--- | :--- |
| 插件名称 | dsh-ip-info |
| 校验 URL | https://github.com/WODE25500/dsh-ip-info |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
