...
- **校验对象**: https://github.com/Mlte0907/dsh-brake
- **校验时间**: 2026-09-20 04:01:29
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-03-10 (193 天前); 公开仓库数: 9; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 791; 包名: dsh-brake; 描述: DSH 死循环刹车器：检测重复调用、序列循环与重复文本上下文，默认提醒，可选执行前拒绝 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 9 处, 函数: 93 个, 比例: 9.7% — try/catch 覆盖比例较低 |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - test/test-e2e.js:4 → API Key / Token 读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 890, 注释行: 115 (12.9%) |
...
| :--- | :--- |
| 插件名称 | dsh-brake |
| 校验 URL | https://github.com/Mlte0907/dsh-brake |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
