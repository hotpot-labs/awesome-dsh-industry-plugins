...
- **校验对象**: https://github.com/liujia-io/dsh-image-picker
- **校验时间**: 2026-08-29 06:14:50
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-07-04 (55 天前); 公开仓库数: 2; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 205; 包名: dsh-image-picker; 描述: DeepSeek Harness Web GUI 输入框 📎 选图按钮:通过系统文件选择器添加参考图,绕开拖拽环境问题,复用官方附件管线(缩略图/上限校验/随消 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 6 处, 函数: 50 个, 比例: 12.0% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - CONTRIBUTING.md:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:50 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CHANGELOG.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/TROUBLESHOOTING.md:36 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, keepachangelog.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:196 → 定时任务 (1 处)<br>  - lib/index.js:97 → 文件写入 (1 处)<br>  - lib/index.js:30 → 文件读取 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - lib/client.js:13 → 云存储上传 (1 处)<br>  - lib/index.js:26 → 云存储上传 (2 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 407, 注释行: 50 (12.3%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-image-picker |
| 校验 URL | https://github.com/liujia-io/dsh-image-picker |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
