...
- **校验对象**: https://github.com/alexfengrui/dsh-client-ui-skin-real-madrid
- **校验时间**: 2026-09-08 03:41:49
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-02-08 (211 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ⚠️  需人工复核 | 许可证: SEE LICENSE IN LICENSE — 需确认与 MIT 兼容性 |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 263; 包名: dsh-client-ui-skin-real-madrid; 描述: Real Madrid dual-mode skin for DeepSeek Harness: Bernabéu night stadium for dark — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ❌ 不通过 | 检测到潜在违规模式:<br>  - NOTICE:25 → 盗版/侵权 (1 处) |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (1): v1.1.0; package.json version: 1.1.0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 20 个, 比例: 0.0% — 未检测到任何 try/catch |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:9 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package.json:43 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - NOTICE:9 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:9 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - lib/client.template.js:21 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: github.com, img.shields.io, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 4.1 | 无混淆代码 | 必查 | ❌ 不通过 | 检测到潜在混淆代码:<br>  - lib/client.js:13 → 超长 base64 字符串 (2 处) |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:14 → 临时文件操作 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 394, 注释行: 48 (12.2%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - README.en.md:45 → 创建开机自启/系统服务 (1 处)<br>  - README.md:45 → 创建开机自启/系统服务 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-client-ui-skin-real-madrid |
| 校验 URL | https://github.com/alexfengrui/dsh-client-ui-skin-real-madrid |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
