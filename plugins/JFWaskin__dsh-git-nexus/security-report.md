...
- **校验对象**: https://github.com/JFWaskin/dsh-git-nexus
- **校验时间**: 2026-08-23 16:32:18
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-07-31 (1119 天前); 公开仓库数: 14; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 399; 包名: dsh-git-nexus; 描述: DSH web plugin: unified Git + GitHub + workflow + files panel (branch, stage/uns — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-typert-protocol: >=0.1.0-rc.6 <0.2.0; engines: {"node": "^22.19.0 \|\| >=24.0.0"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-api-remotes", "@; Cordis/DSH 依赖: @deepseek-ai/dsh-typert-protocol; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 17 处, 函数: 130 个, 比例: 13.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:56 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:32 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - lib/index.js:43 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/index.js:382 → fetch 网络请求 (5 处)<br>涉及的域名: api.github.com, docs.github.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:414 → API Key / Token 读取 (9 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - smoke.test.mjs:140 → 进程控制 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1606, 注释行: 130 (8.1%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - lib/index.js:253 → 读取 git 历史 (2 处) — 需人工判断是否在授权范围内 |
...
| :--- | :--- |
| 插件名称 | dsh-git-nexus |
| 校验 URL | https://github.com/JFWaskin/dsh-git-nexus |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
