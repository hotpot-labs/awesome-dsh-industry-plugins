...
- **校验对象**: https://github.com/lin-1259/dsh-desktop-linux-updater
- **校验时间**: 2026-09-01 04:12:56
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-02-03 (2401 天前); 公开仓库数: 35; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 279; 包名: dsh-desktop-linux-updater; 描述: One-click update for self-built Linux dsh-desktop: checks the fork's GitHub rele — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"client": {"platform": "web", "inject": ["@deepseek-ai/dsh-client-ui-settings", "@deepseek-ai/dsh-c; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 14 处, 函数: 43 个, 比例: 32.6% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - index.js:51 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - SECURITY.md:26 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:41 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - client.js:78 → fetch 网络请求 (3 处)<br>  - index.js:86 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - index.js:76 → fetch 网络请求 (2 处)<br>涉及的域名: api.github.com, github.com, raw.githubusercontent.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client.js:87 → 定时任务 (1 处)<br>  - index.js:74 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 660, 注释行: 47 (7.1%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-desktop-linux-updater |
| 校验 URL | https://github.com/lin-1259/dsh-desktop-linux-updater |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
