...
- **校验对象**: https://github.com/thomasvvugt/dsh-wide-stats-footer
- **校验时间**: 2026-08-27 08:18:03
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-10-15 (2507 天前); 公开仓库数: 15; 粉丝数: 19 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 766; 包名: dsh-wide-stats-footer; 描述: Wide stats footer: removes the max-width clamp on the composer stats line (turns — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - SECURITY.md:16 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:16 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - CONTRIBUTING.md:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - CHANGELOG.md:5 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: github.com, keepachangelog.com, semver.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:32 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - .github/workflows/ci.yml:37 → 文件读取 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 279, 注释行: 87 (31.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-wide-stats-footer |
| 校验 URL | https://github.com/thomasvvugt/dsh-wide-stats-footer |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
