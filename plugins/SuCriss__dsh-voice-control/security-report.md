...
- **校验对象**: https://github.com/SuCriss/dsh-voice-control
- **校验时间**: 2026-09-02 03:36:08
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-03-20 (3088 天前); 公开仓库数: 108; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 117; 包名: dsh-voice-control; 描述: Voice control for DSH web: speech-to-text into the composer (with auto-send) and — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 23 处, 函数: 89 个, 比例: 25.8% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:46 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CONTRIBUTING.md:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, keepachangelog.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/mount-contract.test.cjs:14 → 文件读取 (4 处)<br>  - lib/client.js:105 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - SECURITY.md:12 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 764, 注释行: 120 (15.7%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:25 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-voice-control |
| 校验 URL | https://github.com/SuCriss/dsh-voice-control |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
