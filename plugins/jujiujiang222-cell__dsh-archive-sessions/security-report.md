...
- **校验对象**: https://github.com/jujiujiang222-cell/dsh-archive-sessions
- **校验时间**: 2026-09-09 03:45:57
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-12-15 (267 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 608; 包名: dsh-archive-sessions; 描述: Archived-session manager for DeepSeek Harness: list archived sessions, restore o — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"client": {"inject": ["@deepseek-ai/dsh-client-connection", "@deepseek-ai/dsh-client-runtime", "@de; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 14 处, 函数: 44 个, 比例: 31.8% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:45 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:41 → fetch 网络请求 (1 处)<br>  - docs/registry-submission.md:4 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - docs/awesome-dsh-plugin-entry.yml:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ✅ 通过 | 未检测到敏感配置读取模式 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - docs/dynamic-plugin-source.md:64 → 修改全局配置 (1 处)<br>  - src/index.js:152 → 修改全局配置 (1 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 447, 注释行: 40 (8.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-archive-sessions |
| 校验 URL | https://github.com/jujiujiang222-cell/dsh-archive-sessions |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
