...
- **校验对象**: https://github.com/Amengclass/dsh-settings-hub
- **校验时间**: 2026-08-30 04:21:32
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-01-03 (1334 天前); 公开仓库数: 22; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 385; 包名: dsh-settings-hub; 描述: Unify third-party plugin settings in DeepSeek Harness — auto-discover, group, an — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 7 处, 函数: 123 个, 比例: 5.7% — try/catch 覆盖比例较低 |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - index.js:55 → 读取用户主目录 (1 处)<br>  - scripts/build-dep-map.cjs:16 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README_EN.md:7 → 探测到 HTTP(S) 网络请求 (15 处)<br>  - package.json:21 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (15 处)<br>  - .github/workflows/release.yml:20 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - debug/snapshot.txt:1 → 探测到 HTTP(S) 网络请求 (30 处)<br>涉及的域名: 127.0.0.1, github.com, img.shields.io, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - .github/workflows/release.yml:24 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.js:18 → 文件读取 (1 处)<br>  - scripts/build-dep-map.cjs:113 → 文件写入 (1 处)<br>  - scripts/build-dep-map.cjs:25 → 文件读取 (2 处)<br>  - scripts/build-dep-map.cjs:48 → 进程控制 (2 处)<br>  - lib/client.js:494 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1126, 注释行: 129 (11.5%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:274 → 明文写入敏感信息 (2 处) — 需确认是否加密存储 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - scripts/build-dep-map.cjs:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-settings-hub |
| 校验 URL | https://github.com/Amengclass/dsh-settings-hub |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
