...
- **校验对象**: https://github.com/chengzhicao/llm-capture
- **校验时间**: 2026-09-12 03:49:09
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2015-04-13 (4169 天前); 公开仓库数: 43; 粉丝数: 13 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ❌ 不通过 | 未找到许可证声明 (package.json 无 license 字段且无 LICENSE 文件) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 199; 包名: dsh-llm-capture; 描述: Inspect raw LLM requests and responses in DeepSeek Harness Web — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.2; @deepseek-ai/dsh-fs: ^0.1.3-alpha.2; @deepseek-ai/dsh-llm: ^0.1.3-alpha.2; @deepseek-ai/dsh-typert-protocol: ^0.1.3-alpha.2; engines: {"node": "^22.19.0 \|\| >=24.0.0"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["@deepseek-ai/d; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-fs, @deepseek-ai/dsh-llm, @deepseek-ai/dsh-typert-protocol; Cordis 配置文件: cordis.patch.yml<br>  - lib/client.js:1715 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 46 处, 函数: 707 个, 比例: 6.5% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/client.js:923 → 探测到 HTTP(S) 网络请求 (9 处)<br>涉及的域名: github.com, json-schema.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - lib/client.js:518 → new Function() 任意代码执行 (1 处) |
| 4.3 | npm audit 无高危漏洞 | 必查 | ⚠️  需人工复核 | npm 不可用, 无法执行 npm audit. 建议在具备 npm 的环境中运行 `npm audit` |
| 4.4 | 不使用废弃依赖 | 必查 | ✅ 通过 | 共检查 15 个依赖, 未发现已知废弃包 |
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:4360 → 定时任务 (1 处)<br>  - src/client.ts:50 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 15. repo: 1 deps + 14 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 5316, 注释行: 228 (4.3%) |
...
| :--- | :--- |
| 插件名称 | llm-capture |
| 校验 URL | https://github.com/chengzhicao/llm-capture |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
