...
- **校验对象**: https://github.com/KLRSL/dsh-fuse
- **校验时间**: 2026-09-01 04:09:56
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-12-21 (618 天前); 公开仓库数: 5; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 292; 包名: dsh-fuse; 描述: Fuse v1.0.1 — UI 设计 + 代码规范一体化插件（ui-aesthetics 技能升级版）：设计令牌 theme.json + 代码规范 code — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 24 处, 函数: 166 个, 比例: 14.5% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: tests/test-settings.mjs |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - index.mjs:253 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:25 → 探测到 HTTP(S) 网络请求 (190 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - client.js:190 → fetch 网络请求 (2 处)<br>  - tests/test-settings.mjs:24 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, dsh.invalid, github.com, opencollective.com, registry.npmmirror.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SKILL.md:62 → 敏感凭证读取 (1 处)<br>  - README.md:50 → 敏感凭证读取 (1 处)<br>  - client.js:836 → 敏感凭证读取 (2 处)<br>  - tests/fuse.test.mjs:16 → 敏感凭证读取 (1 处)<br>  - tests/test-client.mjs:102 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - tests/test-settings.mjs:113 → node:vm 任意代码执行 (1 处)<br>  - tests/test-client.mjs:58 → node:vm 任意代码执行 (1 处) |
| 4.3 | npm audit 无高危漏洞 | 必查 | ⚠️  需人工复核 | npm 不可用, 无法执行 npm audit. 建议在具备 npm 的环境中运行 `npm audit` |
| 4.4 | 不使用废弃依赖 | 必查 | ✅ 通过 | 共检查 4 个依赖, 未发现已知废弃包 |
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.mjs:63 → 文件读取 (1 处)<br>  - client.js:408 → 定时任务 (4 处)<br>  - tests/apply-smoke.mjs:61 → 进程控制 (3 处)<br>  - tests/fuse.test.mjs:107 → 文件读取 (2 处)<br>  - tests/test-settings.mjs:21 → 定时任务 (2 处)<br>  ... 共 8 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 4. repo: 1 deps + 3 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1891, 注释行: 175 (9.3%) |
...
| :--- | :--- |
| 插件名称 | dsh-fuse |
| 校验 URL | https://github.com/KLRSL/dsh-fuse |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
