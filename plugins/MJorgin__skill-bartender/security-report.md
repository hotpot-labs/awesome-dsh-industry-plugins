...
- **校验对象**: https://github.com/MJorgin/skill-bartender
- **校验时间**: 2026-09-24 03:48:14
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-08-19 (400 天前); 公开仓库数: 19; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1209; 包名: skill-bartender; 描述: Task-to-skill pairing with a laziness ladder and a safe install cellar. Never ov — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 3 个, 比例: 0.0% — 未检测到任何 try/catch |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - docs/lang/README_ZH.md:11 → 深层目录穿越 (5 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - scripts/validate.py:31 → Python subprocess (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - SECURITY.md:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:11 → 探测到 HTTP(S) 网络请求 (17 处)<br>  - .github/workflows/scan.yml:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - skills/skill-bartender/SKILL.md:49 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/EXAMPLES.md:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: deepseek-harness.github.io, github.com, img.shields.io, x.feishu.cn<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:17 → 敏感凭证读取 (1 处)<br>  - skills/skill-bartender/references/policy.md:36 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 252, 注释行: 26 (10.3%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - scripts/validate.py:1 → 修改系统目录 (1 处)<br>  - scripts/make-banner.py:1 → 修改系统目录 (4 处)<br>  - scripts/make-diagram.py:1 → 修改系统目录 (4 处) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 37 天; 提交总数: 11 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | skill-bartender |
| 校验 URL | https://github.com/MJorgin/skill-bartender |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
