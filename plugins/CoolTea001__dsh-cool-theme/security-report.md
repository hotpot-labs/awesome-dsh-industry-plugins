...
- **校验对象**: https://github.com/CoolTea001/dsh-cool-theme
- **校验时间**: 2026-09-02 03:33:16
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-07-01 (1888 天前); 公开仓库数: 5; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 206; 包名: dsh-cool-theme; 描述: A plugin for changing themes for DSH, with a number of popular default themes bu — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (4): v0.4.0, v0.3.1, v0.2.0, v0.1.0; package.json version: 0.4.0; GitHub Releases (4): v0.4.0, v0.3.1, v0.2.0, v0.1.0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 25 处, 函数: 423 个, 比例: 5.9% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/client/style-injector.ts; 发现清理逻辑: lib/client.js |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:12 → 读取环境变量配置 (2 处)<br>  - AGENTS.md:5 → 读取环境变量配置 (1 处)<br>  - AGENTS.zh.md:5 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.md:1 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.zh.md:1 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - src/client/components.tsx:12 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - src/client/theme.tsx:143 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: cdn.cooltea.top, github.com, img.shields.io, registry.npmjs.org, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 7036, 注释行: 361 (5.1%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - src/client/theme.tsx:85 → 明文写入敏感信息 (3 处)<br>  - lib/client.js:2466 → 明文写入敏感信息 (3 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | dsh-cool-theme |
| 校验 URL | https://github.com/CoolTea001/dsh-cool-theme |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
