...
- **校验对象**: https://github.com/JohnXu22786/codegraph
- **校验时间**: 2026-08-23 16:34:15
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1507; 包名: dsh-codegraph; 描述: CodeGraph for DeepSeek Harness (dsh): a Node bridge that exposes the codegraph P — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml<br>  - src/codegraph/server/mcp.py:43 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 73 个, 比例: 11.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - index.js:53 → 读取环境变量配置 (2 处)<br>  - src/codegraph/config.py:60 → 读取环境变量配置 (7 处)<br>  - tests/test_cli.py:19 → 读取环境变量配置 (1 处)<br>  - tests/test_config.py:62 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - tests/test_cli.py:26 → Python subprocess (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CHANGELOG.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - tests/test_scan_quick.py:105 → fetch 网络请求 (2 处)<br>  - tests/fixtures/proj/web/index.ts:11 → fetch 网络请求 (2 处)<br>涉及的域名: keepachangelog.com, semver.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - index.js:55 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 5360, 注释行: 167 (3.1%) |
...
| :--- | :--- |
| 插件名称 | codegraph |
| 校验 URL | https://github.com/JohnXu22786/codegraph |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
