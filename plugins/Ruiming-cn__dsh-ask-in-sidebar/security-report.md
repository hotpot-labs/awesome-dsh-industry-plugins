...
- **校验对象**: https://github.com/Ruiming-cn/dsh-ask-in-sidebar
- **校验时间**: 2026-08-31 04:27:50
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 314; 包名: dsh-ask-in-sidebar; 描述: Ask about selected assistant text in an ephemeral DSH Web sidebar using a snapsh — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-client-connection: *; @deepseek-ai/dsh-host-webserver: *; @deepseek-ai/dsh-api-session-controller: *; @deepseek-ai/dsh-client-ui-layout: *; @deepseek-ai/dsh-client-ui-slots: * |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["@deepseek-ai/d; Cordis/DSH 依赖: @deepseek-ai/dsh-client-connection, @deepseek-ai/dsh-host-webserver, @deepseek-ai/dsh-api-session-controller, @deepseek-ai/dsh-client-ui-layout, @deepseek-ai/dsh-client-ui-slots; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 42 处, 函数: 225 个, 比例: 18.7% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:5 → 探测到 HTTP(S) 网络请求 (11 处)<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (11 处)<br>  - src/client.js:443 → fetch 网络请求 (3 处)<br>  - lib/client.js:443 → fetch 网络请求 (3 处)<br>涉及的域名: github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/index.js:310 → 定时任务 (1 处)<br>  - lib/index.js:310 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 3244, 注释行: 214 (6.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-ask-in-sidebar |
| 校验 URL | https://github.com/Ruiming-cn/dsh-ask-in-sidebar |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
