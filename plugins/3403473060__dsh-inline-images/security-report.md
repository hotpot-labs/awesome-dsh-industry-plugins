...
- **校验对象**: https://github.com/3403473060/dsh-inline-images
- **校验时间**: 2026-08-23 14:19:00
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-05-30 (2276 天前); 公开仓库数: 5; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 89; 包名: dsh-inline-images; 描述: 对话内联图片:LLM 回复中输出的本地图片路径直接在消息正文内渲染为图片,支持 9 种格式、点击放大灯箱、可调尺寸 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 24 处, 函数: 105 个, 比例: 22.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:26 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/index.ts:258 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .github/workflows/ci.yml:17 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/index.js:250 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - src/index.ts:9 → 敏感凭证读取 (10 处)<br>  - .github/workflows/ci.yml:47 → 敏感凭证读取 (1 处)<br>  - lib/index.js:65 → 敏感凭证读取 (8 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - .github/workflows/ci.yml:21 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - .github/workflows/ci.yml:23 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 927, 注释行: 27 (2.9%) |
...
| 7.6 | 有社区背书 | 推荐 | ⚠️  需人工复核 | Stars: 3, Forks: 2, Watchers: 0 — 社区背书不足, 需人工判断 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-inline-images |
| 校验 URL | https://github.com/3403473060/dsh-inline-images |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
