...
- **校验对象**: https://github.com/ClaireXi99/dsh-data-insight
- **校验时间**: 2026-08-23 14:32:13
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-12-25 (971 天前); 公开仓库数: 14; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 177; 包名: dsh-data-insight; 描述: Data analysis toolkit for DeepSeek Harness — CSV profiling, anomaly detection, a — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: >=1.0.0; @deepseek-ai/dsh-tools: >=0.1.0 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": true}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-tools; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 1 处, 函数: 76 个, 比例: 1.3% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:35 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:24 → 探测到 HTTP(S) 网络请求 (152 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, opencollective.com, paulmillr.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/smoke.mjs:85 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 4. repo: 0 deps + 4 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 733, 注释行: 5 (0.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-data-insight |
| 校验 URL | https://github.com/ClaireXi99/dsh-data-insight |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
