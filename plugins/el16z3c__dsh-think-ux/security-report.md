...
- **校验对象**: https://github.com/el16z3c/dsh-think-ux
- **校验时间**: 2026-09-18 04:00:34
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-05-14 (3413 天前); 公开仓库数: 12; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 3563; 包名: dsh-think-ux; 描述: Smooth think-row preview + bottom-follow glide for the DeepSeek Harness (dsh) We — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 14 处, 函数: 78 个, 比例: 17.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:15 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - trace-sink.cjs:42 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:153 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:193 → fetch 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, awesome-dsh-plugin.com, github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - tests/client-registration.cjs:8 → node:vm 任意代码执行 (1 处) |
| 4.3 | npm audit 无高危漏洞 | 必查 | ⚠️  需人工复核 | npm 不可用, 无法执行 npm audit. 建议在具备 npm 的环境中运行 `npm audit` |
| 4.4 | 不使用废弃依赖 | 必查 | ✅ 通过 | 共检查 0 个依赖, 未发现已知废弃包 |
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - trace-sink.cjs:41 → 开启监听端口 (1 处)<br>  - trace-sink.cjs:32 → 文件写入 (1 处)<br>  - tests/client-registration.cjs:6 → 文件读取 (2 处)<br>  - lib/client.js:553 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1304, 注释行: 467 (35.8%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 2 天; 提交总数: 25 |
...
| :--- | :--- |
| 插件名称 | dsh-think-ux |
| 校验 URL | https://github.com/el16z3c/dsh-think-ux |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
