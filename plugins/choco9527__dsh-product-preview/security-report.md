...
- **校验对象**: https://github.com/choco9527/dsh-product-preview
- **校验时间**: 2026-09-06 03:35:20
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-08-31 (2562 天前); 公开仓库数: 40; 粉丝数: 6 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ❌ 不通过 | 未找到许可证声明 (package.json 无 license 字段且无 LICENSE 文件) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 132; 包名: dsh-product-preview; 描述: Finder-style media artifact preview for DeepSeek Harness — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.2; @deepseek-ai/dsh-client-locale: 0.1.2-alpha.3; @deepseek-ai/dsh-client-ui-chat: 0.1.2-alpha.3; @deepseek-ai/dsh-client-ui-conversation: 0.1.2-alpha.3; @deepseek-ai/dsh-client-ui-layout: 0.1.2-alpha.3 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 9 处, 函数: 131 个, 比例: 6.9% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/client/tab-swipe.ts |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - THIRD_PARTY_NOTICES.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.en.md:23 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:21 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - tests/artifacts.spec.ts:51 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, cdn.example.test, github.com, localhost, www.npmjs.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - pnpm-lock.yaml:17 → 敏感凭证读取 (4 处)<br>  - pnpm-workspace.yaml:16 → 敏感凭证读取 (1 处)<br>  - tests/media-server.spec.ts:61 → 敏感凭证读取 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - tests/media-server.spec.ts:37 → 开启监听端口 (1 处)<br>  - src/client/tab-swipe.ts:144 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ⚠️  需人工复核 | 总依赖数: 24. repo: 0 deps + 24 devDeps — 依赖较多, 需人工审查 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1185, 注释行: 44 (3.7%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - tests/media-server.spec.ts:61 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-product-preview |
| 校验 URL | https://github.com/choco9527/dsh-product-preview |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
