...
- **校验对象**: https://github.com/PaoMoXML/dsh-paste-names
- **校验时间**: 2026-08-25 01:23:58
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-02-16 (3476 天前); 公开仓库数: 22; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 386; 包名: dsh-paste-names; 描述: Paste/drop non-image files or folders into the DSH chat composer: paste inserts  — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v1.3.0, v1.2.0; package.json version: 1.3.0; GitHub Releases (1): v1.3.0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 31 处, 函数: 73 个, 比例: 42.5% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:24 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:74 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/client.js:145 → fetch 网络请求 (2 处)<br>  - lib/index.js:180 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, x<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:136 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 648, 注释行: 111 (17.1%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-paste-names |
| 校验 URL | https://github.com/PaoMoXML/dsh-paste-names |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
