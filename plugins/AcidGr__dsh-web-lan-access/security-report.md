...
- **校验对象**: https://github.com/AcidGr/dsh-web-lan-access
- **校验时间**: 2026-08-23 14:23:14
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-09-26 (1062 天前); 公开仓库数: 3; 粉丝数: 4 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 911; 包名: dsh-web-lan-access; 描述: LAN / remote access support for the DeepSeek Harness Web UI: self-contained 0.0. — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:3 → 读取 npm 凭证 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - README.md:45 → 深层目录穿越 (1 处)<br>  - README.zh.md:45 → 深层目录穿越 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - cordis.patch.yml:17 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:23 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:2 → 探测到 HTTP(S) 网络请求 (11 处)<br>  - README.zh.md:5 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - lib/index.js:6 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, awesome-dsh-plugin.com, github.com, localhost, myhost<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:84 → 敏感凭证读取 (2 处)<br>  - README.zh.md:84 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ⚠️  需人工复核 | 总行数: 51, 注释行: 26 (51.0%) — 注释占比过高, 可能有废弃代码 |
...
| 7.6 | 有社区背书 | 推荐 | ⚠️  需人工复核 | Stars: 25, Forks: 3, Watchers: 0 — 有一定社区基础 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-web-lan-access |
| 校验 URL | https://github.com/AcidGr/dsh-web-lan-access |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
