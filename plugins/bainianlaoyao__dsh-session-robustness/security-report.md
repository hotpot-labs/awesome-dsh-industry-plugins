...
- **校验对象**: https://github.com/bainianlaoyao/dsh-session-robustness
- **校验时间**: 2026-09-12 03:48:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-05-16 (1945 天前); 公开仓库数: 37; 粉丝数: 5 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 406; 包名: dsh-session-robustness; 描述: 工程级会话鲁棒：官方有限次 API 重试耗尽后，对超时/传输/限流等瞬时失败继续重试直到成功或取消，会话不因瞬时 API 故障中断。 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v0.1.6, v0.1.4; package.json version: 0.1.6 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 20 处, 函数: 183 个, 比例: 10.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:27 → WebSocket 连接 (2 处)<br>  - README.en.md:27 → 浏览器网络 API (2 处)<br>  - SECURITY.md:14 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CHANGELOG.md:5 → WebSocket 连接 (1 处)<br>  - CHANGELOG.md:5 → 浏览器网络 API (1 处)<br>涉及的域名: dsh.internal, github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.en.md:23 → 敏感凭证读取 (2 处)<br>  - SECURITY.md:11 → 敏感凭证读取 (2 处)<br>  - CONTRIBUTING.md:14 → 敏感凭证读取 (1 处)<br>  - README.md:36 → 敏感凭证读取 (2 处)<br>  - lib/index.js:22 → 敏感凭证读取 (5 处)<br>  ... 共 6 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:337 → 定时任务 (1 处)<br>  - lib/client.js:167 → 定时任务 (2 处)<br>  - test/smoke-static.mjs:406 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1527, 注释行: 64 (4.2%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - scripts/check-pack.mjs:1 → 修改系统目录 (1 处)<br>  - test/smoke-static.mjs:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-session-robustness |
| 校验 URL | https://github.com/bainianlaoyao/dsh-session-robustness |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
