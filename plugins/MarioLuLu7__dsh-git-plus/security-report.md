...
- **校验对象**: https://github.com/MarioLuLu7/dsh-git-plus
- **校验时间**: 2026-09-02 03:35:18
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-08-22 (3297 天前); 公开仓库数: 26; 粉丝数: 39 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 212; 包名: dsh-git-source-control; 描述: DSH better-sidebar Source Control tab that mirrors VS Code Git workflows. — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | dsh-better-sidebar: >=0.4.0 <1.0.0 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-runtime",; Cordis/DSH 依赖: dsh-better-sidebar; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 26 处, 函数: 215 个, 比例: 12.1% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/plugin.ts; 发现清理逻辑: src/shims.d.ts; 发现清理逻辑: test/git-source-control.test.mjs |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:6 → 读取环境变量配置 (1 处)<br>  - src/plugin.ts:262 → 读取环境变量配置 (7 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - DEVELOPMENT_PLAN.md:52 → fetch 网络请求 (2 处)<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:34 → 探测到 HTTP(S) 网络请求 (14 处)<br>  - src/plugin.ts:323 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/plugin.ts:167 → fetch 网络请求 (3 处)<br>涉及的域名: dsh.local, example.com, github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - src/plugin.ts:262 → 读取环境变量 API Key/凭证 (6 处)<br>  - src/client.tsx:336 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client.tsx:560 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 6. repo: 1 deps + 5 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1506, 注释行: 0 (0.0%) |
...
| :--- | :--- |
| 插件名称 | dsh-git-plus |
| 校验 URL | https://github.com/MarioLuLu7/dsh-git-plus |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
