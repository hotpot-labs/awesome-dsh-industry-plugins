...
- **校验对象**: https://github.com/Lzz2133/dsh-manutd-fixtures
- **校验时间**: 2026-09-05 03:33:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ⚠️  需人工复核 | 账号注册: 2026-08-18 (17 天前); 公开仓库数: 2; 粉丝数: 0; ⚠️ 账号注册不足 30 天 (17 天) |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 145; 包名: dsh-manutd-fixtures; 描述: Manchester United 2026/27 fixtures in the dsh web sidebar, kickoff times in your — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v0.1.0, MU-26/27-fixtures; package.json version: 0.1.0; GitHub Releases (1): MU-26/27-fixtures |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:484 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 807, 注释行: 86 (10.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-manutd-fixtures |
| 校验 URL | https://github.com/Lzz2133/dsh-manutd-fixtures |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
