...
- **校验对象**: https://github.com/Gastronomicluna/Matrix-code
- **校验时间**: 2026-08-25 01:23:06
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-06-06 (1905 天前); 公开仓库数: 4; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 228; 包名: dsh-plugin-matrix; 描述: DeepSeek Harness 矩阵思维插件：侧边栏一键进入黑底 CLI 对话页，Agent 思考过程化作绿色代码雨倾泻而下，回复照常流式显示。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 17 处, 函数: 112 个, 比例: 15.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - client/client.js:402 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - assets/icon.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/SUBMISSION.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client/client.js:397 → 定时任务 (2 处)<br>  - scripts/smoke.js:81 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1027, 注释行: 94 (9.2%) |
...
| :--- | :--- |
| 插件名称 | Matrix-code |
| 校验 URL | https://github.com/Gastronomicluna/Matrix-code |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
