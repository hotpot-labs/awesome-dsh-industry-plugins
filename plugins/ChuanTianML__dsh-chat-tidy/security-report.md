...
- **校验对象**: https://github.com/ChuanTianML/dsh-chat-tidy
- **校验时间**: 2026-08-23 14:31:45
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 773; 包名: dsh-chat-tidy; 描述: Codex-aligned chat typography and layout for the DeepSeek Harness Web UI — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 51 个, 比例: 0.0% — 未检测到任何 try/catch |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: tests/client-lifecycle.spec.ts |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:9 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 652, 注释行: 70 (10.7%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - package.json:69 → 读取 git 历史 (1 处) — 需人工判断是否在授权范围内 |
...
| :--- | :--- |
| 插件名称 | dsh-chat-tidy |
| 校验 URL | https://github.com/ChuanTianML/dsh-chat-tidy |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
