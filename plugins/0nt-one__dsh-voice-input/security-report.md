...
- **校验对象**: https://github.com/0nt-one/dsh-voice-input
- **校验时间**: 2026-08-23 13:04:01
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ⚠️  需人工复核 | 账号注册: 2026-08-16 (7 天前); 公开仓库数: 3; 粉丝数: 0; ⚠️ 账号注册不足 30 天 (7 天) |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 235; 包名: dsh-voice-input-web; 描述: Voice input for DeepSeek Harness Web UI: a mic button in the composer tool row t — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 11 处, 函数: 21 个, 比例: 52.4% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - publish.ps1:42 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:11 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - docs/preview.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - publish.ps1:7 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 316, 注释行: 37 (11.7%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - client.js:41 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-voice-input |
| 校验 URL | https://github.com/0nt-one/dsh-voice-input |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
