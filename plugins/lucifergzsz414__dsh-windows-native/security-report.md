...
- **校验对象**: https://github.com/lucifergzsz414/dsh-windows-native
- **校验时间**: 2026-09-02 03:38:52
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-05-20 (105 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 718; 包名: dsh-windows-native; 描述: Tells DeepSeek Harness the shell rules for native Windows (no WSL) — PowerShell  — README 包含功能/使用说明 |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - dsh-windows-native-mvp-log.md:41 → 深层目录穿越 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - README.md:22 → Python subprocess (1 处)<br>  - index.js:21 → Python subprocess (2 处)<br>  - test/index.test.js:40 → Python subprocess (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 233, 注释行: 0 (0.0%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 6.1 | 无需 root 权限 | 必查 | ❌ 不通过 |   - README.md:31 → 要求 root/sudo 权限<br>  - index.js:34 → 要求 root/sudo 权限 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-windows-native |
| 校验 URL | https://github.com/lucifergzsz414/dsh-windows-native |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
