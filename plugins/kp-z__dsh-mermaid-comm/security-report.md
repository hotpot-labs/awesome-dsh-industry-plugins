...
- **校验对象**: https://github.com/kp-z/dsh-mermaid-comm
- **校验时间**: 2026-09-17 04:01:17
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-07-10 (2990 天前); 公开仓库数: 52; 粉丝数: 18 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 663; 包名: dsh-mermaid-comm; 描述: 让 AI 在开发交流中优先用 Mermaid 图表达：全局 prompt 引导 + mermaid_validate 语法校验。渲染交给 dsh-mermaid — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.1; @deepseek-ai/dsh-system-prompt: *; @deepseek-ai/dsh-tools: *; dsh-mermaid: * |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-system-prompt, @deepseek-ai/dsh-tools, dsh-mermaid; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 13 处, 函数: 81 个, 比例: 16.0% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - tsconfig.json:2 → 深层目录穿越 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:18 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: github.com, img.shields.io, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/vault.test.ts:34 → 文件读取 (1 处)<br>  - test/vault.test.ts:14 → 临时文件操作 (1 处)<br>  - src/vault.ts:154 → 文件写入 (1 处)<br>  - src/vault.ts:111 → 文件读取 (5 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 6. repo: 0 deps + 6 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1312, 注释行: 177 (13.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-mermaid-comm |
| 校验 URL | https://github.com/kp-z/dsh-mermaid-comm |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
