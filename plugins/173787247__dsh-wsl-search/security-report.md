...
- **校验对象**: https://github.com/173787247/dsh-wsl-search
- **校验时间**: 2026-09-24 03:45:53
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 143; 包名: dsh-wsl-search; 描述: DeepSeek Harness WSL plugin: sandboxed ripgrep/fd/ast-grep under allowlisted roo — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 46 个, 比例: 17.4% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/search.js:18 → 读取用户主目录 (1 处)<br>  - test/search.test.js:8 → 读取用户主目录 (3 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ❌ 不通过 | 检测到命令注入风险:<br>  - lib/search.js:10 → 命令字符串拼接变量 (注入风险) (1 处) |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.zh.md:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:44 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.en.md:46 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/awesome-entry.yml:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/search.js:49 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 330, 注释行: 2 (0.6%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 6.1 | 无需 root 权限 | 必查 | ❌ 不通过 |   - README.md:12 → 要求 root/sudo 权限<br>  - README.en.md:12 → 要求 root/sudo 权限 |
...
| :--- | :--- |
| 插件名称 | dsh-wsl-search |
| 校验 URL | https://github.com/173787247/dsh-wsl-search |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
