...
- **校验对象**: https://github.com/drscrewdriver/dsh-date-wrapper
- **校验时间**: 2026-09-12 03:49:56
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1936; 包名: dsh-date-wrapper; 描述: Minimal date line for the DeepSeek Harness runtime-context snapshot: 'Current da — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ⚠️  需人工复核 | 无 git tag; package.json version: 0.1.1-beta.1; 有版本号但无 git tag / release |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | engines: {"node": ">=20", "dsh": ">=0.1.0-rc.7 <0.2.0-0"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 3 处, 函数: 37 个, 比例: 8.1% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CHANGELOG.ja.md:4 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - HANDOVER.md:12 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - CHANGELOG.ko.md:4 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package-lock.json:21 → 探测到 HTTP(S) 网络请求 (95 处)<br>涉及的域名: eslint.org, github.com, keepachangelog.com, opencollective.com, registry.npmmirror.com, semver.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 425, 注释行: 120 (28.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-date-wrapper |
| 校验 URL | https://github.com/drscrewdriver/dsh-date-wrapper |
| 必查项通过率 | 29/33 (88%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
