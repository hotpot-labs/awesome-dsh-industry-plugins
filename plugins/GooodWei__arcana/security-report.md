...
- **校验对象**: https://github.com/GooodWei/arcana
- **校验时间**: 2026-08-23 16:28:52
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 196; 包名: arcana; 描述: Arcana — a draggable, collapsible, usage-sorted floating deck of every slash com — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.1; @deepseek-ai/dsh-api-remotes: ^0.1.0-rc.6; @deepseek-ai/dsh-commands: ^0.1.0-rc.6; @deepseek-ai/dsh-client-runtime: ^0.1.0-rc.6; @deepseek-ai/dsh-client-locale: ^0.1.0-rc.6 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-api-remotes", "@; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-api-remotes, @deepseek-ai/dsh-commands, @deepseek-ai/dsh-client-runtime, @deepseek-ai/dsh-client-locale; Cordis 配置文件: cordis.patch.yml<br>  - README.en.md:53 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 18 处, 函数: 46 个, 比例: 39.1% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/client.js:124 → 读取浏览器 Cookie/本地存储 (4 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:3 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com, raw.githubusercontent.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 457, 注释行: 31 (6.8%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:132 → 明文写入敏感信息 (2 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | arcana |
| 校验 URL | https://github.com/GooodWei/arcana |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
