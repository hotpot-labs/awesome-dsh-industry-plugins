...
- **校验对象**: https://github.com/runcat-tommy/dsh-view-manager
- **校验时间**: 2026-09-17 04:02:56
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 277; 包名: dsh-view-manager; 描述: Manage the conversation view tabs (对话/轨迹/…) in the DeepSeek Harness Web GUI: ena — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 29 处, 函数: 100 个, 比例: 29.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - README.en.md:17 → 读取浏览器 Cookie/本地存储 (1 处)<br>  - lib/index.js:46 → 读取环境变量配置 (3 处)<br>  - lib/client.js:166 → 读取浏览器 Cookie/本地存储 (4 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CHANGELOG.en.md:42 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CHANGELOG.md:32 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:22 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/index.js:87 → fetch 网络请求 (1 处)<br>涉及的域名: github.com, localhost, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:46 → 读取环境变量 API Key/凭证 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:86 → 定时任务 (2 处)<br>  - lib/client.js:255 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 940, 注释行: 58 (6.2%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:178 → 明文写入敏感信息 (2 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-view-manager |
| 校验 URL | https://github.com/runcat-tommy/dsh-view-manager |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
