...
- **校验对象**: https://github.com/Bobnemimimmi/dsh-always-status-bar
- **校验时间**: 2026-08-28 10:13:56
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-11-04 (2123 天前); 公开仓库数: 2; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 154; 包名: dsh-always-status-bar; 描述: DSH Web UI plugin: keep the native per-message status line (date \| time, run dur — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web"}}; Cordis 配置文件: cordis.patch.yml, Bobnemimimmi__dsh-always-status-bar.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:13 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:6 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - awesome-list-submission/data/plugins/Bobnemimimmi__dsh-always-status-bar.yml:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - tests/client-bundle.test.ts:51 → new Function() 任意代码执行 (1 处) |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 474, 注释行: 126 (26.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-always-status-bar |
| 校验 URL | https://github.com/Bobnemimimmi/dsh-always-status-bar |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
