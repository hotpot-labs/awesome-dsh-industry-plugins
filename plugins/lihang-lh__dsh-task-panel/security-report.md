...
- **校验对象**: https://github.com/lihang-lh/dsh-task-panel
- **校验时间**: 2026-08-25 01:26:14
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-11-03 (3216 天前); 公开仓库数: 3; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 830; 包名: dsh-task-panel; 描述: 老板任务面板：发布任务、蒸馏历史会话、阶段化子代理执行与自动读取、面板验收。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 54 处, 函数: 182 个, 比例: 29.7% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - client.js:192 → 读取浏览器 Cookie/本地存储 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - client.js:8 → fetch 网络请求 (2 处)<br>  - specs/proposals/t_mt2l3ulw_p6zbgq/tasks.md:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:148 → 临时文件操作 (1 处)<br>  - client.js:831 → 定时任务 (1 处)<br>  - index.js:32 → 临时文件操作 (5 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1752, 注释行: 68 (3.9%) |
...
| :--- | :--- |
| 插件名称 | dsh-task-panel |
| 校验 URL | https://github.com/lihang-lh/dsh-task-panel |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
