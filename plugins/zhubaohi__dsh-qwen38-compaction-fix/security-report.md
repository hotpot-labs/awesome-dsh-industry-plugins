...
- **校验对象**: https://github.com/zhubaohi/dsh-qwen38-compaction-fix
- **校验时间**: 2026-08-29 06:16:47
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-10-03 (1791 天前); 公开仓库数: 6; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1475; 包名: dsh-qwen38-ninfer-compaction-fix; 描述: DSH plugin (NInfer engine only): fixes compaction failure on local qwen3.8-27b g — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-settings: >=0.1.1-rc.1 <0.1.2 \|\| >=0.1.2-0 <0.2.0-0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 4 处, 函数: 23 个, 比例: 17.4% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - index.js:408 → fetch 网络请求 (3 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - .gitignore:5 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ⚠️  需人工复核 | 总行数: 531, 注释行: 291 (54.8%) — 注释占比过高, 可能有废弃代码 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-qwen38-compaction-fix |
| 校验 URL | https://github.com/zhubaohi/dsh-qwen38-compaction-fix |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
