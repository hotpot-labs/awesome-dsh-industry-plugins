...
- **校验对象**: https://github.com/he-yufeng/dsh-tool-reading-map
- **校验时间**: 2026-09-16 03:59:53
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-06-09 (3020 天前); 公开仓库数: 129; 粉丝数: 555 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 653; 包名: dsh-tool-reading-map; 描述: Repo reading-map tool for DeepSeek Harness: a structured, priority-ranked map of — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-tools: ^0.1.5-rc.1 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 5 处, 函数: 37 个, 比例: 13.5% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - README_CN.md:57 → 读取环境变量配置 (1 处)<br>  - README.md:57 → 读取环境变量配置 (1 处)<br>  - src/scanner.ts:44 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/scanner.ts:239 → 目录遍历 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README_CN.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package-lock.json:25 → 探测到 HTTP(S) 网络请求 (142 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, opencollective.com, registry.npmjs.org, tidelift.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - test/scanner.test.ts:89 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/scanner.test.ts:23 → 文件写入 (1 处)<br>  - src/scanner.ts:155 → 文件读取 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 6. repo: 0 deps + 6 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 542, 注释行: 14 (2.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-tool-reading-map |
| 校验 URL | https://github.com/he-yufeng/dsh-tool-reading-map |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
