...
- **校验对象**: https://github.com/kikomaotu/ccs-balance
- **校验时间**: 2026-08-27 08:16:15
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-11-04 (296 天前); 公开仓库数: 3; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 172; 包名: ccs-balance; 描述: DSH plugin: sync cc-switch provider balances and track token usage & cost by day — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 42 处, 函数: 199 个, 比例: 21.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:50 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:37 → fetch 网络请求 (3 处)<br>  - lib/index.js:415 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: dsh.internal, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:384 → API Key / Token 读取 (3 处)<br>  - lib/ccs_host_helper.py:6 → API Key / Token 读取 (11 处)<br>  - lib/ccs_host_helper.py:14 → 敏感凭证读取 (8 处) — 需人工判断是否读取的是当前会话上下文 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - lib/index.js:16 → 写入配置文件 (2 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:233 → 定时任务 (3 处)<br>  - lib/index.js:51 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1647, 注释行: 28 (1.7%) |
...
| :--- | :--- |
| 插件名称 | ccs-balance |
| 校验 URL | https://github.com/kikomaotu/ccs-balance |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
