...
- **校验对象**: https://github.com/AKS1st/ikun-theme-skin
- **校验时间**: 2026-08-23 14:22:50
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 357; 包名: ikun-theme-skin; 描述: IKUN 主题皮肤 for DeepSeek Harness — ikun 应援主题（星蓝昼/夜、背带裤黑金三套配色接入系统主题列表）、全屏壁纸轮播、基尼太美音 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 28 处, 函数: 173 个, 比例: 16.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:48 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - client.js:35 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - src/client.js:29 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - docs/INSTALL.md:17 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: cdn.jsdelivr.net, commons.wikimedia.org, down.ear0.com, github.com, upload.wikimedia.org, www.ear0.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - client.js:45 → 云存储上传 (3 处)<br>  - src/client.js:39 → 云存储上传 (3 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1326, 注释行: 100 (7.5%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 7 天; 提交总数: 5 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | ikun-theme-skin |
| 校验 URL | https://github.com/AKS1st/ikun-theme-skin |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
