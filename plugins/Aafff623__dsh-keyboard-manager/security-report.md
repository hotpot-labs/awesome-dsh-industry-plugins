...
- **校验对象**: https://github.com/Aafff623/dsh-keyboard-manager
- **校验时间**: 2026-09-12 03:43:25
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 856; 包名: dsh-keyboard-manager; 描述: Keyboard shortcuts for the DSH Web UI: Ctrl+Q left sidebar, Ctrl+E right sidebar — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"engines": {"dsh": ">=0.1.2-rc.1"}, "bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform; Cordis/DSH 依赖: @deepseek-ai/cordis; Cordis 配置文件: cordis.dev.yml, cordis.patch.yml<br>  - test/keyboard-manager.test.mjs:142 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 2 处, 函数: 109 个, 比例: 1.8% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: test/keyboard-manager.test.mjs |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.zh-CN.md:10 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - package.json:52 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:24 → 探测到 HTTP(S) 网络请求 (31 处)<br>  - README.md:10 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - cordis.patch.yml:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, registry.npmmirror.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - test/keyboard-manager.test.mjs:107 → node:vm 任意代码执行 (1 处) |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 959, 注释行: 72 (7.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-keyboard-manager |
| 校验 URL | https://github.com/Aafff623/dsh-keyboard-manager |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
