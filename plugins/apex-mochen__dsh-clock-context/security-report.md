...
- **校验对象**: https://github.com/apex-mochen/dsh-clock-context
- **校验时间**: 2026-09-16 03:58:45
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-03-06 (193 天前); 公开仓库数: 6; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1654; 包名: dsh-clock-context; 描述: Give the DSH agent a clock: injects the current date/time into the runtime conte — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 5 处, 函数: 35 个, 比例: 14.3% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - PUBLISHING.md:7 → 探测到 HTTP(S) 网络请求 (11 处)<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:26 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.md:186 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:171 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, awesome-dsh-plugin.com, github.com, keepachangelog.com, qing3a.github.io, registry.npmjs.org, semver.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - PUBLISHING.md:25 → 敏感凭证读取 (1 处)<br>  - submission/open-market-pr.ps1:7 → 敏感凭证读取 (5 处)<br>  - submission/watch-market-status.ps1:75 → 敏感凭证读取 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - submission/verified/make-submission.mjs:53 → 进程控制 (2 处)<br>  - test/smoke.mjs:73 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 561, 注释行: 127 (22.6%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - submission/open-market-pr.ps1:40 → 硬编码敏感信息 (2 处)<br>  - submission/watch-market-status.ps1:76 → 硬编码敏感信息 (2 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-clock-context |
| 校验 URL | https://github.com/apex-mochen/dsh-clock-context |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
