...
- **校验对象**: https://github.com/Pudge1996/dsh-composer-stretch
- **校验时间**: 2026-08-28 10:18:21
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2016-09-08 (3641 天前); 公开仓库数: 16; 粉丝数: 8 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 232; 包名: dsh-composer-stretch; 描述: Auto-expand composer for DeepSeek Harness: a ⤢ button appears at the top-right o — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"client": {"inject": ["@deepseek-ai/dsh-client-ui-conversation"], "platform": "web"}, "bundle": {"p; Cordis/DSH 依赖: @deepseek-ai/cordis; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 4 处, 函数: 99 个, 比例: 4.0% — try/catch 覆盖比例较低 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - tsdown.config.mjs:63 → 读取环境变量配置 (6 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - screenshots.json:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - DEV-NOTES.md:98 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (6 处)<br>涉及的域名: 127.0.0.1, github.com, img.shields.io, www.npmjs.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - tsdown.config.mjs:63 → 读取环境变量 API Key/凭证 (4 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client/index.js:343 → 定时任务 (1 处)<br>  - lib/client.js:309 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 973, 注释行: 128 (13.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-composer-stretch |
| 校验 URL | https://github.com/Pudge1996/dsh-composer-stretch |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
