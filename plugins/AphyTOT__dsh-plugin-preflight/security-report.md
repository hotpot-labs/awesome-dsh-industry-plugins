...
- **校验对象**: https://github.com/AphyTOT/dsh-plugin-preflight
- **校验时间**: 2026-09-23 03:52:54
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-08-16 (402 天前); 公开仓库数: 3; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1382; 包名: dsh-plugin-preflight; 描述: Preflight checks for a DeepSeek Harness plugin before submitting it to the commu — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.1; @deepseek-ai/dsh-tools: >=0.1.0-rc.1 <0.2.0-0 \|\| >=0.1.5-rc.1 <0.2.0-0; engines: {"node": ">=18.12"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-tools; Cordis 配置文件: cordis.patch.yml<br>  - lib/check.js:170 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 10 处, 函数: 64 个, 比例: 15.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.zh.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package.json:38 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/check.js:317 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .fixtures/wildcard-peer/package.json:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 898, 注释行: 190 (21.2%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - bin/preflight.js:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-preflight |
| 校验 URL | https://github.com/AphyTOT/dsh-plugin-preflight |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
