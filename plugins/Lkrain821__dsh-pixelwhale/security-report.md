...
- **校验对象**: https://github.com/Lkrain821/dsh-pixelwhale
- **校验时间**: 2026-09-18 03:48:54
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-09-07 (741 天前); 公开仓库数: 4; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 233; 包名: dsh-pixelwhale; 描述: 深度求索时，派一只像素蓝鲸为你喷水摆尾。 — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - FONT-LICENSE-OFL.txt:2 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package.json:17 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:13 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: github.com, takwolf.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 4.1 | 无混淆代码 | 必查 | ❌ 不通过 | 检测到潜在混淆代码:<br>  - lib/client.js:25 → 超长 base64 字符串 (2 处) |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:103 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 259, 注释行: 26 (10.0%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 6 天; 提交总数: 2 |
...
| :--- | :--- |
| 插件名称 | dsh-pixelwhale |
| 校验 URL | https://github.com/Lkrain821/dsh-pixelwhale |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
