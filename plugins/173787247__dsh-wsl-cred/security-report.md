...
- **校验对象**: https://github.com/173787247/dsh-wsl-cred
- **校验时间**: 2026-08-31 04:25:57
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 96; 包名: dsh-wsl-cred; 描述: DeepSeek Harness tool: safe Git credential hints for Windows GCM from WSL. — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 2 处, 函数: 18 个, 比例: 11.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:7 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:3 → 敏感凭证读取 (2 处)<br>  - package.json:4 → 敏感凭证读取 (2 处)<br>  - CONTRIBUTING.md:5 → 敏感凭证读取 (1 处)<br>  - README.md:3 → 敏感凭证读取 (5 处)<br>  - README.zh.md:3 → 敏感凭证读取 (2 处)<br>  ... 共 8 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 144, 注释行: 0 (0.0%) |
...
| :--- | :--- |
| 插件名称 | dsh-wsl-cred |
| 校验 URL | https://github.com/173787247/dsh-wsl-cred |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
