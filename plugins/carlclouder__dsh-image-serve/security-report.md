...
- **校验对象**: https://github.com/carlclouder/dsh-image-serve
- **校验时间**: 2026-09-05 03:36:26
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-10-11 (2520 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 259; 包名: dsh-image-serve; 描述: 让 AI 助手能在会话里直接"看到"本地图片：/ws/<绝对路径> 把磁盘上任何位置的文件变成会话 markdown 可渲染的同源 http URL。Show  — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ⚠️  需人工复核 | 无 git tag; package.json version: 2.2.2; 有版本号但无 git tag / release |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 6 处, 函数: 41 个, 比例: 14.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:13 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - lib/index.mjs:6 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - docs/before-after.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/demo-grid.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, raw.githubusercontent.com, x<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/handler.test.mjs:18 → 文件写入 (6 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 437, 注释行: 64 (14.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-image-serve |
| 校验 URL | https://github.com/carlclouder/dsh-image-serve |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
