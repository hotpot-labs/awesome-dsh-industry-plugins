...
- **校验对象**: https://github.com/WODE25500/dsh-maf
- **校验时间**: 2026-09-02 03:36:21
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 237; 包名: dsh-maf; 描述: Microsoft Agent Framework (MAF) integration for DeepSeek Harness: validate, run  — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:4 → 读取环境变量配置 (1 处)<br>  - README.en.md:33 → 读取环境变量配置 (1 处)<br>  - README.md:29 → 读取环境变量配置 (1 处)<br>  - docs/README.zh.md:53 → 读取环境变量配置 (1 处)<br>  - scripts/maf.py:99 → 读取环境变量配置 (1 处)<br>  ... 共 6 处 |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - scripts/maf.py:101 → Python subprocess (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:8 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:6 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 307, 注释行: 17 (5.5%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - scripts/maf.py:1 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | dsh-maf |
| 校验 URL | https://github.com/WODE25500/dsh-maf |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
