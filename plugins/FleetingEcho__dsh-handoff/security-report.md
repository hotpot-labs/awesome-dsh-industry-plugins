...
- **校验对象**: https://github.com/FleetingEcho/dsh-handoff
- **校验时间**: 2026-08-23 16:25:01
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 2761; 包名: @fleetingecho/dsh-handoff; 描述: Self-maintaining handoff.md per working directory + git branch for DeepSeek Harn — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 127 处, 函数: 400 个, 比例: 31.8% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/summarizer.ts:36 → 读取环境变量配置 (2 处)<br>  - src/index.ts:414 → 读取环境变量配置 (10 处)<br>  - src/store.ts:120 → 读取环境变量配置 (4 处)<br>  - lib/index.js:271 → 读取环境变量配置 (16 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/store.ts:141 → 读取用户主目录 (3 处)<br>  - lib/index.js:289 → 读取用户主目录 (3 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:223 → 敏感凭证读取 (3 处)<br>  - src/summarizer.ts:36 → 读取环境变量 API Key/凭证 (2 处)<br>  - src/summarizer.ts:141 → 敏感凭证读取 (2 处)<br>  - src/index.ts:414 → 读取环境变量 API Key/凭证 (10 处)<br>  - src/index.ts:538 → 敏感凭证读取 (2 处)<br>  ... 共 12 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/summarizer.ts:50 → 定时任务 (1 处)<br>  - src/index.ts:117 → 进程控制 (1 处)<br>  - lib/index.js:1244 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 0 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 5166, 注释行: 479 (9.3%) |
...
| :--- | :--- |
| 插件名称 | dsh-handoff |
| 校验 URL | https://github.com/FleetingEcho/dsh-handoff |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
