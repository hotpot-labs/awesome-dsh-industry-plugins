...
- **校验对象**: https://github.com/Frog755/dsh-client-auto-retry
- **校验时间**: 2026-08-23 16:25:42
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-12-19 (612 天前); 公开仓库数: 15; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 628; 包名: @frog755/dsh-client-auto-retry; 描述: DSH client plugin: 检测回合被中断/出错/超长时, 自动发送「继续」重试, 不做模型/provider 切换。带设置卡片。 — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"client": {"inject": [], "platform": "web"}, "bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 18 处, 函数: 41 个, 比例: 43.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:15 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - README-EN.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - assets/demo.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com, img.shields.io, keepachangelog.com, semver.org, university.aliyun.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:80 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 546, 注释行: 33 (6.0%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-client-auto-retry |
| 校验 URL | https://github.com/Frog755/dsh-client-auto-retry |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
