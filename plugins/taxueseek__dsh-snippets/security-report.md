...
- **校验对象**: https://github.com/taxueseek/dsh-snippets
- **校验时间**: 2026-08-25 01:28:03
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 199; 包名: dsh-snippets; 描述: 极简常用片段/命令工具箱 for DeepSeek Harness: JSONL-backed, five tools, no UI, no deps beyo — README 包含功能/使用说明 |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/index.ts:35 → 读取用户主目录 (1 处)<br>  - lib/index.js:22 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package-lock.json:23 → 探测到 HTTP(S) 网络请求 (20 处)<br>涉及的域名: registry.npmmirror.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/snippets.test.ts:33 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 0 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 787, 注释行: 53 (6.7%) |
...
| :--- | :--- |
| 插件名称 | dsh-snippets |
| 校验 URL | https://github.com/taxueseek/dsh-snippets |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
