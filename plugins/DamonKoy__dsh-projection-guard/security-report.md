...
- **校验对象**: https://github.com/DamonKoy/dsh-projection-guard
- **校验时间**: 2026-08-23 14:32:50
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 388; 包名: dsh-projection-guard; 描述: Guards the persisted session-projection cache: per-row JSON degradation so one m — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.1; @deepseek-ai/dsh-session: ^0.1.0-rc.6; @deepseek-ai/dsh-session-projection: ^0.1.0-rc.6; @deepseek-ai/dsh-session-projection-cache: ^0.1.0-rc.6; @deepseek-ai/dsh-session-persistence: ^0.1.0-rc.6 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-session, @deepseek-ai/dsh-session-projection, @deepseek-ai/dsh-session-projection-cache, @deepseek-ai/dsh-session-persistence; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 7 处, 函数: 33 个, 比例: 21.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - client/client.js:15 → fetch 网络请求 (1 处)<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client/client.js:35 → 定时任务 (1 处)<br>  - lib/index.js:129 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 303, 注释行: 69 (22.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-projection-guard |
| 校验 URL | https://github.com/DamonKoy/dsh-projection-guard |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
