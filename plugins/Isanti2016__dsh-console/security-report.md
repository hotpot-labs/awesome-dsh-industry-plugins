...
- **校验对象**: https://github.com/Isanti2016/dsh-console
- **校验时间**: 2026-08-23 16:31:47
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 667; 包名: dsh-console; 描述: Console commands for DeepSeek Harness: web service / SSH tunnel management, one- — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 7 处, 函数: 40 个, 比例: 17.5% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:4 → 读取 npm 凭证 (1 处)<br>  - README.md:23 → 读取 SSH 私钥目录 (2 处)<br>  - README.md:23 → 读取 SSH 私钥 (2 处)<br>  - README.zh.md:23 → 读取 SSH 私钥目录 (2 处)<br>  - README.zh.md:23 → 读取 SSH 私钥 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - marketplace/SUBMIT.md:4 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - lib/backend.js:31 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - lib/backend.js:31 → fetch 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/backend.test.js:58 → 进程控制 (1 处)<br>  - lib/backend.js:21 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 369, 注释行: 42 (11.4%) |
...
| :--- | :--- |
| 插件名称 | dsh-console |
| 校验 URL | https://github.com/Isanti2016/dsh-console |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
