...
- **校验对象**: https://github.com/john-walks-slow/dsh-set-model
- **校验时间**: 2026-09-21 04:01:09
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 415; 包名: dsh-set-model; 描述: Dynamic model and reasoning-effort switching for DeepSeek Harness (DSH): set_mod — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: 4.0.2; @deepseek-ai/dsh-agent: 0.1.2-rc.1; @deepseek-ai/dsh-llm: 0.1.2-rc.1; @deepseek-ai/dsh-session: 0.1.2-rc.1; @deepseek-ai/dsh-tools: 0.1.2-rc.1 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 10 处, 函数: 63 个, 比例: 15.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:51 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:34 → 探测到 HTTP(S) 网络请求 (45 处)<br>  - docs/freeform/260917-awesome-entry-draft.yml:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.en.md:136 → 临时文件操作 (1 处)<br>  - package.json:24 → 临时文件操作 (1 处)<br>  - README.md:136 → 临时文件操作 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 13. repo: 7 deps + 6 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1101, 注释行: 60 (5.4%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - AGENTS.md:21 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | dsh-set-model |
| 校验 URL | https://github.com/john-walks-slow/dsh-set-model |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
