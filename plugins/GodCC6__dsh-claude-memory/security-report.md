...
- **校验对象**: https://github.com/GodCC6/dsh-claude-memory
- **校验时间**: 2026-09-13 03:51:19
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1511; 包名: dsh-claude-memory; 描述: Read-only DeepSeek Harness plugin that surfaces Claude Code's project memory, re — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 28 处, 函数: 192 个, 比例: 14.6% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - test/schema.mjs:22 → 读取用户主目录 (2 处)<br>  - src/paths.js:43 → 读取用户主目录 (3 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:42 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:23 → 敏感凭证读取 (8 处)<br>  - README.md:195 → 会话历史读取 (1 处)<br>  - README.zh.md:112 → 敏感凭证读取 (2 处)<br>  - cordis.patch.yml:18 → 敏感凭证读取 (1 处)<br>  - test/plugin.mjs:149 → 敏感凭证读取 (1 处)<br>  ... 共 14 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/plugin.mjs:275 → 进程控制 (1 处)<br>  - test/smoke.mjs:414 → 进程控制 (1 处)<br>  - test/schema.mjs:35 → 进程控制 (2 处)<br>  - test/live.mjs:27 → 进程控制 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2374, 注释行: 502 (21.1%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - test/smoke.mjs:103 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - test/smoke.mjs:98 → 修改系统目录 (2 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-claude-memory |
| 校验 URL | https://github.com/GodCC6/dsh-claude-memory |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
