...
- **校验对象**: https://github.com/H2O-MERO/dsh-git-sidebar
- **校验时间**: 2026-08-28 10:15:59
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-02-12 (2389 天前); 公开仓库数: 12; 粉丝数: 21 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 175; 包名: dsh-git-sidebar; 描述: DSH plugin: VSCode-style uncommitted changes sidebar for the DeepSeek Harness we — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"client": {"inject": ["slots"], "platform": "web"}, "bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:61 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:61 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - client.js:108 → fetch 网络请求 (1 处)<br>  - index.js:144 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, localhost<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 814, 注释行: 53 (6.5%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - client.js:103 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - index.js:92 → 读取 git 历史 (1 处) — 需人工判断是否在授权范围内 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-git-sidebar |
| 校验 URL | https://github.com/H2O-MERO/dsh-git-sidebar |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
