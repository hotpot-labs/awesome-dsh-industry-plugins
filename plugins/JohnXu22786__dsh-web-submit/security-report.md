...
- **校验对象**: https://github.com/JohnXu22786/dsh-web-submit
- **校验时间**: 2026-08-28 10:16:48
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 270; 包名: dsh-web-submit; 描述: DeepSeek Harness plugin: run headless-style tasks through the running dsh web pr — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 14 个, 比例: 57.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:36 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - index.js:36 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - index.js:85 → fetch 网络请求 (1 处)<br>  - examples/usage.mjs:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - examples/usage.mjs:11 → fetch 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, localhost<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.js:214 → 定时任务 (1 处)<br>  - examples/usage.mjs:37 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 268, 注释行: 31 (11.6%) |
...
| :--- | :--- |
| 插件名称 | dsh-web-submit |
| 校验 URL | https://github.com/JohnXu22786/dsh-web-submit |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
