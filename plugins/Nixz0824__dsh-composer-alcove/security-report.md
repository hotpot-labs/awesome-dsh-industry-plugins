...
- **校验对象**: https://github.com/Nixz0824/dsh-composer-alcove
- **校验时间**: 2026-09-07 03:35:23
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-04-14 (510 天前); 公开仓库数: 11; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 615; 包名: dsh-composer-alcove; 描述: DSH composer: scroll to an iPhone home-indicator pill; corner arcs dock the real — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (4): v0.5.9, v0.5.8, v0.5.4, v0.5.3; package.json version: 0.5.9; GitHub Releases (4): v0.5.9, v0.5.8, v0.5.4, v0.5.3 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"client": {"inject": ["@deepseek-ai/dsh-client-runtime"], "platform": "web"}, "bundle": {"patch": "; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 4 处, 函数: 94 个, 比例: 4.3% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:9 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - README.zh.md:7 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:162 → 定时任务 (4 处)<br>  - scripts/smoke.mjs:86 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1186, 注释行: 36 (3.0%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-composer-alcove |
| 校验 URL | https://github.com/Nixz0824/dsh-composer-alcove |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
