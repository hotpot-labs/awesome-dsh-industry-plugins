...
- **校验对象**: https://github.com/KarlOfLaw/dsh-goal-mode-enhance
- **校验时间**: 2026-08-25 01:23:29
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 385; 包名: dsh-goal-mode; 描述: Enhanced goal surface for the DeepSeek Harness web UI: full lifecycle dock above — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: *; @deepseek-ai/dsh-client-runtime: *; @deepseek-ai/dsh-client-ui-slots: *; @deepseek-ai/dsh-client-ui-settings: *; @deepseek-ai/dsh-client-locale: * |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["@deepseek-ai/d; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-client-runtime, @deepseek-ai/dsh-client-ui-slots, @deepseek-ai/dsh-client-ui-settings, @deepseek-ai/dsh-client-locale; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 14 处, 函数: 166 个, 比例: 8.4% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - INSTALL.md:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1787, 注释行: 171 (9.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-goal-mode-enhance |
| 校验 URL | https://github.com/KarlOfLaw/dsh-goal-mode-enhance |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
