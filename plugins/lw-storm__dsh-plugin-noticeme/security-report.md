...
- **校验对象**: https://github.com/lw-storm/dsh-plugin-noticeme
- **校验时间**: 2026-09-03 03:33:48
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-08-02 (31 天前); 公开仓库数: 5; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 196; 包名: dsh-plugin-noticeme; 描述: Notify me when DSH needs me: desktop notification (with tab-title fallback) when — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (4): v2.2.0, v2.1.0, v2.0.0, v1.0.0; package.json version: 2.2.0 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-connectio; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-tools; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 29 处, 函数: 50 个, 比例: 58.0% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - client/client.js:102 → fetch 网络请求 (2 处)<br>  - lib/index.js:253 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: localhost<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client/client.js:112 → 定时任务 (2 处)<br>  - lib/index.js:171 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 453, 注释行: 68 (15.0%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-noticeme |
| 校验 URL | https://github.com/lw-storm/dsh-plugin-noticeme |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
