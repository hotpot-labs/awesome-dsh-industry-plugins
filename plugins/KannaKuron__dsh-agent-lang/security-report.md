...
- **校验对象**: https://github.com/KannaKuron/dsh-agent-lang
- **校验时间**: 2026-09-03 03:32:34
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-09-21 (1443 天前); 公开仓库数: 12; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 326; 包名: dsh-agent-lang; 描述: DSH plugin: make every tool-call `description` (including run_code's, in any pre — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (4): v0.4.1, v0.4.0, v0.3.1, v0.2.0; package.json version: 0.4.1; GitHub Releases (4): v0.4.1, v0.4.0, v0.3.1, v0.2.0 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-runtime",; Cordis/DSH 依赖: @deepseek-ai/cordis; Cordis 配置文件: cordis.patch.yml<br>  - cordis.patch.yml:9 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 39 处, 函数: 118 个, 比例: 33.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README_KO.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README_JA.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README_EN.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, registry.npmjs.org, registry.npmmirror.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1299, 注释行: 325 (25.0%) |
...
| :--- | :--- |
| 插件名称 | dsh-agent-lang |
| 校验 URL | https://github.com/KannaKuron/dsh-agent-lang |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
