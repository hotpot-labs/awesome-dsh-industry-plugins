...
- **校验对象**: https://github.com/Young4ever33/dsh-token-attention
- **校验时间**: 2026-08-25 01:24:34
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-07-23 (762 天前); 公开仓库数: 9; 粉丝数: 30 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 617; 包名: dsh-token-attention; 描述: Token Check（词元管理）— token 注意力管理面板（DSH 插件） — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-client-runtime: ^0.1.1-rc.1; @deepseek-ai/dsh-client-ui-slots: ^0.1.1-rc.1; engines: {"node": ">=24"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["@deepseek-ai/d; Cordis/DSH 依赖: @deepseek-ai/dsh-client-runtime, @deepseek-ai/dsh-client-ui-slots; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 32 处, 函数: 301 个, 比例: 10.6% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/node/demo.ts:127 → 深层目录穿越 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:9 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:32 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.zh.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - src/client/index.js:263 → fetch 网络请求 (10 处)<br>涉及的域名: 127.0.0.1, dsh.local, github.com, mirrors.cloud.tencent.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client/index.js:160 → 定时任务 (2 处)<br>  - src/node/index.ts:87 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 0 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2865, 注释行: 238 (8.3%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-token-attention |
| 校验 URL | https://github.com/Young4ever33/dsh-token-attention |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
