...
- **校验对象**: https://github.com/boheastill/phone-eye
- **校验时间**: 2026-08-27 08:15:13
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-08-29 (2919 天前); 公开仓库数: 30; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 723; 包名: phone-eye; 描述: Let your AI agent see and operate a real Android phone — vision + UI-tree fusion — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - server.py:37 → 读取环境变量配置 (4 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - server.py:48 → Python subprocess (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:46 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - server.py:38 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - examples/loop-mobile-web-qa.md:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/fleet.md:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/PROJECT-internal.md:36 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, 192.168.31.109, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - examples/device-setup-wizard.md:9 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - SECURITY.md:5 → 临时文件操作 (1 处)<br>  - server.py:40 → 临时文件操作 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 223, 注释行: 5 (2.2%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - server.py:1 → 修改系统目录 (1 处)<br>  - docs/fleet.md:13 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | phone-eye |
| 校验 URL | https://github.com/boheastill/phone-eye |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
