...
- **校验对象**: https://github.com/polohot/dsh-adrian-inject-context
- **校验时间**: 2026-09-14 04:00:17
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-10-11 (2894 天前); 公开仓库数: 12; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 575; 包名: dsh-adrian-inject-context; 描述: Inject Context for DeepSeek Harness (DSH): persist your own context entries and  — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (4): v0.3.2, v0.3.1, v0.3.0, v0.2.0; package.json version: 0.3.2 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 22 处, 函数: 135 个, 比例: 16.3% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:43 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:889 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:219 → fetch 网络请求 (2 处)<br>  - lib/client.js:115 → fetch 网络请求 (2 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:133 → 定时任务 (1 处)<br>  - lib/client.js:142 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1284, 注释行: 201 (15.7%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 7 天; 提交总数: 9 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-adrian-inject-context |
| 校验 URL | https://github.com/polohot/dsh-adrian-inject-context |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
