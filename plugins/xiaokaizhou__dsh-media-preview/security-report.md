...
- **校验对象**: https://github.com/xiaokaizhou/dsh-media-preview
- **校验时间**: 2026-09-03 03:35:25
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-01-04 (3529 天前); 公开仓库数: 22; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 316; 包名: media-preview; 描述: DSH 插件：在聊天记录中自动将本地音视频/图片路径渲染为可播放的预览组件。When an assistant message or tool result c — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 14 处, 函数: 23 个, 比例: 60.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:3 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - .github/awesome-dsh-plugin-contributing.md:16 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .github/demo-preview.png:22 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: cdn.site.com, example.com, github.com, img.shields.io, raw.githubusercontent.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.en.md:18 → 临时文件操作 (3 处)<br>  - README.md:18 → 临时文件操作 (3 处)<br>  - client/client.js:13 → 临时文件操作 (7 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 683, 注释行: 142 (20.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-media-preview |
| 校验 URL | https://github.com/xiaokaizhou/dsh-media-preview |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
