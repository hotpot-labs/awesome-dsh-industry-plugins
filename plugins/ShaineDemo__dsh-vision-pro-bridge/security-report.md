...
- **校验对象**: https://github.com/ShaineDemo/dsh-vision-pro-bridge
- **校验时间**: 2026-09-05 03:35:11
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-02-26 (921 天前); 公开仓库数: 6; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 632; 包名: dsh-vision-pro-bridge; 描述: Give text-only DeepSeek-V4-Pro real vision with zero new dependencies and DeepSe — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | engines: {"node": ">=22", "dsh": ">=0.1.1-rc.2"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 11 处, 函数: 17 个, 比例: 64.7% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/index.js:39 → 读取环境变量配置 (4 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:39 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.zh-CN.md:53 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:26 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (6 处)<br>涉及的域名: api.deepseek.com, github.com, img.shields.io, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.zh-CN.md:11 → API Key / Token 读取 (3 处)<br>  - README.zh-CN.md:52 → 敏感凭证读取 (1 处)<br>  - package.json:4 → API Key / Token 读取 (1 处)<br>  - README.md:17 → API Key / Token 读取 (3 处)<br>  - README.md:60 → 敏感凭证读取 (1 处)<br>  ... 共 8 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - README.md:29 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 286, 注释行: 30 (10.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-vision-pro-bridge |
| 校验 URL | https://github.com/ShaineDemo/dsh-vision-pro-bridge |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
