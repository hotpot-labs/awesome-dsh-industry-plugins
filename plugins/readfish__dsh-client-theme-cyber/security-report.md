...
- **校验对象**: https://github.com/readfish/dsh-client-theme-cyber
- **校验时间**: 2026-09-14 04:00:40
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-09-15 (1094 天前); 公开仓库数: 2; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 352; 包名: @anywhere-labs/dsh-client-theme-cyber; 描述: Cyber Sand theme for DeepSeek Harness Web: a dark cyber-city palette (from the r — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 8 个, 比例: 0.0% — 未检测到任何 try/catch |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - market/SUBMIT.md:22 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - market/catalog-entry.theme-cyber.json:2 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - market/data/plugins/readfish__dsh-client-theme-cyber.yml:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com, raw.githubusercontent.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - README.md:8 → 云存储上传 (2 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 412, 注释行: 78 (18.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-client-theme-cyber |
| 校验 URL | https://github.com/readfish/dsh-client-theme-cyber |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
