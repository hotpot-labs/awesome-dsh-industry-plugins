...
- **校验对象**: https://github.com/MoonlitDropOfBlood/dsh-whale-pet
- **校验时间**: 2026-09-07 03:35:09
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2016-01-08 (3894 天前); 公开仓库数: 35; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 357; 包名: @duke-dsh-plugins/dsh-whaleharbor-pet; 描述: 鲸港 WhaleHarbor 大白鲸娘桌面窗宠：透明置顶浮窗，实时反映 DSH 空闲/繁忙/审批/子代理等工作状态。2D 拆件 + CSS 纸片人动画。 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (3): v1.1.1, v1.1.0, v1.0.0; package.json version: 1.1.1; GitHub Releases (3): v1.1.1, v1.1.0, v1.0.0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 33 处, 函数: 62 个, 比例: 53.2% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - index.js:30 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - index.js:408 → 目录遍历 (1 处)<br>  - index.js:37 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ❌ 不通过 | 检测到命令注入风险:<br>  - index.js:365 → exec 命令拼接 (1 处) |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - index.js:32 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - index.js:67 → fetch 网络请求 (2 处)<br>  - CHANGELOG.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:29 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:48 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, github.com, keepachangelog.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - index.js:30 → 读取环境变量 API Key/凭证 (3 处)<br>  - .github/workflows/release.yml:108 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.js:656 → 开启监听端口 (1 处)<br>  - index.js:367 → 定时任务 (6 处)<br>  - index.js:47 → 文件写入 (1 处)<br>  - index.js:41 → 文件读取 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - .github/workflows/release.yml:37 → 云存储上传 (2 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 771, 注释行: 69 (8.9%) |
...
| :--- | :--- |
| 插件名称 | dsh-whale-pet |
| 校验 URL | https://github.com/MoonlitDropOfBlood/dsh-whale-pet |
| 必查项通过率 | 20/33 (61%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
