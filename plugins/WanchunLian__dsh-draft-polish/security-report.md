...
- **校验对象**: https://github.com/WanchunLian/dsh-draft-polish
- **校验时间**: 2026-09-19 03:47:23
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-10-25 (694 天前); 公开仓库数: 3; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 621; 包名: dsh-draft-polish; 描述: DSH Web 插件：一键把口语化草稿润色成专业表达（默认快路径出核心意思，更多风格按需展开，替换回输入框由用户确认再发送） — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-runtime", "@dee; Cordis 配置文件: cordis.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 17 处, 函数: 96 个, 比例: 17.7% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:72 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:36 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:48 → fetch 网络请求 (1 处)<br>  - docs/design.md:382 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/design.md:505 → fetch 网络请求 (1 处)<br>涉及的域名: 127.0.0.1<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:277 → 定时任务 (1 处)<br>  - lib/polish-core.js:59 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1075, 注释行: 126 (11.7%) |
...
| 校验 URL | https://github.com/WanchunLian/dsh-draft-polish |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
