...
- **校验对象**: https://github.com/131CDA1/dsh-scrape-webpage
- **校验时间**: 2026-08-23 13:05:14
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-06-18 (1526 天前); 公开仓库数: 9; 粉丝数: 7 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 426; 包名: dsh-scrape-webpage; 描述: 网页抓取与分析插件：抓取网页正文并给出统计与关键词分析，支持内容图片下载与识图分析器扩展接口。 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ❌ 不通过 | 检测到潜在违规模式:<br>  - README.md:5 → 非法爬取 (1 处) |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | 无 git tag; package.json version: 1.0.0; GitHub Releases (2): dsh-scrape-webpage-v1.0.1, dsh-scrape-webpage |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 18 处, 函数: 37 个, 比例: 48.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:28 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:54 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - CHANGELOG.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/html.js:38 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:91 → fetch 网络请求 (1 处)<br>涉及的域名: example.com, github.com, keepachangelog.com, semver.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:105 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 721, 注释行: 61 (8.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-scrape-webpage |
| 校验 URL | https://github.com/131CDA1/dsh-scrape-webpage |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
