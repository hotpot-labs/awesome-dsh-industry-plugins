...
- **校验对象**: https://github.com/chouxiaohuai/dsh-uiskin-theme
- **校验时间**: 2026-08-25 01:24:59
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-07-06 (49 天前); 公开仓库数: 2; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: UNLICENSED (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 687; 包名: uiskin-theme; 描述: Blue Glass Theme (ocean fantasy) for DeepSeek Harness Web — a static profile bun — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 21 个, 比例: 0.0% — 未检测到任何 try/catch |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:22 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - scripts/client.template.js:54 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/client.js:54 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: pnpm.io, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/build-client.mjs:27 → 进程控制 (3 处)<br>  - lib/client.js:17 → 临时文件操作 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 355, 注释行: 55 (15.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-uiskin-theme |
| 校验 URL | https://github.com/chouxiaohuai/dsh-uiskin-theme |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
