...
- **校验对象**: https://github.com/kaka-crypto/dsh-disk-guard
- **校验时间**: 2026-08-30 04:32:58
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-12-22 (2442 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 490; 包名: dsh-disk-guard; 描述: 磁盘守护：把 DSH 的下载/产物/缓存/临时文件重定向到用户指定目录（默认自动挑剩余空间最大的非 C 盘），向每个会话注入『路径纪律』提示词，并提供 disk — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 60 处, 函数: 66 个, 比例: 90.9% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - README.en.md:12 → 读取环境变量配置 (1 处)<br>  - config.example.yaml:13 → 读取环境变量配置 (1 处)<br>  - README.md:13 → 读取环境变量配置 (3 处)<br>  - VERIFICATION.md:13 → 读取环境变量配置 (2 处)<br>  - src/index.ts:12 → 读取环境变量配置 (9 处)<br>  ... 共 6 处 |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/index.ts:92 → 读取用户主目录 (3 处)<br>  - lib/index.js:11 → 读取用户主目录 (3 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:19 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - submission/kaka-crypto__dsh-disk-guard.yml:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - src/index.ts:91 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/index.js:10 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - src/index.ts:548 → 写入配置文件 (1 处)<br>  - lib/index.js:392 → 写入配置文件 (1 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.en.md:10 → 临时文件操作 (2 处)<br>  - cordis.patch.yml:13 → 临时文件操作 (1 处)<br>  - config.example.yaml:8 → 临时文件操作 (2 处)<br>  - README.md:11 → 临时文件操作 (4 处)<br>  - VERIFICATION.md:20 → 临时文件操作 (1 处)<br>  ... 共 7 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1241, 注释行: 94 (7.6%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - README.md:129 → Windows 系统配置修改 (1 处)<br>  - scripts/migrate-c-drive.ps1:76 → Windows 系统配置修改 (1 处) |
...
| :--- | :--- |
| 插件名称 | dsh-disk-guard |
| 校验 URL | https://github.com/kaka-crypto/dsh-disk-guard |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
