...
- **校验对象**: https://github.com/xyAxzy/dsh-conversation-map
- **校验时间**: 2026-09-03 03:35:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ⚠️  需人工复核 | 账号注册: 2026-08-23 (10 天前); 公开仓库数: 2; 粉丝数: 0; ⚠️ 账号注册不足 30 天 (10 天) |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 130; 包名: dsh-conversation-map; 描述: Conversation map for DeepSeek Harness web: a live tree of every session in the w — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "external": ["react"]}}; Cordis 配置文件: cordis.patch.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - lib/index.js:407 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:1841 → fetch 网络请求 (2 处)<br>  - lib/client.js:1858 → 浏览器网络 API (1 处)<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:212 → 定时任务 (2 处)<br>  - lib/client.js:1728 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2849, 注释行: 445 (15.6%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:671 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 校验 URL | https://github.com/xyAxzy/dsh-conversation-map |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
