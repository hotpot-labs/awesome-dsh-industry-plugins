...
- **校验对象**: https://github.com/qys0012/dsh-hotspots
- **校验时间**: 2026-08-30 04:34:58
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ⚠️  需人工复核 | 账号注册: 2026-08-21 (8 天前); 公开仓库数: 2; 粉丝数: 0; ⚠️ 账号注册不足 30 天 (8 天) |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 402; 包名: dsh-hotspots; 描述: 时事热点：AI 大事 + 工科热点 实时聚合面板（DeepSeek Harness Web 插件）。侧边栏入口 + 三视图面板 + 设置管理卡片，数据来自 ds — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v1.0.1, v1.0.0; package.json version: 1.1.0; GitHub Releases (1): v1.0.0 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"engines": {"dsh": ">=0.1.1-rc.1"}, "bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject":; Cordis 配置文件: cordis.patch.yml<br>  - dsh/client.js:143 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 36 处, 函数: 104 个, 比例: 34.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:34 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - dsh/client.js:158 → fetch 网络请求 (3 处)<br>  - dsh/index.js:253 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: a.com, b.com, c.com, example.org, github.com, img.shields.io, keepachangelog.com, mp.weixin.qq.com, semver.org, sub.douyin.com (+6 更多)<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - dsh/client.js:160 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1004, 注释行: 48 (4.8%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - dsh/client.js:137 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | dsh-hotspots |
| 校验 URL | https://github.com/qys0012/dsh-hotspots |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
