...
- **校验对象**: https://github.com/Bing-Bryan/dsh-unread-dot
- **校验时间**: 2026-08-23 14:28:07
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-11-11 (285 天前); 公开仓库数: 4; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 297; 包名: dsh-unread-dot; 描述: DSH web plugin: macOS Dock unread red dot (dot = running, number = results) + bu — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 17 处, 函数: 49 个, 比例: 34.7% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:5 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package.json:33 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: developer.chrome.com, github.com, img.shields.io, pixabay.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 339, 注释行: 41 (12.1%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:80 → 明文写入敏感信息 (2 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | dsh-unread-dot |
| 校验 URL | https://github.com/Bing-Bryan/dsh-unread-dot |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
