...
- **校验对象**: https://github.com/himeope/dsh-queue-first-enter
- **校验时间**: 2026-09-12 03:50:28
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-06-20 (1910 天前); 公开仓库数: 7; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 768; 包名: dsh-queue-first-enter; 描述: DSH Web plugin: pressing Enter with an empty composer immediately steers the fir — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-client-runtime: >=0.0.1-rc.5 <0.1.0 \|\| >=0.1.0-rc.1 <0.2.0-0; engines: {"node": ">=20"} |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 16 处, 函数: 59 个, 比例: 27.1% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - client.js:44 → 读取浏览器 Cookie/本地存储 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:34 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - smoke-test.mjs:29 → new Function() 任意代码执行 (1 处) |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 477, 注释行: 50 (10.5%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - client.js:44 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-queue-first-enter |
| 校验 URL | https://github.com/himeope/dsh-queue-first-enter |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
