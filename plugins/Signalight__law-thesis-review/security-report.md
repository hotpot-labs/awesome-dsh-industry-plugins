...
- **校验对象**: https://github.com/Signalight/law-thesis-review
- **校验时间**: 2026-09-20 04:01:57
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-02-28 (204 天前); 公开仓库数: 6; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 311; 包名: @signalight/dsh-law-thesis-review; 描述: 国际经济法论文审查 skill：读取学生 Word 论文，对照自查清单，以 Word 修订+批注输出审查稿。 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (1): dsh-plugin; package.json version: 0.3.0; GitHub Releases (1): dsh-plugin |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 2 处, 函数: 7 个, 比例: 28.6% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - skills/iet-law-thesis-review/scripts/review_docx.py:369 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - skills/iet-law-thesis-review/scripts/add_comment.py:86 → 探测到 HTTP(S) 网络请求 (26 处)<br>  - skills/iet-law-thesis-review/scripts/review_docx.py:40 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - skills/iet-law-thesis-review/scripts/apply_edits.py:32 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: purl.org, schemas.microsoft.com, schemas.openxmlformats.org, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2234, 注释行: 112 (5.0%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - skills/iet-law-thesis-review/scripts/review_docx.py:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | law-thesis-review |
| 校验 URL | https://github.com/Signalight/law-thesis-review |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
