...
- **校验对象**: https://github.com/lyxx999/Automatic-session-renaming-for-dsh
- **校验时间**: 2026-09-08 03:44:19
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-02-27 (557 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 371; 包名: @lyxx/dsh-session-autotitle; 描述: DSH 会话自动命名：全消息 LLM 总结标题（首条提示词/handoff 自动 + 会话头按钮/命令手动），标题调用强制关思考，标题语言与最大字节数可设置。 — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.0; @deepseek-ai/dsh-llm: *; @deepseek-ai/dsh-session-title: *; @deepseek-ai/dsh-timeout: * |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web"}}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-llm, @deepseek-ai/dsh-session-title, @deepseek-ai/dsh-timeout; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 19 处, 函数: 232 个, 比例: 8.2% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: test/title.test.mjs; 发现清理逻辑: src/title.mjs |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - test/host.test.mjs:18 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, localhost<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - test/host.test.mjs:19 → API Key / Token 读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1933, 注释行: 129 (6.7%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 12 天; 提交总数: 6 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | Automatic-session-renaming-for-dsh |
| 校验 URL | https://github.com/lyxx999/Automatic-session-renaming-for-dsh |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
