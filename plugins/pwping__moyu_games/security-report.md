...
- **校验对象**: https://github.com/pwping/moyu_games
- **校验时间**: 2026-08-28 10:31:19
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2015-08-14 (4032 天前); 公开仓库数: 3; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: Apache-2.0 (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 318; 包名: moyu-games; 描述: A 'slacking off' mini-games plugin for DeepSeek Harness: five casual games (slid — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 21 处, 函数: 473 个, 比例: 4.4% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/index.ts; 发现清理逻辑: src/stream.ts; 发现清理逻辑: tests/stream.spec.ts |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:89 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:49 → 探测到 HTTP(S) 网络请求 (319 处)<br>  - README.md:46 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - src/client/styles.ts:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/client/api.ts:52 → 浏览器网络 API (1 处)<br>涉及的域名: fonts.googleapis.com, github.com, opencollective.com, registry.npmmirror.com, tidelift.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - package-lock.json:442 → 敏感凭证读取 (5 处)<br>  - tsdown.config.ts:72 → 读取环境变量 API Key/凭证 (4 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/stream.ts:99 → 定时任务 (1 处)<br>  - src/client/ui.tsx:191 → 定时任务 (7 处)<br>  - lib/client.js:1507 → 定时任务 (7 处)<br>  - lib/index.js:57 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ⚠️  需人工复核 | 总依赖数: 24. repo: 1 deps + 23 devDeps — 依赖较多, 需人工审查 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 5997, 注释行: 820 (13.7%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - src/client/ui.tsx:51 → 明文写入敏感信息 (2 处)<br>  - lib/client.js:1350 → 明文写入敏感信息 (2 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | moyu_games |
| 校验 URL | https://github.com/pwping/moyu_games |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
