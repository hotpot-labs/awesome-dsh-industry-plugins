...
- **校验对象**: https://github.com/nobu121/dsh-opencode-session
- **校验时间**: 2026-09-17 04:02:03
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-01-28 (2058 天前); 公开仓库数: 12; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 720; 包名: dsh-opencode-session; 描述: DeepSeek Harness plugin: automatically adds a stable per-conversation x-opencode — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 51 个, 比例: 15.7% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:42 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - tests/test.mjs:66 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - tests/test.mjs:84 → fetch 网络请求 (6 处)<br>  - lib/index.js:132 → fetch 网络请求 (3 处)<br>涉及的域名: 127.0.0.1, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - tests/test.mjs:86 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - tests/test.mjs:65 → 开启监听端口 (1 处)<br>  - tests/test.mjs:214 → 定时任务 (1 处)<br>  - tests/test.mjs:248 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 468, 注释行: 80 (17.1%) |
...
| 7.6 | 有社区背书 | 推荐 | ⚠️  需人工复核 | Stars: 3, Forks: 3, Watchers: 0 — 社区背书不足, 需人工判断 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-opencode-session |
| 校验 URL | https://github.com/nobu121/dsh-opencode-session |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
