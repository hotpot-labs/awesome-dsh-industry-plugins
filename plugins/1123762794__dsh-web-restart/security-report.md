...
- **校验对象**: https://github.com/1123762794/dsh-web-restart
- **校验时间**: 2026-08-23 13:05:00
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-11-02 (2851 天前); 公开仓库数: 8; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 554; 包名: dsh-web-restart; 描述: One-click restart button for the DeepSeek Harness Web UI: a sidebar footer butto — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 5 处, 函数: 26 个, 比例: 19.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - lib/client.js:68 → fetch 网络请求 (2 处)<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:73 → 定时任务 (1 处)<br>  - lib/index.js:49 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 220, 注释行: 34 (15.5%) |
...
| 7.6 | 有社区背书 | 推荐 | ⚠️  需人工复核 | Stars: 5, Forks: 5, Watchers: 0 — 社区背书不足, 需人工判断 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-web-restart |
| 校验 URL | https://github.com/1123762794/dsh-web-restart |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
