...
- **校验对象**: https://github.com/RrcoVer0/dsh-webnovel-writer
- **校验时间**: 2026-09-23 03:53:55
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-05-19 (856 天前); 公开仓库数: 2; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 229; 包名: dsh-webnovel-writer; 描述: Adds Chinese web-fiction planning, file-backed memory, chapter workflow, consist — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.2; @deepseek-ai/dsh-skill: >=0.1.6-alpha.2 <0.2.0-0; @deepseek-ai/dsh-system-prompt: >=0.1.6-alpha.2 <0.2.0-0; engines: {"node": ">=20", "dsh": ">=0.1.6-alpha.2 <0.2.0-0"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-skill, @deepseek-ai/dsh-system-prompt; Cordis 配置文件: cordis.patch.yml, RrcoVer0__dsh-webnovel-writer.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:33 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - submission/RrcoVer0__dsh-webnovel-writer.yml:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 126, 注释行: 1 (0.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-webnovel-writer |
| 校验 URL | https://github.com/RrcoVer0/dsh-webnovel-writer |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
