...
- **校验对象**: https://github.com/JohnXu22786/worktree-mgr
- **校验时间**: 2026-08-23 16:35:15
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 2286; 包名: worktree-mgr; 描述: dsh 插件：为任务自动创建隔离的 git 工作区，管理 创建/同步/收尾 全生命周期 — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/git.js:24 → 读取环境变量配置 (2 处)<br>  - src/vault.js:69 → 读取环境变量配置 (2 处)<br>  - src/triggers.js:38 → 读取环境变量配置 (1 处)<br>  - src/tools.js:73 → 读取环境变量配置 (4 处)<br>  - src/config.js:35 → 读取环境变量配置 (3 处)<br>  ... 共 7 处 |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/vault.js:69 → 读取用户主目录 (2 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package-lock.json:24 → 探测到 HTTP(S) 网络请求 (23 处)<br>涉及的域名: registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - src/vault.js:69 → 读取环境变量 API Key/凭证 (2 处)<br>  - src/tools.js:73 → 读取环境变量 API Key/凭证 (3 处)<br>  - bin/wtm.js:151 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/vault.js:232 → 定时任务 (2 处)<br>  - tests/vault.test.js:106 → 定时任务 (6 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 0 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 3856, 注释行: 688 (17.8%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - src/vault.js:214 → 明文写入敏感信息 (1 处)<br>  - tests/vault.test.js:149 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - bin/wtm.js:1 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | worktree-mgr |
| 校验 URL | https://github.com/JohnXu22786/worktree-mgr |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
