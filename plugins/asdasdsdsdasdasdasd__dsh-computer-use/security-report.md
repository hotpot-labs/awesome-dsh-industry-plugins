...
- **校验对象**: https://github.com/asdasdsdsdasdasdasd/dsh-computer-use
- **校验时间**: 2026-08-30 04:30:28
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-09-04 (1455 天前); 公开仓库数: 16; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 829; 包名: dsh-computer-use; 描述: DeepSeek Harness plugin: Linux X11 computer-use tools (screenshot, mouse, keyboa — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 6 处, 函数: 46 个, 比例: 13.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - xc.py:144 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - tests/test_xc.py:20 → Python subprocess (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - SECURITY.md:33 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:41 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - index.js:120 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - CHANGELOG.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, img.shields.io, keepachangelog.com, semver.org, www.youtube.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:17 → 敏感凭证读取 (4 处)<br>  - README.md:112 → 敏感凭证读取 (2 处)<br>  - promotion/show-hn.md:41 → 敏感凭证读取 (1 处)<br>  - promotion/reddit-localllama.md:42 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:124 → 临时文件操作 (2 处)<br>  - index.js:387 → 定时任务 (2 处)<br>  - index.js:44 → 临时文件操作 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1100, 注释行: 67 (6.1%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 6.1 | 无需 root 权限 | 必查 | ❌ 不通过 |   - CONTRIBUTING.md:12 → 要求 root/sudo 权限<br>  - README.md:57 → 要求 root/sudo 权限<br>  - .github/workflows/ci.yml:21 → 要求 root/sudo 权限<br>  - scripts/install-deps.sh:4 → 要求 root/sudo 权限 |
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - xc.py:1 → 修改系统目录 (7 处)<br>  - tests/test_xc.py:1 → 修改系统目录 (1 处)<br>  - scripts/install-deps.sh:1 → 修改系统目录 (1 处) |
...
| 校验 URL | https://github.com/asdasdsdsdasdasdasd/dsh-computer-use |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
