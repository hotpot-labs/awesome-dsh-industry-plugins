...
- **校验对象**: https://github.com/ZZJQ678/dsh-global-context
- **校验时间**: 2026-09-20 04:02:26
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-02-13 (218 天前); 公开仓库数: 7; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ⚠️  需人工复核 | README 字数: 27; 包名: dsh-global-context; 描述: 全局提示词配置：在对话视图新增一个标签页，写进去的内容会加在每个会话系统提示词的最前面，保存后下一条消息即生效。 — README 内容较少, 需人工比对 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 57 处, 函数: 151 个, 比例: 37.7% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/index.js:49 → 读取环境变量配置 (3 处)<br>  - scripts/verify-live.mjs:33 → 读取环境变量配置 (2 处)<br>  - test/host.test.mjs:19 → 读取环境变量配置 (3 处)<br>  - test/client.test.mjs:15 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - PUBLISHING.md:3 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package.json:33 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/client.js:115 → fetch 网络请求 (2 处)<br>  - scripts/verify-live.mjs:7 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - scripts/verify-live.mjs:74 → fetch 网络请求 (9 处)<br>涉及的域名: 127.0.0.1, awesome-dsh-plugin.com, evil.example, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:49 → 读取环境变量 API Key/凭证 (3 处)<br>  - lib/client.js:116 → 敏感凭证读取 (1 处)<br>  - scripts/verify-live.mjs:33 → 读取环境变量 API Key/凭证 (2 处)<br>  - test/host.test.mjs:19 → 读取环境变量 API Key/凭证 (3 处)<br>  - test/client.test.mjs:15 → 读取环境变量 API Key/凭证 (3 处)<br>  ... 共 6 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/host.test.mjs:29 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1728, 注释行: 187 (10.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-global-context |
| 校验 URL | https://github.com/ZZJQ678/dsh-global-context |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
