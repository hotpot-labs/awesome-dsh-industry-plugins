...
- **校验对象**: https://github.com/dong3434/dsh-auto-maintenance
- **校验时间**: 2026-08-26 01:27:31
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-05-26 (1917 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 561; 包名: dsh-auto-maintenance; 描述: DSH Auto Maintenance System — startup diagnosis, auto-fix, scheduled backups, sm — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.1-rc.1; engines: {"node": ">=18", "dsh": ">=0.1.0"} |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 29 处, 函数: 53 个, 比例: 54.7% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/monitor.js:20 → 读取环境变量配置 (1 处)<br>  - lib/core.js:21 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/monitor.js:20 → 读取用户主目录 (1 处)<br>  - lib/core.js:21 → 读取用户主目录 (3 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - publish.bat:14 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package.json:36 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - PUBLISH-GUIDE.md:29 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: github.com, img.shields.io, opensource.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:34 → 敏感凭证读取 (2 处)<br>  - test/smoke.js:29 → API Key / Token 读取 (3 处)<br>  - test/smoke.js:29 → 敏感凭证读取 (1 处)<br>  - lib/monitor.js:20 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/monitor.js:15 → 敏感凭证读取 (1 处)<br>  ... 共 8 处 — 需人工判断是否读取的是当前会话上下文 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - test/smoke.js:28 → 写入配置文件 (2 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/core.js:346 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 704, 注释行: 111 (15.8%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - test/smoke.js:29 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - bin/dsh-auto-maintenance.js:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-auto-maintenance |
| 校验 URL | https://github.com/dong3434/dsh-auto-maintenance |
| 必查项通过率 | 20/33 (61%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
