...
- **校验对象**: https://github.com/Jimmyzwang-cloud/dsh-inkscreen-theme
- **校验时间**: 2026-08-31 04:27:17
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-01-05 (3524 天前); 公开仓库数: 10; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 297; 包名: dsh-inkscreen-theme; 描述: DSH web plugin: an ink-and-paper Apple-glass theme with editorial typography, co — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 1 处, 函数: 28 个, 比例: 3.6% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: fonts.googleapis.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - lib/client.js:112 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 290, 注释行: 5 (1.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-inkscreen-theme |
| 校验 URL | https://github.com/Jimmyzwang-cloud/dsh-inkscreen-theme |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
