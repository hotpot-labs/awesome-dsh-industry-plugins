...
- **校验对象**: https://github.com/CTWCTW9999/dsh-agent-preset-router
- **校验时间**: 2026-08-23 14:29:52
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-01-18 (217 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1129; 包名: dsh-agent-preset-router; 描述: 自动模式 Auto Mode: an AI pre-judgment layer for DeepSeek Harness — a flash-model ro — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 19 处, 函数: 111 个, 比例: 17.1% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - test/host.test.mjs:33 → 读取环境变量配置 (2 处)<br>  - test/client.test.mjs:13 → 读取环境变量配置 (1 处)<br>  - test/rpc.test.mjs:7 → 读取环境变量配置 (1 处)<br>  - lib/index.js:45 → 读取环境变量配置 (1 处) |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:20 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (10 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (10 处)<br>  - test/rpc.test.mjs:16 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/client.js:53 → fetch 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, evil.example, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - test/host.test.mjs:33 → 读取环境变量 API Key/凭证 (2 处)<br>  - test/client.test.mjs:13 → 读取环境变量 API Key/凭证 (1 处)<br>  - test/rpc.test.mjs:7 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/index.js:45 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - lib/index.js:116 → 写入配置文件 (1 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - test/client.test.mjs:51 → node:vm 任意代码执行 (1 处) |
| 4.3 | npm audit 无高危漏洞 | 必查 | ⚠️  需人工复核 | npm 不可用, 无法执行 npm audit. 建议在具备 npm 的环境中运行 `npm audit` |
| 4.4 | 不使用废弃依赖 | 必查 | ✅ 通过 | 共检查 0 个依赖, 未发现已知废弃包 |
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/host.test.mjs:66 → 进程控制 (1 处)<br>  - test/client.test.mjs:212 → 进程控制 (1 处)<br>  - test/rpc.test.mjs:162 → 进程控制 (1 处)<br>  - lib/client.js:99 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1141, 注释行: 113 (9.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-agent-preset-router |
| 校验 URL | https://github.com/CTWCTW9999/dsh-agent-preset-router |
| 必查项通过率 | 20/33 (61%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
