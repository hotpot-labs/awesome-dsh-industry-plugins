...
- **校验对象**: https://github.com/173787247/dsh-wsl-media
- **校验时间**: 2026-09-24 03:45:37
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 160; 包名: dsh-wsl-media; 描述: DeepSeek Harness WSL plugin: ffprobe, ffmpeg extract/thumbnail, PDF, whisper, pa — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 17 处, 函数: 68 个, 比例: 25.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:4 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/media.js:41 → 读取用户主目录 (1 处)<br>  - test/media.test.js:9 → 读取用户主目录 (4 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ❌ 不通过 | 检测到命令注入风险:<br>  - lib/media.js:8 → 命令字符串拼接变量 (注入风险) (1 处) |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.zh.md:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:10 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.en.md:10 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/awesome-entry.yml:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - test/media.test.js:14 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:40 → 临时文件操作 (1 处)<br>  - README.en.md:38 → 临时文件操作 (1 处)<br>  - lib/media.js:50 → 定时任务 (1 处)<br>  - lib/media.js:42 → 临时文件操作 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 496, 注释行: 0 (0.0%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - test/media.test.js:14 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | dsh-wsl-media |
| 校验 URL | https://github.com/173787247/dsh-wsl-media |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
