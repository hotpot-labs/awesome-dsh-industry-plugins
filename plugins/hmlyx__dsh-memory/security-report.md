...
- **校验对象**: https://github.com/hmlyx/dsh-memory
- **校验时间**: 2026-09-01 04:12:24
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 190; 包名: dsh-memory; 描述: 对话记忆：记忆标签页、自动命名、隐私隔离、侧栏AI名、经验自动记录。静态 profile 插件，任何预设可用，重启不消失。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 31 处, 函数: 123 个, 比例: 25.2% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - index.mjs:131 → 目录遍历 (2 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - index.mjs:536 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - client.js:54 → fetch 网络请求 (3 处)<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.mjs:121 → 文件写入 (2 处)<br>  - index.mjs:116 → 文件读取 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1013, 注释行: 89 (8.8%) |
...
| :--- | :--- |
| 插件名称 | dsh-memory |
| 校验 URL | https://github.com/hmlyx/dsh-memory |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
