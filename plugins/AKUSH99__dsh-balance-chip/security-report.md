...
- **校验对象**: https://github.com/AKUSH99/dsh-balance-chip
- **校验时间**: 2026-09-07 03:33:48
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-05-30 (1560 天前); 公开仓库数: 10; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 474; 包名: dsh-balance-chip; 描述: DeepSeek API balance in the DSH sidebar footer – live status dot plus amount, 60 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (1): v1.0.1; package.json version: 1.0.2; GitHub Releases (1): v1.0.1 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 6 处, 函数: 19 个, 比例: 31.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:31 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - client/client.js:23 → fetch 网络请求 (2 处)<br>  - lib/index.js:22 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:22 → fetch 网络请求 (1 处)<br>涉及的域名: api.deepseek.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - CHANGELOG.md:9 → 敏感凭证读取 (1 处)<br>  - README.md:16 → API Key / Token 读取 (3 处)<br>  - README.md:12 → 敏感凭证读取 (5 处)<br>  - lib/index.js:16 → API Key / Token 读取 (1 处)<br>  - lib/index.js:3 → 敏感凭证读取 (6 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client/client.js:32 → 定时任务 (2 处)<br>  - lib/index.js:60 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 226, 注释行: 6 (2.7%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 11 天; 提交总数: 11 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-balance-chip |
| 校验 URL | https://github.com/AKUSH99/dsh-balance-chip |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
