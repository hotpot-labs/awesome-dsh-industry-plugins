...
- **校验对象**: https://github.com/better-er/dsh-write-create-only
- **校验时间**: 2026-09-05 03:36:13
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 175; 包名: dsh-write-create-only; 描述: 禁止 write 覆盖已存在的文件：write 只允许创建新文件，目标已存在时在工具执行前直接拒绝并提示改用 edit，全局所有会话生效，防止模型用 write — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:16 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:17 → 探测到 HTTP(S) 网络请求 (27 处)<br>  - .github/workflows/release.yml:70 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, repo.huaweicloud.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - .github/workflows/release.yml:38 → 敏感凭证读取 (1 处)<br>  - .github/workflows/ci.yml:11 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - .github/workflows/release.yml:55 → 文件写入 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 193, 注释行: 46 (23.8%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - .github/workflows/release.yml:58 → 读取 git 历史 (1 处) — 需人工判断是否在授权范围内 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-write-create-only |
| 校验 URL | https://github.com/better-er/dsh-write-create-only |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
