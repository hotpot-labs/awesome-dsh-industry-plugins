...
- **校验对象**: https://github.com/173787247/dsh-wsl-tmux
- **校验时间**: 2026-09-24 03:46:06
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 126; 包名: dsh-wsl-tmux; 描述: DeepSeek Harness WSL plugin: tmux list + capture-pane (read-only). — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 4 处, 函数: 35 个, 比例: 11.4% |
...
| 3.4 | 无命令注入风险 | 必查 | ❌ 不通过 | 检测到命令注入风险:<br>  - lib/tmux.js:7 → 命令字符串拼接变量 (注入风险) (1 处) |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/tmux.js:19 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
...
| :--- | :--- |
| 插件名称 | dsh-wsl-tmux |
| 校验 URL | https://github.com/173787247/dsh-wsl-tmux |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
