...
- **校验对象**: https://github.com/CN-Leo/dsh-deepseek-balance
- **校验时间**: 2026-08-23 14:29:48
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2016-07-17 (3689 天前); 公开仓库数: 8; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 197; 包名: dsh-deepseek-balance; 描述: Real-time DeepSeek account balance display in the DSH web UI, via the official / — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-ui-conver; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 18 处, 函数: 30 个, 比例: 60.0% |
...
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - lib/index.js:96 → 进程执行 (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:48 → fetch 网络请求 (1 处)<br>  - client/client.js:84 → fetch 网络请求 (1 处)<br>  - lib/index.js:12 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: api.deepseek.com, github.com, localhost<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - package.json:3 → API Key / Token 读取 (1 处)<br>  - package.json:3 → 敏感凭证读取 (1 处)<br>  - README.md:21 → API Key / Token 读取 (4 处)<br>  - README.md:21 → 敏感凭证读取 (4 处)<br>  - lib/index.js:4 → API Key / Token 读取 (3 处)<br>  ... 共 6 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client/client.js:120 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 377, 注释行: 39 (10.3%) |
...
| 校验 URL | https://github.com/CN-Leo/dsh-deepseek-balance |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
