...
- **校验对象**: https://github.com/soarGuo/dsh-skin-lab
- **校验时间**: 2026-08-30 04:36:15
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-07-10 (2973 天前); 公开仓库数: 12; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 225; 包名: dsh-skin-lab; 描述: Skin Lab for the DSH Web GUI: official token browser, live try-on (theme overrid — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v1.0.2, v1.0.0; package.json version: 1.0.2 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 5 处, 函数: 182 个, 比例: 2.7% — try/catch 覆盖比例较低 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - tsdown.config.ts:61 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:40 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:20 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - src/client/presets.ts:46 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:27 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: cdn.jsdelivr.net, github.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - pnpm-lock.yaml:203 → 敏感凭证读取 (5 处)<br>  - tsdown.config.ts:61 → 读取环境变量 API Key/凭证 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client/skin-studio.tsx:156 → 定时任务 (1 处)<br>  - docs/screenshots/skin1.png:762 → 临时文件操作 (1 处)<br>  - scripts/extract-tokens.mjs:224 → 进程控制 (1 处)<br>  - lib/client.js:811 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - package.json:4 → 云存储上传 (1 处)<br>  - CHANGELOG.md:18 → 云存储上传 (1 处)<br>  - src/index.ts:35 → 云存储上传 (1 处)<br>  - src/client/studio.css.ts:371 → 云存储上传 (1 处)<br>  - src/client/studio-controller.ts:30 → 云存储上传 (2 处)<br>  ... 共 7 处 — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ⚠️  需人工复核 | 总依赖数: 17. repo: 1 deps + 16 devDeps — 依赖较多, 需人工审查 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 7962, 注释行: 402 (5.0%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 4 天; 提交总数: 13 |
...
| :--- | :--- |
| 插件名称 | dsh-skin-lab |
| 校验 URL | https://github.com/soarGuo/dsh-skin-lab |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
