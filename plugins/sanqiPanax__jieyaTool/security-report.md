...
- **校验对象**: https://github.com/sanqiPanax/jieyaTool
- **校验时间**: 2026-09-08 03:44:54
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-02-13 (1667 天前); 公开仓库数: 12; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 381; 包名: dsh-auto-extract; 描述: DSH 自动解压工具:魔数识别真格式(不信伪装后缀)、递归解压嵌套压缩包、多密码池自动尝试、SFX/分卷配对、私有格式归类报告。核心基于 7-Zip。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 33 处, 函数: 47 个, 比例: 70.2% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/archive-core.js:18 → 读取环境变量配置 (2 处) |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:44 → 敏感凭证读取 (3 处)<br>  - lib/archive-core.js:18 → 读取环境变量 API Key/凭证 (2 处)<br>  - lib/archive-core.js:5 → 敏感凭证读取 (26 处)<br>  - lib/index.js:14 → 敏感凭证读取 (7 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 586, 注释行: 61 (10.4%) |
...
| :--- | :--- |
| 插件名称 | jieyaTool |
| 校验 URL | https://github.com/sanqiPanax/jieyaTool |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
