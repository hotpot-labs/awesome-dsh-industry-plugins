...
- **校验对象**: https://github.com/ZI-LV68/dsh-deepseek-model-router
- **校验时间**: 2026-09-09 03:44:49
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-07-15 (785 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 348; 包名: dsh-deepseek-model-router; 描述: Auto-switch DeepSeek models for DeepSeek Harness: routes every model request to  — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ❌ 不通过 | 检测到潜在违规模式:<br>  - docs/GITHUB_ONLY.md:39 → 盗版/侵权 (1 处) |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/cordis; Cordis 配置文件: cordis.patch.yml<br>  - README.md:36 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:29 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/plugins-entry.yml:4 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/PUBLISH.md:4 → 探测到 HTTP(S) 网络请求 (6 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 581, 注释行: 98 (16.9%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 15 天; 提交总数: 15 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-deepseek-model-router |
| 校验 URL | https://github.com/ZI-LV68/dsh-deepseek-model-router |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
