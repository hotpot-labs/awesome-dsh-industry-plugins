...
- **校验对象**: https://github.com/huang-chunc/dsh-user-message-timeline
- **校验时间**: 2026-08-27 08:15:56
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-11-20 (1010 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 329; 包名: dsh-user-message-timeline; 描述: DSH User Message Timeline - 对话内悬浮药丸导轨，支持预览、跳转、拖拽与分页 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (11): v0.2.0, v0.1.10, v0.1.9, v0.1.8, v0.1.7; package.json version: 0.2.0; GitHub Releases (5): v0.2.0, v0.1.10, v0.1.9, v0.1.8, v0.1.7 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 185 处, 函数: 178 个, 比例: 103.9% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/client.js:25 → 读取浏览器 Cookie/本地存储 (13 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README_EN.md:6 → 探测到 HTTP(S) 网络请求 (18 处)<br>  - package.json:40 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:6 → 探测到 HTTP(S) 网络请求 (18 处)<br>  - awesome-entry.json:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, www.bilibili.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:157 → 定时任务 (22 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1227, 注释行: 88 (7.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-user-message-timeline |
| 校验 URL | https://github.com/huang-chunc/dsh-user-message-timeline |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
