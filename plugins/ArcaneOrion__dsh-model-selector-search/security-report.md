...
- **校验对象**: https://github.com/ArcaneOrion/dsh-model-selector-search
- **校验时间**: 2026-09-16 03:55:28
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 619; 包名: @arcaneorion/dsh-model-selector-search; 描述: DSH session model selector with search and reasoning-effort pane: shadows the na — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.2; @deepseek-ai/dsh-client-connection: 0.1.1-rc.2; @deepseek-ai/dsh-client-ui-conversation: 0.1.1-rc.2; @deepseek-ai/dsh-client-ui-model-selection: 0.1.1-rc.2 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["slots", "connection"], "platform"; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-client-connection, @deepseek-ai/dsh-client-ui-conversation, @deepseek-ai/dsh-client-ui-model-selection; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 10 处, 函数: 176 个, 比例: 5.7% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:16 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ✅ 通过 | 未检测到敏感配置读取模式 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - src/client.js:196 → 修改设置 (1 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:85 → 临时文件操作 (1 处)<br>  - tests/search-select.test.cjs:357 → 进程控制 (1 处)<br>  - tests/slot-priority.test.cjs:57 → 文件读取 (1 处)<br>  - tests/slot-priority.test.cjs:64 → 进程控制 (1 处)<br>  - tests/effort-memory.test.cjs:9 → 文件读取 (1 处)<br>  ... 共 7 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1137, 注释行: 156 (13.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-model-selector-search |
| 校验 URL | https://github.com/ArcaneOrion/dsh-model-selector-search |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
