...
- **校验对象**: https://github.com/Elohia/dsh-genome
- **校验时间**: 2026-08-23 16:23:51
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 371; 包名: dsh-genome; 描述: DSH 能力转化与组装引擎（钱学森工程控制论/系统论框架）：静态常驻层（三库管理 registry/metrics/进化树 + Web UI 基因组面板）+ 动 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 21 处, 函数: 110 个, 比例: 19.1% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - test/stress-verify.mjs:3 → 读取环境变量配置 (1 处)<br>  - lib/index.js:45 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - test/host-verify.mjs:7 → 读取用户主目录 (1 处)<br>  - test/minimal-verify.mjs:7 → 读取用户主目录 (1 处)<br>  - test/e2e-verify.mjs:7 → 读取用户主目录 (1 处)<br>  - lib/index.js:30 → 读取用户主目录 (2 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:23 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - test/stress-verify.mjs:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - test/stress-verify.mjs:6 → fetch 网络请求 (2 处)<br>  - test/e2e-verify.mjs:18 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/client.js:57 → fetch 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, api.github.com, github.com, localhost<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - test/stress-verify.mjs:3 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/index.js:45 → 读取环境变量 API Key/凭证 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/host-verify.mjs:76 → 进程控制 (1 处)<br>  - test/minimal-verify.mjs:58 → 进程控制 (1 处)<br>  - test/stress-verify.mjs:84 → 进程控制 (1 处)<br>  - test/e2e-verify.mjs:150 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1140, 注释行: 117 (10.3%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 8 天; 提交总数: 1 |
...
| :--- | :--- |
| 插件名称 | dsh-genome |
| 校验 URL | https://github.com/Elohia/dsh-genome |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
