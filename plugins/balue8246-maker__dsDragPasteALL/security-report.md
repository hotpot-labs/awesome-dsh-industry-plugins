...
- **校验对象**: https://github.com/balue8246-maker/dsDragPasteALL
- **校验时间**: 2026-09-06 03:35:04
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-03-03 (186 天前); 公开仓库数: 6; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 262; 包名: dsdragpasteall; 描述: dsDragPasteALL：拖拽任意文件/粘贴非图片文件到 DSH 页面（纯图片粘贴交给原生成图），后台静默反查文件真实路径（Spotlight + find — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 28 处, 函数: 64 个, 比例: 43.8% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:23 → 读取用户主目录 (4 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - registry.yml:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:264 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/client.js:53 → fetch 网络请求 (1 处)<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - install.sh:23 → 文件写入 (1 处)<br>  - install.sh:18 → 文件读取 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 688, 注释行: 64 (9.3%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - install.sh:1 → 修改系统目录 (1 处)<br>  - scripts/ci.sh:1 → 修改系统目录 (1 处)<br>  - scripts/smoke-test.sh:1 → 修改系统目录 (1 处) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 15 天; 提交总数: 11 |
...
| :--- | :--- |
| 插件名称 | dsDragPasteALL |
| 校验 URL | https://github.com/balue8246-maker/dsDragPasteALL |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
