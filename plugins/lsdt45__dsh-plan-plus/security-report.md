...
- **校验对象**: https://github.com/lsdt45/dsh-plan-plus
- **校验时间**: 2026-09-13 03:55:04
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-08-27 (2938 天前); 公开仓库数: 32; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 442; 包名: dsh-plan-plus; 描述: DeepSeek Harness 计划评审增强插件：评审弹窗内直接输入意见继续对话修改计划；输入行提供「查看计划」按钮与浮层，浮层支持直接编辑计划内容并留档。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 12 处, 函数: 111 个, 比例: 10.8% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - client.js:711 → fetch 网络请求 (1 处)<br>  - package.json:33 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:51 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - index.mjs:70 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, github.com, planx.local<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1271, 注释行: 26 (2.0%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - client.js:125 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | dsh-plan-plus |
| 校验 URL | https://github.com/lsdt45/dsh-plan-plus |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
