...
- **校验对象**: https://github.com/WsTe47/dsh-step-clock
- **校验时间**: 2026-09-18 03:50:25
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-11-01 (2146 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 770; 包名: @climber47/dsh-step-clock; 描述: Live per-step elapsed-time clock for the DeepSeek Harness web GUI: shows which s — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"engines": {"dsh": ">=0.1.5-rc.1"}, "bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject":; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 119 个, 比例: 0.0% — 未检测到任何 try/catch |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: tests/behaviour.mjs; 发现清理逻辑: lib/client.js; 发现清理逻辑: src/client/index.js |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:60 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - contrib/awesome-dsh-plugin-entry.yml:16 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - docs/review-0.1.0.md:426 → 代理/隧道 (1 处) |
...
| 4.2 | 无 eval/vm/new Function | 必查 | ❌ 不通过 | 检测到危险 API:<br>  - tests/behaviour.mjs:27 → new Function() 任意代码执行 (1 处) |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1362, 注释行: 370 (27.2%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - tests/behaviour.mjs:1 → 修改系统目录 (1 处)<br>  - scripts/build-client.mjs:1 → 修改系统目录 (1 处) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 5 天; 提交总数: 7 |
...
| :--- | :--- |
| 插件名称 | dsh-step-clock |
| 校验 URL | https://github.com/WsTe47/dsh-step-clock |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
