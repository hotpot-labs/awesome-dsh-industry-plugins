...
- **校验对象**: https://github.com/balue8246-maker/dseyesopen
- **校验时间**: 2026-09-06 03:35:06
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 186; 包名: dseyesopen; 描述: dseyesopen：让无识图能力的 DeepSeek 仿佛睁开了眼睛。发图自动交免费视觉小模型识别，后台注入文字描述，对话框无感。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 23 处, 函数: 37 个, 比例: 62.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:22 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - registry.yml:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:24 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: open.bigmodel.cn<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:26 → API Key / Token 读取 (1 处)<br>  - lib/index.js:23 → API Key / Token 读取 (2 处)<br>  - lib/index.js:260 → 敏感凭证读取 (8 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:371 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 399, 注释行: 40 (10.0%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 15 天; 提交总数: 7 |
...
| :--- | :--- |
| 插件名称 | dseyesopen |
| 校验 URL | https://github.com/balue8246-maker/dseyesopen |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
