...
- **校验对象**: https://github.com/AcidGr/dsh-web-mobile-fix
- **校验时间**: 2026-08-23 14:23:18
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 350; 包名: dsh-web-mobile-fix; 描述: Mobile layout fixes for the DeepSeek Harness Web UI: settings panel, popups, sid — README 包含功能/使用说明 |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - README.md:52 → 深层目录穿越 (1 处)<br>  - README.zh.md:52 → 深层目录穿越 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:27 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:2 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.zh.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 212, 注释行: 8 (3.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-web-mobile-fix |
| 校验 URL | https://github.com/AcidGr/dsh-web-mobile-fix |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
