...
- **校验对象**: https://github.com/qwert702/dsh-auto-translate
- **校验时间**: 2026-09-06 03:36:23
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-03-21 (169 天前); 公开仓库数: 22; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 225; 包名: dsh-auto-translate; 描述: Auto-translate for the DeepSeek Harness Web GUI: when the assistant's reply is i — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "dsh-auto-translate.yml"}, "client": {"platform": "web", "inject": ["@deepseek-; Cordis 配置文件: dsh-auto-translate.yml<br>  - dsh-auto-translate.yml:1 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 13 处, 函数: 95 个, 比例: 13.7% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - test/smoke.cjs:23 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:75 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:40 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:34 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/index.js:112 → fetch 网络请求 (1 处)<br>  - lib/client.js:168 → fetch 网络请求 (1 处)<br>涉及的域名: api.deepseek.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.en.md:74 → API Key / Token 读取 (2 处)<br>  - README.en.md:74 → 敏感凭证读取 (3 处)<br>  - README.en.md:10 → 会话历史读取 (1 处)<br>  - README.md:39 → API Key / Token 读取 (2 处)<br>  - lib/index.js:33 → API Key / Token 读取 (14 处)<br>  ... 共 9 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:219 → 定时任务 (1 处)<br>  - test/smoke.cjs:224 → 文件读取 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1182, 注释行: 215 (18.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-auto-translate |
| 校验 URL | https://github.com/qwert702/dsh-auto-translate |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
