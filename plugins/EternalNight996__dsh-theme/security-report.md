...
- **校验对象**: https://github.com/EternalNight996/dsh-theme
- **校验时间**: 2026-08-27 08:11:07
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-09-26 (2160 天前); 公开仓库数: 20; 粉丝数: 4 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 363; 包名: @eternalnight/dsh-theme; 描述: DeepSeek Harness 主题皮肤插件：给 DSH Web GUI 换背景（内置主题 / 静态图片 / 动态视频环绕跟随帧），设置页与侧边栏底部一键切换 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (10): v0.1.9, v0.1.8, v0.1.7, v0.1.6, v0.1.5; package.json version: 0.1.9 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 14 处, 函数: 132 个, 比例: 10.6% |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/client/index.tsx:15 → 深层目录穿越 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - PUBLISH.md:3 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - README.en.md:2 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:27 → 探测到 HTTP(S) 网络请求 (33 处)<br>  - README.md:2 → 探测到 HTTP(S) 网络请求 (7 处)<br>涉及的域名: ...git, gitee.com, github.com, img.shields.io, localhost, raw.githubusercontent.com, registry.npmjs.org, registry.npmmirror.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.js:170 → 文件写入 (1 处)<br>  - index.js:114 → 文件读取 (1 处)<br>  - src/client/index.tsx:377 → 定时任务 (1 处)<br>  - lib/client.js:64 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 3. repo: 1 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 469, 注释行: 65 (13.9%) |
...
| :--- | :--- |
| 插件名称 | dsh-theme |
| 校验 URL | https://github.com/EternalNight996/dsh-theme |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
