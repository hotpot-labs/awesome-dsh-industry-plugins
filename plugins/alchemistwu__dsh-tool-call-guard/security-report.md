...
- **校验对象**: https://github.com/alchemistwu/dsh-tool-call-guard
- **校验时间**: 2026-09-02 03:36:59
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-07-12 (3339 天前); 公开仓库数: 33; 粉丝数: 12 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 541; 包名: dsh-tool-call-guard; 描述: Neutralize tool calls with invalid JSON arguments on the wire, so one malformed  — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:18 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - README.zh.md:7 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - .github/workflows/publish.yml:21 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 386, 注释行: 78 (20.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-tool-call-guard |
| 校验 URL | https://github.com/alchemistwu/dsh-tool-call-guard |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
