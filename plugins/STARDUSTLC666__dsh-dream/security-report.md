...
- **校验对象**: https://github.com/STARDUSTLC666/dsh-dream
- **校验时间**: 2026-09-02 03:35:59
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-06-08 (85 天前); 公开仓库数: 26; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 309; 包名: @stardustlc/dsh-dream; 描述: DSH 做梦插件：agent 在会话间隙回放近期对话（dream_digest）、反思并写入梦境日记（dream_save/journal/recall），高频 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 25 处, 函数: 226 个, 比例: 11.1% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/index.ts; 发现清理逻辑: test/register.test.mjs; 发现清理逻辑: lib/index.js |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/config.ts:37 → 读取用户主目录 (1 处)<br>  - lib/config.js:19 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - package.json:55 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.en.md:43 → API Key / Token 读取 (1 处)<br>  - README.en.md:39 → 敏感凭证读取 (4 处)<br>  - README.md:79 → API Key / Token 读取 (1 处)<br>  - README.md:74 → 敏感凭证读取 (3 处)<br>  - src/mask.ts:19 → API Key / Token 读取 (1 处)<br>  ... 共 18 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2259, 注释行: 199 (8.8%) |
...
| :--- | :--- |
| 插件名称 | dsh-dream |
| 校验 URL | https://github.com/STARDUSTLC666/dsh-dream |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
