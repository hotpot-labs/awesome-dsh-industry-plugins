...
- **校验对象**: https://github.com/kobenfang/BigTimer
- **校验时间**: 2026-09-05 03:38:23
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2016-06-29 (3719 天前); 公开仓库数: 13; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 56; 包名: @kobenfang/dsh-bigtimer; 描述: Agent Skill for DeepSeek Harness (dsh): bigtimer. — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - scripts/bigtimer.py:21 → 读取环境变量配置 (9 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - scripts/bigtimer.py:103 → Python subprocess (5 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - SKILL.md:56 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.zh.md:7 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: clawhub.ai, github.com, open.feishu.cn<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - _selftest.mjs:16 → 文件读取 (1 处)<br>  - _selftest.mjs:6 → 进程控制 (8 处)<br>  - lib/index.js:13 → 文件读取 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 443, 注释行: 21 (4.7%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - _selftest.mjs:16 → 读取文档/压缩文件 (1 处) — 需人工判断是否在授权范围内 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - scripts/bigtimer.py:1 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | BigTimer |
| 校验 URL | https://github.com/kobenfang/BigTimer |
| 必查项通过率 | 21/33 (64%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
