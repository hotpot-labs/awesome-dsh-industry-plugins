...
- **校验对象**: https://github.com/RyanShen3/dsh-toutiao-reader
- **校验时间**: 2026-09-08 03:40:37
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-04-05 (2712 天前); 公开仓库数: 2; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 716; 包名: dsh-toutiao-reader; 描述: 浏览今日头条文章的实战经验打包成 DSH 插件：webfetch 工具（三级策略：直连 → Edge 常驻无头浏览器 CDP → dump-dom 兜底，自动识 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 22 处, 函数: 49 个, 比例: 44.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:20 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - PUBLISH.md:7 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - README.md:99 → WebSocket 连接 (1 处)<br>  - README.md:10 → fetch 网络请求 (1 处)<br>  - README.md:99 → 浏览器网络 API (1 处)<br>涉及的域名: 127.0.0.1, awesome-dsh-plugin.com, github.com, m.toutiao.com, www.toutiao.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:92 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:137 → 文件写入 (1 处)<br>  - lib/fetch.js:43 → 定时任务 (1 处)<br>  - lib/fetch.js:100 → 文件写入 (3 处)<br>  - lib/fetch.js:99 → 文件读取 (3 处)<br>  - lib/fetch.js:91 → 临时文件操作 (5 处)<br>  ... 共 7 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - PUBLISH.md:27 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 534, 注释行: 75 (14.0%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - .github/workflows/ci.yml:21 → 读取文档/压缩文件 (1 处) — 需人工判断是否在授权范围内 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-toutiao-reader |
| 校验 URL | https://github.com/RyanShen3/dsh-toutiao-reader |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
