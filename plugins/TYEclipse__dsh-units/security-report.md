...
- **校验对象**: https://github.com/TYEclipse/dsh-units
- **校验时间**: 2026-09-12 03:46:55
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2015-07-02 (4089 天前); 公开仓库数: 44; 粉丝数: 5 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1219; 包名: dsh-units; 描述: Unit conversion toolbox for DeepSeek Harness (dsh): length, mass, temperature, a — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 4 处, 函数: 115 个, 比例: 3.5% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:44 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1210, 注释行: 133 (11.0%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - .githooks/pre-push:19 → 读取 git 历史 (1 处)<br>  - .githooks/pre-commit:17 → 读取 git 历史 (3 处) — 需人工判断是否在授权范围内 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - .githooks/pre-commit:1 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | dsh-units |
| 校验 URL | https://github.com/TYEclipse/dsh-units |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
