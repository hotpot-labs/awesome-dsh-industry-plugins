...
- **校验对象**: https://github.com/173787247/dsh-wsl-ssh-agent
- **校验时间**: 2026-09-02 03:32:24
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 56; 包名: dsh-wsl-ssh-agent; 描述: Hint how to forward Windows OpenSSH agent into WSL. — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/ssh-agent.js:22 → 读取环境变量配置 (1 处)<br>  - lib/wsl-host.js:20 → 读取环境变量配置 (2 处) |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:3 → 敏感凭证读取 (1 处)<br>  - lib/ssh-agent.js:22 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 142, 注释行: 0 (0.0%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-wsl-ssh-agent |
| 校验 URL | https://github.com/173787247/dsh-wsl-ssh-agent |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
