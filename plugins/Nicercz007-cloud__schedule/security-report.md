...
- **校验对象**: https://github.com/Nicercz007-cloud/schedule
- **校验时间**: 2026-09-07 03:35:19
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-07-15 (53 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 438; 包名: schedule; 描述: DSH Desktop 桌面小组件：每日时间安排（周期重复 + 起止时间 + 倒计时便签）、目标设置（进度百分比 + 自由流程图工作台）、日期规划（跨天任务月历 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (3): v0.16.7, v0.16.6, v0.16.5; package.json version: 0.16.7; GitHub Releases (3): v0.16.7, v0.16.6, v0.16.5 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"engines": {"dsh": ">=0.1.2-alpha.3"}, "bundle": {"id": "schedule", "name": "Schedule Widgets", "pa; Cordis 配置文件: cordis.patch.yml<br>  - lib/client.js:2796 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 79 处, 函数: 383 个, 比例: 20.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:6 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:11 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - lib/client.js:201 → 探测到 HTTP(S) 网络请求 (9 处)<br>涉及的域名: github.com, img.shields.io, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:73 → 临时文件操作 (3 处)<br>  - lib/client.js:1209 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 3219, 注释行: 487 (15.1%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:1280 → 明文写入敏感信息 (4 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | schedule |
| 校验 URL | https://github.com/Nicercz007-cloud/schedule |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
