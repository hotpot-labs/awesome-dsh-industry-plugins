...
- **校验对象**: https://github.com/WODE25500/dsh-token-skills
- **校验时间**: 2026-09-02 03:36:38
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 475; 包名: dsh-token-skills; 描述: Cut token cost in 4 layers without cutting model intelligence: handoff, rtk, hea — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 2 处, 函数: 5 个, 比例: 40.0% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:28 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - README.zh.md:28 → 探测到 HTTP(S) 网络请求 (9 处)<br>  - skills/headroom/SKILL.md:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - skills/caveman/SKILL.md:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - skills/handoff/SKILL.md:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, mintlify.wiki, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/check-skills.mjs:57 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 71, 注释行: 16 (22.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-token-skills |
| 校验 URL | https://github.com/WODE25500/dsh-token-skills |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
