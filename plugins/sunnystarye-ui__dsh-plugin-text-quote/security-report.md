...
- **校验对象**: https://github.com/sunnystarye-ui/dsh-plugin-text-quote
- **校验时间**: 2026-09-10 03:41:39
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-04-03 (160 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 131; 包名: dsh-plugin-text-quote; 描述: Codex-style text annotation for DeepSeek Harness conversations: select text, add — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 4 处, 函数: 65 个, 比例: 6.2% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:25 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client.js:326 → 定时任务 (1 处)<br>  - scripts/check.mjs:13 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 380, 注释行: 2 (0.5%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - client.js:19 → 硬编码敏感信息 (1 处) — 需确认是否加密存储 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 16 天; 提交总数: 10 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-text-quote |
| 校验 URL | https://github.com/sunnystarye-ui/dsh-plugin-text-quote |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🟡 灰名单 (需人工复核) |
| 主要风险说明 | 详见各检查项结果 |
