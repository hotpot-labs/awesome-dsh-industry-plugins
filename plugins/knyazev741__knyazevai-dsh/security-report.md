...
- **校验对象**: https://github.com/knyazev741/knyazevai-dsh
- **校验时间**: 2026-09-21 04:01:17
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-09-14 (2197 天前); 公开仓库数: 30; 粉丝数: 4 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 494; 包名: @knyazevai/dsh-provider; 描述: DeepSeek Harness provider bundle for the KnyazevAI API — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (5): v0.1.4, v0.1.3, v0.1.2, v0.1.1, dsh-provider-v0.1.0; package.json version: 0.1.0; GitHub Releases (1): dsh-provider-v0.1.0 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | engines: {"node": "^22.19.0 \|\| >=24.0.0", "dsh": ">=0.1.5-rc.2"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 1 处, 函数: 46 个, 比例: 2.2% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: test/provider.test.js |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - test/provider.test.js:23 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - cordis.patch.yml:12 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/provider.js:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - test/provider.test.js:98 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, knyazevai.work<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:40 → API Key / Token 读取 (1 处)<br>  - README.md:40 → 敏感凭证读取 (1 处)<br>  - AGENTS.md:15 → API Key / Token 读取 (1 处)<br>  - AGENTS.md:15 → 敏感凭证读取 (1 处)<br>  - cordis.patch.yml:10 → API Key / Token 读取 (2 处)<br>  ... 共 9 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 500, 注释行: 16 (3.2%) |
...
| :--- | :--- |
| 插件名称 | knyazevai-dsh |
| 校验 URL | https://github.com/knyazev741/knyazevai-dsh |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
