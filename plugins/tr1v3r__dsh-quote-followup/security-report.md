...
- **校验对象**: https://github.com/tr1v3r/dsh-quote-followup
- **校验时间**: 2026-09-10 03:41:51
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-04-28 (3421 天前); 公开仓库数: 44; 粉丝数: 37 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 426; 包名: dsh-quote-followup; 描述: Quote selected Web conversation text into the DeepSeek Harness composer for targ — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (4): v0.2.6, v0.2.5, v0.2.4, v0.2.3; package.json version: 0.2.6; GitHub Releases (3): v0.2.5, v0.2.4, v0.2.3 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 13 处, 函数: 96 个, 比例: 13.5% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: lib/client.js; 发现清理逻辑: test/web-repeat-harness.mjs |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:37 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - CLAUDE.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - AGENTS.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.zh.md:5 → 探测到 HTTP(S) 网络请求 (6 处)<br>涉及的域名: dsh.market, github.com, img.shields.io, raw.githubusercontent.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - .github/workflows/ci.yml:56 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 952, 注释行: 103 (10.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-quote-followup |
| 校验 URL | https://github.com/tr1v3r/dsh-quote-followup |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
