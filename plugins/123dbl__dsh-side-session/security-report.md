...
- **校验对象**: https://github.com/123dbl/dsh-side-session
- **校验时间**: 2026-09-05 03:32:26
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-09-07 (3285 天前); 公开仓库数: 58; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 390; 包名: dsh-side-session; 描述: Side-chat (旁会话) plugin for DeepSeek Harness: launch a fresh, continuable paralle — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 2 处, 函数: 32 个, 比例: 6.2% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:35 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:45 → 定时任务 (1 处)<br>  - src/client/index.ts:58 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 5. repo: 0 deps + 5 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 419, 注释行: 132 (31.5%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-side-session |
| 校验 URL | https://github.com/123dbl/dsh-side-session |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
