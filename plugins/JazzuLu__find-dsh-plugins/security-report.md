...
- **校验对象**: https://github.com/JazzuLu/find-dsh-plugins
- **校验时间**: 2026-08-23 16:33:05
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-04-24 (3408 天前); 公开仓库数: 53; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: BSD-3-Clause (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 639; 包名: find-dsh-plugins; 描述: 对话式查找 DSH 插件的增强版 skill：语义检索 × 四源聚合 × 安全审计 — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:3 → 探测到 HTTP(S) 网络请求 (13 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (13 处)<br>  - tests/fetch-sources.test.mjs:8 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - tests/fixtures/lanshu-sample.json:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - tests/fixtures/dshso-sample.json:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: api.github.com, awesome-dsh-plugin.com, dsh.lanshuagent.com, example.com, github.com, img.shields.io, raw.githubusercontent.com, www.dsh.so<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - docs/superpowers/plans/2026-08-15-find-dsh-plugins.md:1010 → 文件读取 (1 处)<br>  - docs/superpowers/plans/2026-08-15-find-dsh-plugins.md:677 → 进程控制 (2 处)<br>  - scripts/audit.mjs:62 → 进程控制 (1 处)<br>  - scripts/search.mjs:70 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 539, 注释行: 16 (3.0%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - docs/superpowers/plans/2026-08-15-find-dsh-plugins.md:1010 → 读取文档/压缩文件 (1 处) — 需人工判断是否在授权范围内 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - tests/audit.test.mjs:13 → 修改系统目录 (1 处)<br>  - docs/superpowers/plans/2026-08-15-find-dsh-plugins.md:581 → 修改系统目录 (1 处) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 5 天; 提交总数: 21 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | find-dsh-plugins |
| 校验 URL | https://github.com/JazzuLu/find-dsh-plugins |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
