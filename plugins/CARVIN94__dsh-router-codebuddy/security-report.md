...
- **校验对象**: https://github.com/CARVIN94/dsh-router-codebuddy
- **校验时间**: 2026-08-30 04:22:05
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 291; 包名: dsh-router-codebuddy; 描述: DSH plugin: CodeBuddy supplier for dsh-router. Provides the codebuddy supplier ( — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 15 处, 函数: 35 个, 比例: 42.9% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/contract.ts |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - src/plugin.ts:4 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - src/plugin.ts:153 → fetch 网络请求 (6 处)<br>  - src/plugin.ts:82 → 浏览器网络 API (1 处)<br>涉及的域名: copilot.tencent.com, github.com, localhost, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:57 → API Key / Token 读取 (1 处)<br>  - src/contract.ts:28 → 敏感凭证读取 (3 处)<br>  - src/plugin.ts:7 → API Key / Token 读取 (16 处)<br>  - src/plugin.ts:11 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/plugin.ts:339 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 3. repo: 0 deps + 3 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 688, 注释行: 131 (19.0%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-router-codebuddy |
| 校验 URL | https://github.com/CARVIN94/dsh-router-codebuddy |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
