...
- **校验对象**: https://github.com/Entaum/dsh-free-games
- **校验时间**: 2026-09-07 03:34:27
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2010-06-18 (5924 天前); 公开仓库数: 3; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 456; 包名: dsh-free-games; 描述: Free Spawnd.gg games cabinet for DeepSeek Harness Web, with search, hover previe — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 15 处, 函数: 95 个, 比例: 15.8% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:42 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (12 处)<br>  - AGENTS.md:4 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - data/spawnd_game_list.json:2 → 探测到 HTTP(S) 网络请求 (126 处)<br>  - lib/index.js:10 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: assets.spawnd.gg, dsh.pub, evil.example, evil.test, github.com, spawnd.gg, user, www.spawnd.gg<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:19 → 敏感凭证读取 (1 处)<br>  - lib/client.js:394 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - package.json:24 → 临时文件操作 (1 处)<br>  - scripts/check.mjs:77 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1086, 注释行: 0 (0.0%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:38 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | dsh-free-games |
| 校验 URL | https://github.com/Entaum/dsh-free-games |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
