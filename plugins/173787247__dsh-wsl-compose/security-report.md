...
- **校验对象**: https://github.com/173787247/dsh-wsl-compose
- **校验时间**: 2026-09-24 03:45:09
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 146; 包名: dsh-wsl-compose; 描述: DeepSeek Harness WSL plugin: docker compose ps/logs; optional up/down with confi — README 包含功能/使用说明 |
...
| 3.4 | 无命令注入风险 | 必查 | ❌ 不通过 | 检测到命令注入风险:<br>  - lib/compose.js:9 → 命令字符串拼接变量 (注入风险) (1 处) |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/compose.js:38 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 309, 注释行: 5 (1.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-wsl-compose |
| 校验 URL | https://github.com/173787247/dsh-wsl-compose |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
