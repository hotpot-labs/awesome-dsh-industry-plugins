...
- **校验对象**: https://github.com/kongdexu/dsh-win-notify
- **校验时间**: 2026-09-02 03:38:10
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2021-12-01 (1735 天前); 公开仓库数: 14; 粉丝数: 5 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 462; 包名: dsh-win-notify; 描述: DSH (DeepSeek Harness) host plugin for Windows: real OS-level toasts that persis — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 25 处, 函数: 126 个, 比例: 19.8% |
...
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - src/index.ts:108 → 进程执行 (2 处)<br>  - lib/index.js:91 → 进程执行 (2 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:55 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:27 → 探测到 HTTP(S) 网络请求 (151 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - docs/DEVELOPMENT.md:157 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, opencollective.com, registry.npmjs.org, tidelift.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/smoke.mjs:76 → 进程控制 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 5. repo: 0 deps + 5 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1054, 注释行: 192 (18.2%) |
...
| :--- | :--- |
| 插件名称 | dsh-win-notify |
| 校验 URL | https://github.com/kongdexu/dsh-win-notify |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
