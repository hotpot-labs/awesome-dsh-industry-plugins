...
- **校验对象**: https://github.com/Cola1018/dsh-aicc-zhunao
- **校验时间**: 2026-08-23 14:32:17
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-06-02 (81 天前); 公开仓库数: 8; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 180; 包名: dsh-aicc-zhunao; 描述: A public DeepSeek Harness preset for main-brain orchestration, execution gating, — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./agent.cordis.yml"}}; Cordis 配置文件: agent.cordis.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:16 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - CONTRIBUTING.md:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - agent.cordis.yml:2 → 敏感凭证读取 (1 处)<br>  - skills/aicc-law/SKILL.md:13 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 26, 注释行: 1 (3.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-aicc-zhunao |
| 校验 URL | https://github.com/Cola1018/dsh-aicc-zhunao |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
