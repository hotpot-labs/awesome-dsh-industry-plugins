...
- **校验对象**: https://github.com/gitByteFree/dsh-mermaid-smooth
- **校验时间**: 2026-09-01 04:12:20
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-06-07 (2642 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 439; 包名: dsh-mermaid-smooth; 描述: Render mermaid fences in dsh web chat as diagrams by default with smooth zoom/pa — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 12 处, 函数: 61 个, 比例: 19.7% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:18 → 探测到 HTTP(S) 网络请求 (146 处)<br>  - README.md:67 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:46 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, opencollective.com, registry.npmmirror.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - docs/3.png:972 → DDoS 攻击 (1 处) |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 761, 注释行: 157 (20.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-mermaid-smooth |
| 校验 URL | https://github.com/gitByteFree/dsh-mermaid-smooth |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
