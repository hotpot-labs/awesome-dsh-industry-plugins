...
- **校验对象**: https://github.com/bigharm/dsh-agentnoodle
- **校验时间**: 2026-09-02 03:37:09
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-08-30 (732 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 473; 包名: dsh-agentnoodle; 描述: 可扩展的 AI 群像聊天游戏框架（DeepSeek Harness 插件）：输入一个行动，多个 NPC 各自回应，带头像插图；世界、地点、NPC 与功能扩展都可 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ⚠️  需人工复核 | 无 git tag; package.json version: 0.1.14; 有版本号但无 git tag / release |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 62 处, 函数: 196 个, 比例: 31.6% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - README.md:109 → 读取环境变量配置 (1 处)<br>  - CHANGELOG.md:64 → 读取环境变量配置 (1 处)<br>  - docs/gameplay.md:24 → 读取环境变量配置 (1 处)<br>  - seed/.env.example:1 → 读取环境变量配置 (1 处)<br>  - lib/game.js:23 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:13 → 读取用户主目录 (2 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:26 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - client/client.js:41 → fetch 网络请求 (1 处)<br>涉及的域名: github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - scripts/selftest.mjs:60 → 会话历史读取 (5 处)<br>  - seed/worlds/tavern/sessions/characters/char_default.json:22 → 会话历史读取 (1 处)<br>  - seed/worlds/jiuxiao/locations.json:4 → 敏感凭证读取 (4 处)<br>  - seed/worlds/jiuxiao/sessions/characters/char_default.json:22 → 会话历史读取 (1 处)<br>  - lib/game.js:106 → 会话历史读取 (6 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1674, 注释行: 55 (3.3%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-agentnoodle |
| 校验 URL | https://github.com/bigharm/dsh-agentnoodle |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
