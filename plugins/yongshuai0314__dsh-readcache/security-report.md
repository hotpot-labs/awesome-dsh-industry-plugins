...
- **校验对象**: https://github.com/yongshuai0314/dsh-readcache
- **校验时间**: 2026-08-26 01:29:34
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-11-19 (2836 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 192; 包名: dsh-readcache; 描述: 为 DSH read 工具提供版本令牌校验的进程内结果缓存：未变更文件的重复读取跳过文件 I/O 与重渲染，并附带 readcache 工具查看统计与清空缓存。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 3 处, 函数: 8 个, 比例: 37.5% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - .npmrc:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 202, 注释行: 57 (28.2%) |
...
| :--- | :--- |
| 插件名称 | dsh-readcache |
| 校验 URL | https://github.com/yongshuai0314/dsh-readcache |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
