...
- **校验对象**: https://github.com/better-er/dsh-write-rule-guard
- **校验时间**: 2026-09-05 03:36:16
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 391; 包名: dsh-write-rule-guard; 描述: 禁止在 edit/write 写入内容中使用匹配正则的字符，可在 cordis 配置文件里配置正则与报错文案。host 半身在工具执行前拦截 edit / wr — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 1 处, 函数: 21 个, 比例: 4.8% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:19 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .github/workflows/release.yml:27 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 256, 注释行: 70 (27.3%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-write-rule-guard |
| 校验 URL | https://github.com/better-er/dsh-write-rule-guard |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
