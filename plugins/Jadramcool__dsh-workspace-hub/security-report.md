...
- **校验对象**: https://github.com/Jadramcool/dsh-workspace-hub
- **校验时间**: 2026-08-26 01:25:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-03-29 (2340 天前); 公开仓库数: 17; 粉丝数: 6 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: Apache-2.0 (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 364; 包名: dsh-workspace-hub; 描述: DSH workspace folders: categorize workspaces into folders in the left sidebar (d — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 136 处, 函数: 782 个, 比例: 17.4% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/index.ts:184 → 读取环境变量配置 (8 处)<br>  - src/client/index.js:428 → 读取浏览器 Cookie/本地存储 (5 处)<br>  - lib/client.js:428 → 读取浏览器 Cookie/本地存储 (5 处)<br>  - lib/index.js:129 → 读取环境变量配置 (8 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:65 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - LICENSE:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - src/index.ts:82 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - src/index.ts:437 → fetch 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, opencode.ai, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - src/index.ts:184 → 读取环境变量 API Key/凭证 (8 处)<br>  - lib/index.js:129 → 读取环境变量 API Key/凭证 (8 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/index.ts:220 → 定时任务 (3 处)<br>  - src/client/index.js:516 → 定时任务 (6 处)<br>  - lib/client.js:516 → 定时任务 (6 处)<br>  - lib/index.js:170 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 5. repo: 0 deps + 5 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 5291, 注释行: 298 (5.6%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - src/client/index.js:259 → 明文写入敏感信息 (5 处)<br>  - lib/client.js:259 → 明文写入敏感信息 (5 处) — 需确认是否加密存储 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 3 天; 提交总数: 34 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-workspace-hub |
| 校验 URL | https://github.com/Jadramcool/dsh-workspace-hub |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
