...
- **校验对象**: https://github.com/Entity-Him/dsh-sky-skin
- **校验时间**: 2026-09-02 03:34:04
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-04-06 (1609 天前); 公开仓库数: 17; 粉丝数: 9 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ⚠️  需人工复核 | 许可证: CC-BY-SA-4.0 — 需确认与 MIT 兼容性 |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 340; 包名: dsh-sky-skin; 描述: 光遇·遇境 (Sky: Children of the Light) 主题皮肤 — 七大地图主题(遇境/晨岛/云野/雨林/霞谷/暮土/禁阁)随心切换,设置页一键 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 13 处, 函数: 59 个, 比例: 22.0% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:40 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - LICENSE:4 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - assets/tgc-sky-mobile.jpg:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - assets/home-arena.jpg:1 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - assets/sky-1-2.jpg:1 → 探测到 HTTP(S) 网络请求 (7 处)<br>涉及的域名: github.com, x<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - assets/candle-3.png:6296 → DDoS 攻击 (1 处) |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:889 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1288, 注释行: 69 (5.4%) |
...
| :--- | :--- |
| 插件名称 | dsh-sky-skin |
| 校验 URL | https://github.com/Entity-Him/dsh-sky-skin |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
