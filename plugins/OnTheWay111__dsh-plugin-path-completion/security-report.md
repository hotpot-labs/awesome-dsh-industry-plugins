...
- **校验对象**: https://github.com/OnTheWay111/dsh-plugin-path-completion
- **校验时间**: 2026-09-21 03:58:37
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2016-04-18 (3807 天前); 公开仓库数: 63; 粉丝数: 40 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1195; 包名: @onthewayli/dsh-plugin-path-completion; 描述: Codex-style @path file completion for the DeepSeek Harness Web composer: a bound — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: >=4.0.0 <5.0.0-0; engines: {"node": ">=22.19"} |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 8 处, 函数: 81 个, 比例: 9.9% — try/catch 覆盖比例较低 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - tsdown.config.ts:55 → 读取环境变量配置 (6 处)<br>  - tests/plugin.test.ts:19 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:21 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - tests/plugin.test.ts:118 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - docs/awesome-submission.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - src/index.ts:95 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: evil.example, github.com, localhost<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - tsdown.config.ts:55 → 读取环境变量 API Key/凭证 (4 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - docs/awesome-pr-body.md:45 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 0 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 853, 注释行: 211 (24.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-path-completion |
| 校验 URL | https://github.com/OnTheWay111/dsh-plugin-path-completion |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
