...
- **校验对象**: https://github.com/m1452700576/dsh-app-updater
- **校验时间**: 2026-09-02 03:38:54
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-01-14 (2787 天前); 公开仓库数: 6; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: Apache-2.0 (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 545; 包名: dsh-app-updater; 描述: Sidebar button that checks whether DSH has a newer release and auto-downloads/op — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 37 处, 函数: 118 个, 比例: 31.4% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:29 → 读取环境变量配置 (7 处)<br>  - lib/client.js:156 → 读取浏览器 Cookie/本地存储 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:453 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - PR-awesome-dsh-plugin.md:16 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .gitignore:75 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .npmrc:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:26 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - .editorconfig:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: api.github.com, awesome-dsh-plugin.com, gh-proxy.com, ghfast.top, ghproxy.net, github.com, mirror.ghproxy.com, www.contributor-covenant.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - LICENSE:149 → 挖矿相关 (1 处) |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - push-to-github.sh:9 → 敏感凭证读取 (1 处)<br>  - lib/index.js:316 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - MARKET_PUBLISH.md:43 → 临时文件操作 (2 处)<br>  - lib/client.js:214 → 定时任务 (1 处)<br>  - lib/index.js:134 → 定时任务 (4 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 991, 注释行: 53 (5.3%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-app-updater |
| 校验 URL | https://github.com/m1452700576/dsh-app-updater |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
