...
- **校验对象**: https://github.com/icanfinish11/dsh-context-mode
- **校验时间**: 2026-09-12 03:50:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-11-19 (661 天前); 公开仓库数: 3; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ⚠️  需人工复核 | 许可证: Elastic-2.0 — 需确认与 MIT 兼容性 |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 768; 包名: dsh-context-mode; 描述: DeepSeek Harness (dsh) plugin for context-mode: in-process ctx_* tools (sandboxe — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml<br>  - README.md:20 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 16 处, 函数: 45 个, 比例: 35.6% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: scripts/dev-boot-probe.mjs |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - index.js:65 → 读取环境变量配置 (9 处)<br>  - scripts/dev-harness.mjs:23 → 读取环境变量配置 (1 处)<br>  - scripts/dev-boot-probe.mjs:10 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - index.js:224 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:123 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - index.js:65 → 读取环境变量 API Key/凭证 (9 处)<br>  - scripts/dev-harness.mjs:23 → 读取环境变量 API Key/凭证 (1 处)<br>  - scripts/dev-boot-probe.mjs:10 → 读取环境变量 API Key/凭证 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/dev-boot-probe.mjs:34 → 进程控制 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 627, 注释行: 112 (17.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-context-mode |
| 校验 URL | https://github.com/icanfinish11/dsh-context-mode |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
