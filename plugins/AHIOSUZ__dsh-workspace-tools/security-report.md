...
- **校验对象**: https://github.com/AHIOSUZ/dsh-workspace-tools
- **校验时间**: 2026-09-12 03:43:18
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2015-01-31 (4241 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 774; 包名: @ahiosuz/dsh-workspace-tools; 描述: Workspace tools for DeepSeek Harness: per-workspace default Agent preset and per — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (5): v0.4.6, v0.4.4, v0.4.3, v0.4.1, v0.4.0; package.json version: 0.4.6 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 28 处, 函数: 176 个, 比例: 15.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:33 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package-lock.json:27 → 探测到 HTTP(S) 网络请求 (116 处)<br>  - market-entry.yml:2 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, opencollective.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - package-lock.json:181 → 敏感凭证读取 (5 处)<br>  - README.md:72 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/check.mjs:7 → 进程控制 (2 处)<br>  - scripts/smoke.mjs:128 → 定时任务 (6 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1130, 注释行: 88 (7.8%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-workspace-tools |
| 校验 URL | https://github.com/AHIOSUZ/dsh-workspace-tools |
| 必查项通过率 | 29/33 (88%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🟡 灰名单 (需人工复核) |
| 主要风险说明 | 详见各检查项结果 |
