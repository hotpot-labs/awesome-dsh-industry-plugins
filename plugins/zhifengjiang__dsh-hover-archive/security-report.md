...
- **校验对象**: https://github.com/zhifengjiang/dsh-hover-archive
- **校验时间**: 2026-08-26 01:29:50
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2013-12-24 (4627 天前); 公开仓库数: 30; 粉丝数: 5 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 498; 包名: dsh-hover-archive; 描述: Codex-style hover archive for DeepSeek Harness web: hovering a sidebar session r — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 1 处, 函数: 63 个, 比例: 1.6% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - src/client.js:46 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, img.shields.io, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client.js:158 → 定时任务 (2 处)<br>  - test/smoke.test.mjs:145 → 定时任务 (3 处)<br>  - test/smoke.test.mjs:326 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 561, 注释行: 54 (9.6%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 7 天; 提交总数: 11 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-hover-archive |
| 校验 URL | https://github.com/zhifengjiang/dsh-hover-archive |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
