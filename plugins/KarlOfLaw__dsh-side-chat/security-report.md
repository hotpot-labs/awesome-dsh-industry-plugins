...
- **校验对象**: https://github.com/KarlOfLaw/dsh-side-chat
- **校验时间**: 2026-08-25 01:23:37
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-12-02 (1361 天前); 公开仓库数: 10; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 530; 包名: dsh-side-chat; 描述: DeepSeek Harness 原生并排侧聊：归档独立会话、按需主会话上下文、原生会话 UI 与可配置生命周期。 — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (3): v1.1.2, v1.1.0, v1.0.0; package.json version: 1.1.2; GitHub Releases (3): v1.1.2, v1.1.0, v1.0.0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 13 处, 函数: 193 个, 比例: 6.7% — try/catch 覆盖比例较低 |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/host.template.js; 发现清理逻辑: tests/integration.test.mjs; 发现清理逻辑: dev/seed-acceptance.mjs |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - build-formal.mjs:6 → 读取环境变量配置 (1 处)<br>  - src/client.js:47 → 读取浏览器 Cookie/本地存储 (2 处)<br>  - dev/build-dev.mjs:15 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README_EN.md:12 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:20 → 探测到 HTTP(S) 网络请求 (27 处)<br>  - README.md:12 → 探测到 HTTP(S) 网络请求 (6 处)<br>涉及的域名: github.com, img.shields.io, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - build-formal.mjs:6 → 读取环境变量 API Key/凭证 (1 处)<br>  - dev/build-dev.mjs:15 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client.js:345 → 定时任务 (2 处)<br>  - dev/seed-acceptance.mjs:16 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1555, 注释行: 40 (2.6%) |
...
| :--- | :--- |
| 插件名称 | dsh-side-chat |
| 校验 URL | https://github.com/KarlOfLaw/dsh-side-chat |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
