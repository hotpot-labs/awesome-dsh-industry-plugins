...
- **校验对象**: https://github.com/bobostudio/dsh-session-lens
- **校验时间**: 2026-08-29 06:14:18
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-09-13 (1445 天前); 公开仓库数: 39; 粉丝数: 12 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 275; 包名: dsh-session-lens; 描述: Session insights and one-click shareable HTML replay for DeepSeek Harness (DSH)  — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 25 处, 函数: 285 个, 比例: 8.8% — try/catch 覆盖比例较低 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/client/lens-view.tsx:35 → 读取浏览器 Cookie/本地存储 (2 处)<br>  - lib/client.js:195 → 读取浏览器 Cookie/本地存储 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/index.ts:226 → 读取用户主目录 (1 处)<br>  - tests/server.test.ts:147 → 深层目录穿越 (1 处)<br>  - lib/index.js:1099 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:5 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:21 → 探测到 HTTP(S) 网络请求 (35 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - CHANGELOG.md:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, dshfind.com, github.com, img.shields.io, keepachangelog.com, localhost, registry.npmjs.org, semver.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - tests/redact.test.ts:27 → 敏感凭证读取 (3 处)<br>  - tests/server.test.ts:172 → 敏感凭证读取 (1 处)<br>  - tests/rc6-format.test.ts:50 → 敏感凭证读取 (2 处)<br>  - tests/export-html.test.ts:67 → 敏感凭证读取 (2 处)<br>  - tests/fixtures/sample.session.jsonl:6 → 敏感凭证读取 (2 处)<br>  ... 共 6 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client/lens-view.tsx:270 → 定时任务 (1 处)<br>  - lib/client.js:388 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 5. repo: 0 deps + 5 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 4682, 注释行: 346 (7.4%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-session-lens |
| 校验 URL | https://github.com/bobostudio/dsh-session-lens |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
