...
- **校验对象**: https://github.com/Saretheya/dsh-settings-nav-order
- **校验时间**: 2026-09-18 03:50:02
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-08-15 (34 天前); 公开仓库数: 2; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 434; 包名: dsh-settings-nav-order; 描述: Reorder the DeepSeek Harness settings panel nav by long-pressing and dragging a  — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: >=4.0.1 <5; engines: {"node": "^22.19.0 \|\| >=24.0.0"} |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 24 处, 函数: 75 个, 比例: 32.0% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:9 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package.json:67 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:9 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - lib/index.js:58 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:214 → fetch 网络请求 (2 处)<br>涉及的域名: github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:206 → 定时任务 (4 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 713, 注释行: 135 (18.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-settings-nav-order |
| 校验 URL | https://github.com/Saretheya/dsh-settings-nav-order |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
