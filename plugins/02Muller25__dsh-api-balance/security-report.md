...
- **校验对象**: https://github.com/02Muller25/dsh-api-balance
- **校验时间**: 2026-08-23 13:03:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-06-12 (72 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 309; 包名: api-balance; 描述: Real-time DeepSeek API account balance readout for the DeepSeek Harness web GUI  — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v0.2.1, v0.2.0; package.json version: 0.2.1; GitHub Releases (1): v0.2.1 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 10 处, 函数: 33 个, 比例: 30.3% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:25 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/screenshot.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:87 → fetch 网络请求 (1 处)<br>  - lib/index.js:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: api.deepseek.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:19 → API Key / Token 读取 (2 处)<br>  - README.md:19 → 敏感凭证读取 (3 处)<br>  - lib/index.js:14 → API Key / Token 读取 (2 处)<br>  - lib/index.js:3 → 敏感凭证读取 (9 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:105 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 351, 注释行: 15 (4.3%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:45 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 校验 URL | https://github.com/02Muller25/dsh-api-balance |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
