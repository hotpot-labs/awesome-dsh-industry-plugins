...
- **校验对象**: https://github.com/yanglingrise/dsh-erii-boot-splash
- **校验时间**: 2026-09-11 03:40:39
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-01-12 (606 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 350; 包名: dsh-erii-boot-splash; 描述: Erii (Sakura) themed boot splash animation for the DeepSeek Harness Web UI: fall — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:27 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, www.deepseek.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:57 → 定时任务 (1 处)<br>  - lib/client.js:75 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 137, 注释行: 12 (8.8%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - README.md:42 → 创建开机自启/系统服务 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-erii-boot-splash |
| 校验 URL | https://github.com/yanglingrise/dsh-erii-boot-splash |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
