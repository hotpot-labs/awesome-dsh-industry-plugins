...
- **校验对象**: https://github.com/nirvanaslash/dsh-bat2exe
- **校验时间**: 2026-09-17 04:01:59
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-12-04 (3209 天前); 公开仓库数: 56; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 519; 包名: dsh-bat2exe; 描述: Compile Windows .bat/.cmd launchers into real .exe shells inside DeepSeek Harnes — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-tools: ^0.1.0-rc.7 \|\| ^0.1.1-rc.1 \|\| ^0.1.2-rc.1 \|\| ^0.1.5-rc.1; engines: {"node": ">=20"} |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - self-test.mjs:46 → 读取环境变量配置 (1 处)<br>  - lib/index.js:45 → 读取环境变量配置 (6 处) |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - self-test.mjs:46 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/index.js:45 → 读取环境变量 API Key/凭证 (6 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - e2e-test.mjs:41 → 进程控制 (1 处)<br>  - self-test.mjs:78 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 758, 注释行: 103 (13.6%) |
...
| :--- | :--- |
| 插件名称 | dsh-bat2exe |
| 校验 URL | https://github.com/nirvanaslash/dsh-bat2exe |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
