...
- **校验对象**: https://github.com/Isilsolme/dsh-anthropic-fonts
- **校验时间**: 2026-08-23 16:31:52
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 293; 包名: dsh-anthropic-fonts; 描述: Switch the DSH web interface to Anthropic Sans Web Text and model conversation t — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": [], "platform": "web", "immediately; Cordis/DSH 依赖: @deepseek-ai/cordis; Cordis 配置文件: cordis.patch.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 49, 注释行: 2 (4.1%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-anthropic-fonts |
| 校验 URL | https://github.com/Isilsolme/dsh-anthropic-fonts |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
