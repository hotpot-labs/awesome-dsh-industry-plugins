...
- **校验对象**: https://github.com/corlinniu/dsh-daily-chat
- **校验时间**: 2026-09-23 03:54:43
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-02-23 (1672 天前); 公开仓库数: 8; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ❌ 不通过 | 未找到许可证声明 (package.json 无 license 字段且无 LICENSE 文件) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1059; 包名: @local/dsh-daily-chat; 描述: Split New Session into 日常 (a lightweight chat agent) and 工作 (the shipped coding  — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 26 处, 函数: 84 个, 比例: 31.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - index.js:75 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - index.js:77 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - index.js:75 → 读取环境变量 API Key/凭证 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client.js:511 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1131, 注释行: 406 (35.9%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - client.js:321 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | dsh-daily-chat |
| 校验 URL | https://github.com/corlinniu/dsh-daily-chat |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
