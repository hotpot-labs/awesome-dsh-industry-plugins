...
- **校验对象**: https://github.com/EvilIrving/dsh-context-proxy
- **校验时间**: 2026-08-23 16:24:15
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 477; 包名: dsh-context-proxy; 描述: Thin on-demand context-retrieval layer for DeepSeek Harness: context_query / con — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.1; @deepseek-ai/dsh-agent: ^0.1.0-rc.6; @deepseek-ai/dsh-llm: ^0.1.0-rc.6; @deepseek-ai/dsh-session: ^0.1.0-rc.6; @deepseek-ai/dsh-session-query: ^0.1.0-rc.6 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-agent, @deepseek-ai/dsh-llm, @deepseek-ai/dsh-session, @deepseek-ai/dsh-session-query; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 46 个, 比例: 0.0% — 未检测到任何 try/catch |
...
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - src/tools.ts:246 → 进程执行 (1 处)<br>  - lib/tools.js:199 → 进程执行 (1 处) |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - tests/filters.spec.ts:27 → 临时文件操作 (8 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 14. repo: 2 deps + 12 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 711, 注释行: 120 (16.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-context-proxy |
| 校验 URL | https://github.com/EvilIrving/dsh-context-proxy |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
