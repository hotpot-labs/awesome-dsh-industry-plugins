...
- **校验对象**: https://github.com/jinwendijv/dsh-applauncher
- **校验时间**: 2026-09-01 04:12:41
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-04-25 (128 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 182; 包名: dsh-applauncher; 描述: DSH 应用启动器：扫描本机已安装的 Windows 应用，在侧边栏设置图标上方提供一键启动（零依赖、纯原生 JS，宿主侧通过 PowerShell 扫描 +  — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 41 处, 函数: 103 个, 比例: 39.8% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - screenshots.json:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - widget.js:133 → fetch 网络请求 (3 处)<br>涉及的域名: github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:31 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
| 3.8 | 不篡改全局配置 | 必查 | ⚠️  需人工复核 | 检测到配置修改:<br>  - lib/index.js:63 → 写入配置文件 (1 处)<br>  - lib/index.js:63 → 写入 DSH 配置 (1 处) — 需人工判断是否涉及全局配置篡改 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:262 → 定时任务 (2 处)<br>  - lib/index.js:63 → 文件写入 (1 处)<br>  - lib/index.js:49 → 文件读取 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1186, 注释行: 106 (8.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-applauncher |
| 校验 URL | https://github.com/jinwendijv/dsh-applauncher |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
