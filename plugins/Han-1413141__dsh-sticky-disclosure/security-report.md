...
- **校验对象**: https://github.com/Han-1413141/dsh-sticky-disclosure
- **校验时间**: 2026-08-23 16:29:42
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 664; 包名: dsh-sticky-disclosure; 描述: DSH Web client plugin: collapse every expanded collapsible section in the conver — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (1): v1.0.0; package.json version: 1.0.1; GitHub Releases (1): v1.0.0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 76 个, 比例: 10.5% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - install.ps1:12 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.en.md:3 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - package.json:24 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - docs/PROMO.md:13 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, awesome-dsh-plugin.com, github.com, img.shields.io, raw.githubusercontent.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1128, 注释行: 139 (12.3%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:143 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-sticky-disclosure |
| 校验 URL | https://github.com/Han-1413141/dsh-sticky-disclosure |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
