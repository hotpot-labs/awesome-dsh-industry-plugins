...
- **校验对象**: https://github.com/RGarvel/dsh-channel-view
- **校验时间**: 2026-09-04 03:44:03
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-11-05 (2859 天前); 公开仓库数: 27; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 479; 包名: dsh-channel-view; 描述: Spike: channel-grouped session view for DeepSeek Harness — sidebar tab injection — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (1): v0.0.1-spike.4; package.json version: 0.0.1-spike.5 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": [], "platform": "web"}}; Cordis 配置文件: cordis.patch.yml<br>  - README.md:25 → Monkey patch / prototype pollution<br>  - cordis.patch.yml:1 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 72 个, 比例: 11.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:37 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: github.com, img.shields.io, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:284 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 523, 注释行: 77 (14.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-channel-view |
| 校验 URL | https://github.com/RGarvel/dsh-channel-view |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
