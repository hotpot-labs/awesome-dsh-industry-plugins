...
- **校验对象**: https://github.com/jiekesu967/dsh-plugin-opencode-usage
- **校验时间**: 2026-09-13 03:53:55
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-07-24 (2241 天前); 公开仓库数: 3; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 420; 包名: dsh-plugin-opencode-usage; 描述: OpenCode Go 用量百分比悬浮面板:在 DeepSeek Harness Web 会话窗口左下角显示滚动/每周/每月用量百分比与重置时间。 — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:29 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - cordis.patch.yml:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:6 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/index.js:68 → fetch 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, opencode.ai<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:33 → API Key / Token 读取 (4 处)<br>  - README.md:33 → 敏感凭证读取 (5 处)<br>  - cordis.patch.yml:2 → API Key / Token 读取 (1 处)<br>  - cordis.patch.yml:2 → 敏感凭证读取 (1 处)<br>  - lib/index.js:7 → API Key / Token 读取 (9 处)<br>  ... 共 6 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:66 → 定时任务 (1 处)<br>  - lib/client.js:341 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 0 deps + 2 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 817, 注释行: 51 (6.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-opencode-usage |
| 校验 URL | https://github.com/jiekesu967/dsh-plugin-opencode-usage |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
