...
- **校验对象**: https://github.com/Vladimir-Kryshchenko/dsh-route-fence-linter
- **校验时间**: 2026-09-06 03:34:46
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-01-07 (606 天前); 公开仓库数: 8; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 847; 包名: dsh-route-fence-linter; 描述: Static linter: every plugin HTTP route must carry a browser-trust fence (loopbac — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 9 处, 函数: 291 个, 比例: 3.1% — try/catch 覆盖比例较低 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/plugin.ts:115 → 读取环境变量配置 (1 处)<br>  - src/cli.ts:16 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - src/plugin.ts:115 → 读取用户主目录 (1 处)<br>  - src/cli.ts:16 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:30 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:45 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - tests/scan.spec.ts:227 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - tests/fixtures/try-catch/index.ts:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - tests/fixtures/traced-factory-unfenced/trust-fence.ts:15 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: evil.example, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - src/plugin.ts:115 → 读取环境变量 API Key/凭证 (1 处)<br>  - src/cli.ts:16 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2107, 注释行: 430 (20.4%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 14 天; 提交总数: 9 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-route-fence-linter |
| 校验 URL | https://github.com/Vladimir-Kryshchenko/dsh-route-fence-linter |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
