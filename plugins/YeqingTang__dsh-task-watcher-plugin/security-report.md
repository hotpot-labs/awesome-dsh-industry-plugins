...
- **校验对象**: https://github.com/YeqingTang/dsh-task-watcher-plugin
- **校验时间**: 2026-09-07 03:36:05
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-03-31 (2717 天前); 公开仓库数: 16; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 148; 包名: @yeqingtang/dsh-task-watcher-plugin; 描述: Windows tray monitor for DeepSeek Harness tasks: deploys and launches DshTaskWat — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ⚠️  需人工复核 | 无 git tag; package.json version: 0.5.5; 有版本号但无 git tag / release |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 18 处, 函数: 45 个, 比例: 40.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:4 → 读取 npm 凭证 (1 处)<br>  - lib/index.js:25 → 读取环境变量配置 (2 处)<br>  - assets/DshTaskWatcher.ps1:2262 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CHANGELOG.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:36 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:63 → fetch 网络请求 (1 处)<br>  - assets/config.json:2 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:25 → 读取环境变量 API Key/凭证 (2 处)<br>  - assets/DshTaskWatcher.ps1:291 → 敏感凭证读取 (8 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:168 → 定时任务 (2 处)<br>  - lib/client.js:61 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 482, 注释行: 54 (11.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-task-watcher-plugin |
| 校验 URL | https://github.com/YeqingTang/dsh-task-watcher-plugin |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
