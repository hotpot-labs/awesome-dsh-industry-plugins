...
- **校验对象**: https://github.com/xobexo/dsh-smart-scenario-router
- **校验时间**: 2026-08-29 06:15:30
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-01-22 (583 天前); 公开仓库数: 4; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 325; 包名: dsh-smart-scenario-router; 描述: Persistent DeepSeek Harness scenario router with domestic-model priority, fallba — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"engines": {"dsh": ">=0.1.1-rc.1"}, "bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform; Cordis/DSH 依赖: @deepseek-ai/dsh-settings; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 12 处, 函数: 59 个, 比例: 20.3% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:38 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - client.js:18 → fetch 网络请求 (2 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 429, 注释行: 1 (0.2%) |
...
| 5.4 | 无未授权读取 | 必查 | ⚠️  需人工复核 | 检测到潜在未授权读取:<br>  - CONTRIBUTING.md:41 → 读取 git 历史 (1 处) — 需人工判断是否在授权范围内 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-smart-scenario-router |
| 校验 URL | https://github.com/xobexo/dsh-smart-scenario-router |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
