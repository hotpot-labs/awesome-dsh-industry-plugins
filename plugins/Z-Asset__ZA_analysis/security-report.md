...
- **校验对象**: https://github.com/Z-Asset/ZA_analysis
- **校验时间**: 2026-09-19 03:47:41
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 98; 包名: @zasset/za-analysis; 描述: Z Research analysis stage — end-to-end model training and estimation for asset p — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 16 处, 函数: 55 个, 比例: 29.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:59 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package-lock.json:27 → 探测到 HTTP(S) 网络请求 (16 处)<br>涉及的域名: github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/orchestrator.ts:241 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 5. repo: 1 deps + 4 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 850, 注释行: 117 (13.8%) |
...
| :--- | :--- |
| 插件名称 | ZA_analysis |
| 校验 URL | https://github.com/Z-Asset/ZA_analysis |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
