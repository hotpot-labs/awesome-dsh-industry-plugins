...
- **校验对象**: https://github.com/longisland-icetea/dsh-lan-access
- **校验时间**: 2026-09-13 03:54:58
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2016-08-04 (3692 天前); 公开仓库数: 6; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 678; 包名: dsh-lan-access; 描述: Open the DeepSeek Harness Web LAN access fences: serve on all interfaces, trust  — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (3): v0.6.0, v0.4.0, v0.3.0; package.json version: 0.6.0; GitHub Releases (3): v0.6.0, v0.4.0, v0.3.0 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.2; engines: {"node": ">=20.18.1"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-connectio; Cordis/DSH 依赖: @deepseek-ai/cordis; Cordis 配置文件: cordis.patch.yml<br>  - lib/index.js:47 → Monkey patch / prototype pollution<br>  - lib/types/index.d.ts:63 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 13 处, 函数: 104 个, 比例: 12.5% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: lib/client.js; 发现清理逻辑: test/no-auth.mjs |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.zh.md:41 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - cordis.patch.yml:71 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/index.js:105 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - lib/client.js:47 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: 127.0.0.1, 192.168.255.5, dsh.invalid, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:29 → 敏感凭证读取 (2 处)<br>  - README.zh.md:77 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:184 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1340, 注释行: 300 (22.4%) |
...
| :--- | :--- |
| 插件名称 | dsh-lan-access |
| 校验 URL | https://github.com/longisland-icetea/dsh-lan-access |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
