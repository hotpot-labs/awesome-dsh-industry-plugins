...
- **校验对象**: https://github.com/ikta2010/dsh-loop-breaker
- **校验时间**: 2026-09-20 04:04:00
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 120; 包名: dsh-loop-breaker; 描述: 硬止损：同一工具+同一参数反复调用时中断本轮（补上 repeat-tool-reminder 只提醒不中断的缺口） — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 3 处, 函数: 15 个, 比例: 20.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/index.js:43 → 读取环境变量配置 (1 处) |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:43 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/index.js:8 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 273, 注释行: 68 (24.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-loop-breaker |
| 校验 URL | https://github.com/ikta2010/dsh-loop-breaker |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
