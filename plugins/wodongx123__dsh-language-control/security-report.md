...
- **校验对象**: https://github.com/wodongx123/dsh-language-control
- **校验时间**: 2026-08-31 04:29:48
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-05-14 (3030 天前); 公开仓库数: 22; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 175; 包名: @deepseek-ai/dsh-language-control; 描述: DSH plugin that forces the agent's chain-of-thought and reasoning to stay in Chi — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-locale", ; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-host-webserver; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 7 处, 函数: 28 个, 比例: 25.0% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - lib/client.js:109 → fetch 网络请求 (3 处)<br>  - lib/index.js:69 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: localhost<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:130 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 369, 注释行: 43 (11.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-language-control |
| 校验 URL | https://github.com/wodongx123/dsh-language-control |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
