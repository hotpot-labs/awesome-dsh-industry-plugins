...
- **校验对象**: https://github.com/13071301808/dsh-composer-expand
- **校验时间**: 2026-08-23 13:05:10
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-08-18 (1101 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 291; 包名: dsh-composer-expand; 描述: Composer expand toggle for DeepSeek Harness: a ⬆/⬇ button in the composer tool r — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 12 处, 函数: 47 个, 比例: 25.5% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:24 → 探测到 HTTP(S) 网络请求 (88 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .github/workflows/publish.yml:24 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, opencollective.com, registry.npmmirror.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 470, 注释行: 115 (24.5%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - src/client/index.js:88 → 明文写入敏感信息 (1 处)<br>  - lib/client.js:79 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 6 天; 提交总数: 8 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-composer-expand |
| 校验 URL | https://github.com/13071301808/dsh-composer-expand |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
