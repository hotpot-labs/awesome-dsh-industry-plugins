...
- **校验对象**: https://github.com/roclee2692/dsh-model-groups
- **校验时间**: 2026-09-08 03:44:43
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-09-15 (722 天前); 公开仓库数: 43; 粉丝数: 3 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 254; 包名: dsh-model-groups; 描述: Platform and manufacturer grouped model picker for DeepSeek Harness — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 3 处, 函数: 34 个, 比例: 8.8% — try/catch 覆盖比例较低 |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - README.md:5 → 敏感凭证读取 (1 处)<br>  - lib/index.js:1 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 116, 注释行: 1 (0.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-model-groups |
| 校验 URL | https://github.com/roclee2692/dsh-model-groups |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
