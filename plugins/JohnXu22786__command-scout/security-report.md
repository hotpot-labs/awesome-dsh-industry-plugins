...
- **校验对象**: https://github.com/JohnXu22786/command-scout
- **校验时间**: 2026-08-23 16:34:18
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1293; 包名: dsh-command-scout; 描述: Scans the project workspace at startup and exposes its declared build commands ( — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 28 处, 函数: 188 个, 比例: 14.9% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: src/dsh-adapter.js |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/execute.js:104 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ❌ 不通过 | 检测到命令注入风险:<br>  - src/execute.js:9 → 开启 shell 模式 (注入风险) (3 处) |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - test/deno-collector.test.js:23 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: x<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/execute.js:127 → 定时任务 (2 处)<br>  - src/execute.js:54 → 进程控制 (2 处)<br>  - src/cli.js:116 → 文件写入 (1 处)<br>  - src/cli.js:161 → 文件读取 (1 处)<br>  - src/collectors/common.js:35 → 文件读取 (1 处)<br>  ... 共 16 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2422, 注释行: 402 (16.6%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - bin/command-scout.mjs:1 → 修改系统目录 (1 处) |
...
| :--- | :--- |
| 插件名称 | command-scout |
| 校验 URL | https://github.com/JohnXu22786/command-scout |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
