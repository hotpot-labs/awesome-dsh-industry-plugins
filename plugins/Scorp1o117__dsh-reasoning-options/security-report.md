...
- **校验对象**: https://github.com/Scorp1o117/dsh-reasoning-options
- **校验时间**: 2026-09-06 03:34:26
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 417; 包名: dsh-reasoning-options; 描述: DeepSeek Harness 推理强度增强插件：自动为 llm-pi-ai 下所有自定义模型补齐 x-opencode reasoning effort 档 — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-settings: ^0.1.0-rc.7 \|\| ^0.1.1-rc.1 \|\| ^0.1.2-rc.1; engines: {"node": ">=20.18"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/dsh-settings; Cordis 配置文件: cordis.patch.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:24 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - tests/ops.test.mjs:12 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: api.commandcode.ai, github.com, opencode.ai, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.js:177 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 259, 注释行: 82 (31.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-reasoning-options |
| 校验 URL | https://github.com/Scorp1o117/dsh-reasoning-options |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
