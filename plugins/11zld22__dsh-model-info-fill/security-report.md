...
- **校验对象**: https://github.com/11zld22/dsh-model-info-fill
- **校验时间**: 2026-09-08 03:38:14
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-08-27 (2202 天前); 公开仓库数: 6; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 196; 包名: dsh-model-info-fill; 描述: 模型信息补全：按模型名自动补全上下文、输出上限、思考档位和图片能力 — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject": ["@deepseek-ai/dsh-client-runtime",; Cordis 配置文件: cordis.patch.yml<br>  - src/catalog.js:266 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 21 处, 函数: 164 个, 比例: 12.8% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/index.js:24 → 读取环境变量配置 (1 处) |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:5 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/index.js:114 → fetch 网络请求 (1 处)<br>  - src/client.js:173 → fetch 网络请求 (1 处)<br>  - src/catalog.js:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, models.dev<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - src/index.js:24 → 读取环境变量 API Key/凭证 (1 处)<br>  - src/index.js:135 → 敏感凭证读取 (1 处)<br>  - src/client.js:174 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1460, 注释行: 1 (0.1%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-model-info-fill |
| 校验 URL | https://github.com/11zld22/dsh-model-info-fill |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
