...
- **校验对象**: https://github.com/Rice00/dsh-loupe
- **校验时间**: 2026-09-25 04:01:21
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 549; 包名: dsh-loupe; 描述: Select text in a DSH conversation and explain it in a floating window: independe — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 59 处, 函数: 156 个, 比例: 37.8% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/index.js:61 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:63 → 读取用户主目录 (1 处)<br>  - docs/M1-实施记录.md:30 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:12 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - package.json:21 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/client.js:751 → fetch 网络请求 (8 处)<br>  - lib/index.js:589 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: dsh.internal, github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:61 → 读取环境变量 API Key/凭证 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:1376 → 定时任务 (1 处)<br>  - lib/index.js:372 → 文件写入 (2 处)<br>  - lib/index.js:81 → 文件读取 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2296, 注释行: 333 (14.5%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:490 → 明文写入敏感信息 (2 处) — 需确认是否加密存储 |
...
| :--- | :--- |
| 插件名称 | dsh-loupe |
| 校验 URL | https://github.com/Rice00/dsh-loupe |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
