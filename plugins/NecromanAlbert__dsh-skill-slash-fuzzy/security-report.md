...
- **校验对象**: https://github.com/NecromanAlbert/dsh-skill-slash-fuzzy
- **校验时间**: 2026-09-02 03:35:29
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-02-04 (2401 天前); 公开仓库数: 26; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 238; 包名: dsh-skill-slash-fuzzy; 描述: Resolve unique kebab-case skill substrings in DeepSeek Harness slash tokens, so  — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: >=0.0.1-rc.1 <0.1.0 \|\| >=4.0.1 <5.0.0-0; @deepseek-ai/dsh-client-runtime: >=0.0.1-rc.1 <0.1.0 \|\| >=0.1.0-rc.1 <0.2.0-0; @deepseek-ai/dsh-client-ui-input-trigger: >=0.0.1-rc.1 <0.1.0 \|\| >=0.1.0-rc.1 <0.2.0-0; engines: {"node": ">=22"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["@deepseek-ai/d; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-client-runtime, @deepseek-ai/dsh-client-ui-input-trigger; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 3 处, 函数: 43 个, 比例: 7.0% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:58 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:9 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - README.zh.md:9 → 探测到 HTTP(S) 网络请求 (8 处)<br>涉及的域名: deepseek-harness.github.io, github.com, img.shields.io, nodejs.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 365, 注释行: 7 (1.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-skill-slash-fuzzy |
| 校验 URL | https://github.com/NecromanAlbert/dsh-skill-slash-fuzzy |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
