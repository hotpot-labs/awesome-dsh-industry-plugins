...
- **校验对象**: https://github.com/jing-hy/computer-user
- **校验时间**: 2026-08-28 10:24:18
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1135; 包名: computer-user; 描述: Codex-style computer use for DeepSeek Harness (DSH): read the screen and drive t — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (1): v0.1.0; package.json version: 0.3.6 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/dsh-settings: ^0.1.0-rc.6 \|\| ^0.1.1-rc.2; engines: {"node": "^22.19 \|\| >=24"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web"}}; Cordis/DSH 依赖: @deepseek-ai/dsh-settings; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 16 处, 函数: 158 个, 比例: 10.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:64 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - client.js:44 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - client.js:229 → 定时任务 (1 处)<br>  - src/tools.js:272 → 定时任务 (1 处)<br>  - src/ps.js:42 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1481, 注释行: 168 (11.3%) |
...
| :--- | :--- |
| 插件名称 | computer-user |
| 校验 URL | https://github.com/jing-hy/computer-user |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
