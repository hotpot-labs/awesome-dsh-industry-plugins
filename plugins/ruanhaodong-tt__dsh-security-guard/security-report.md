...
- **校验对象**: https://github.com/ruanhaodong-tt/dsh-security-guard
- **校验时间**: 2026-09-06 03:36:27
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-07-21 (777 天前); 公开仓库数: 9; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 446; 包名: dsh-security-guard; 描述: DSH runtime security guard plugin — loader import confinement, HTTP Host header  — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: >=0.0.1-rc.1 <0.1.0 \|\| >=0.1.0-rc.1 <0.2.0-0; @deepseek-ai/cordis-plugin-loader: >=0.0.1-rc.1 <0.1.0 \|\| >=0.1.0-rc.1 <0.2.0-0; @deepseek-ai/dsh-host-webserver: >=0.0.1-rc.1 <0.1.0 \|\| >=0.1.0-rc.1 <0.2.0-0 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/cordis-plugin-loader, @deepseek-ai/dsh-host-webserver; Cordis 配置文件: cordis.patch.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/web.js:26 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - patches/dsh-security-0.1.1-rc.2.patch:186 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - patches/dsh-security-0.1.1-rc.2.patch:77 → WebSocket 连接 (4 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - verify.mjs:64 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 254, 注释行: 11 (4.3%) |
...
| 校验 URL | https://github.com/ruanhaodong-tt/dsh-security-guard |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
