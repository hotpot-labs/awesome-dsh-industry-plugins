...
- **校验对象**: https://github.com/Choi-Peng/dsh-footer-order
- **校验时间**: 2026-08-23 14:31:15
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 469; 包名: @choi-p/dsh-footer-order; 描述: DSH web sidebar footer action ordering: force the sidebar.footer.action slot int — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 26 处, 函数: 150 个, 比例: 17.3% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:17 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - catalog.json:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/imgs/after.png:8 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: 127.0.0.1, github.com, img.shields.io, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - .gitignore:14 → 敏感凭证读取 (1 处)<br>  - scripts/smoke-host.mjs:159 → 敏感凭证读取 (2 处)<br>  - lib/index.js:78 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/smoke-host.mjs:37 → 定时任务 (1 处)<br>  - scripts/smoke-client.mjs:84 → 定时任务 (2 处)<br>  - scripts/smoke-client.mjs:111 → 进程控制 (1 处)<br>  - lib/client.js:421 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1517, 注释行: 215 (14.2%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - README.en.md:38 → 创建开机自启/系统服务 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-footer-order |
| 校验 URL | https://github.com/Choi-Peng/dsh-footer-order |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
