...
- **校验对象**: https://github.com/AKS1st/dock-files
- **校验时间**: 2026-08-23 14:22:01
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 253; 包名: dock-files; 描述: File explorer for the DSH dock: browse the active conversation's working directo — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 59 处, 函数: 469 个, 比例: 12.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:33 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:38 → 探测到 HTTP(S) 网络请求 (93 处)<br>  - README.md:33 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/index.ts:452 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: dsh.internal, github.com, opencollective.com, registry.npmmirror.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - README.en.md:5 → 云存储上传 (5 处)<br>  - src/index.ts:414 → 云存储上传 (5 处)<br>  - src/client/styles.ts:300 → 云存储上传 (1 处)<br>  - src/client/i18n.ts:49 → 云存储上传 (5 处)<br>  - src/client/ExplorerView.tsx:129 → 云存储上传 (39 处)<br>  ... 共 7 处 — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 6. repo: 0 deps + 6 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 5055, 注释行: 987 (19.5%) |
...
| :--- | :--- |
| 插件名称 | dock-files |
| 校验 URL | https://github.com/AKS1st/dock-files |
| 必查项通过率 | 29/33 (88%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
