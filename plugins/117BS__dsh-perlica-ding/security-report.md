...
- **校验对象**: https://github.com/117BS/dsh-perlica-ding
- **校验时间**: 2026-08-23 13:05:07
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2022-08-16 (1468 天前); 公开仓库数: 2; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 250; 包名: dsh-perlica-ding; 描述: 佩丽卡终端 (Perlica Terminal)：明日方舟终末地佩丽卡主题的分级任务提示音插件。计划出方案、任务完成、需要你回应、出错时播放不同提示音，普通问答 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 11 处, 函数: 32 个, 比例: 34.4% |
...
| 3.3 | 无无限制系统命令执行 | 必查 | ⚠️  需人工复核 | 检测到命令执行:<br>  - index.mjs:172 → 进程执行 (1 处)<br>⚠️ 检测到可能存在命令白名单机制, 需人工确认 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:6 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:6 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: awesome-dsh-plugin.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/verify.mjs:216 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 492, 注释行: 77 (15.7%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - index.mjs:84 → 修改系统目录 (12 处) |
...
| 7.6 | 有社区背书 | 推荐 | ⚠️  需人工复核 | Stars: 4, Forks: 0, Watchers: 1 — 社区背书不足, 需人工判断 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-perlica-ding |
| 校验 URL | https://github.com/117BS/dsh-perlica-ding |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
