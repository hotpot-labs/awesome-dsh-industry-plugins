...
- **校验对象**: https://github.com/AtinyFurina/dsh-memory-triage
- **校验时间**: 2026-09-16 03:55:40
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-07-28 (780 天前); 公开仓库数: 14; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ❌ 不通过 | 许可证: GPL-3.0 — GPL 强传染协议, 商用场景禁止 |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 281; 包名: dsh-memory-triage; 描述: DeepSeek Harness 记忆分诊插件：自动判断记忆保留/归档，并自动路由到项目桶或总体桶（规则优先 + LLM 兜底） — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:20 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - LICENSE:4 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - docs/banner.png:4 → 探测到 HTTP(S) 网络请求 (7 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - LICENSE:300 → 挖矿相关 (1 处) |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - LICENSE:340 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:307 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 887, 注释行: 42 (4.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-memory-triage |
| 校验 URL | https://github.com/AtinyFurina/dsh-memory-triage |
| 必查项通过率 | 20/33 (61%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
