...
- **校验对象**: https://github.com/Elohia/dsh-plugin-mm-vision
- **校验时间**: 2026-08-23 16:23:55
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 263; 包名: dsh-plugin-mm-vision; 描述: mm-vision (通感编码器) for DeepSeek Harness — give any text-only LLM the ability to s — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 24 个, 比例: 33.3% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/core.js:54 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/core.js:51 → 读取用户主目录 (2 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - lib/core.js:19 → 加载 child_process (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - lib/core.js:8 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/core.js:291 → fetch 网络请求 (1 处)<br>涉及的域名: dashscope.aliyuncs.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - cordis.patch.yml:4 → API Key / Token 读取 (1 处)<br>  - README.md:40 → API Key / Token 读取 (5 处)<br>  - lib/index.js:16 → API Key / Token 读取 (1 处)<br>  - lib/core.js:54 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/core.js:32 → API Key / Token 读取 (20 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/core.js:68 → 文件读取 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 567, 注释行: 63 (11.1%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - scripts/ascii_dot.py:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-mm-vision |
| 校验 URL | https://github.com/Elohia/dsh-plugin-mm-vision |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
