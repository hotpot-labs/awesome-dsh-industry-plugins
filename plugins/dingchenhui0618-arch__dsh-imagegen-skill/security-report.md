...
- **校验对象**: https://github.com/dingchenhui0618-arch/dsh-imagegen-skill
- **校验时间**: 2026-09-20 04:03:21
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-05-27 (115 天前); 公开仓库数: 5; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: Apache-2.0 (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 630; 包名: dsh-imagegen-skill; 描述: Bundled imagegen skill for DeepSeek Harness: generate or edit raster images (pho — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ❌ 不通过 | 检测到潜在违规模式:<br>  - skills/imagegen/references/sample-prompts.md:81 → 盗版/侵权 (2 处) |
...
| 2.3 | 具备异常捕获机制 | 必查 | ❌ 不通过 | try/catch: 0 处, 函数: 24 个, 比例: 0.0% — 未检测到任何 try/catch |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:34 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - LICENSE:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - skills/imagegen/SKILL.md:32 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - skills/imagegen/assets/imagegen-small.svg:1 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: api.openai.com, docs.astral.sh, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - LICENSE:149 → 挖矿相关 (1 处) |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - NOTICE:45 → 敏感凭证读取 (1 处)<br>  - README.md:25 → API Key / Token 读取 (2 处)<br>  - README.md:25 → 敏感凭证读取 (1 处)<br>  - skills/imagegen/SKILL.md:14 → API Key / Token 读取 (2 处)<br>  - skills/imagegen/SKILL.md:51 → 敏感凭证读取 (1 处)<br>  ... 共 7 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1730, 注释行: 41 (2.4%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - README.md:38 → 硬编码敏感信息 (1 处)<br>  - skills/imagegen/SKILL.md:33 → 硬编码敏感信息 (1 处) — 需确认是否加密存储 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - skills/imagegen/scripts/image_gen.py:1 → 修改系统目录 (1 处)<br>  - skills/imagegen/scripts/remove_chroma_key.py:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-imagegen-skill |
| 校验 URL | https://github.com/dingchenhui0618-arch/dsh-imagegen-skill |
| 必查项通过率 | 22/33 (67%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
