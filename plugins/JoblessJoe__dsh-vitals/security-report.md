...
- **校验对象**: https://github.com/JoblessJoe/dsh-vitals
- **校验时间**: 2026-09-24 03:47:43
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-02-07 (593 天前); 公开仓库数: 5; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 601; 包名: dsh-vitals; 描述: Live hardware load panel for the DeepSeek Harness web GUI: a btop-style tab show — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v0.1.3, v0.1.2; package.json version: 0.1.3; GitHub Releases (2): v0.1.3, v0.1.2 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 30 处, 函数: 109 个, 比例: 27.5% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - package.json:60 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:236 → fetch 网络请求 (1 处)<br>  - src/client/body.tsx:177 → fetch 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:255 → 定时任务 (1 处)<br>  - src/client/body.tsx:196 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 984, 注释行: 113 (11.5%) |
...
| :--- | :--- |
| 插件名称 | dsh-vitals |
| 校验 URL | https://github.com/JoblessJoe/dsh-vitals |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
