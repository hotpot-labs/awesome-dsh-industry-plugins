...
- **校验对象**: https://github.com/RempleXI/word-docx-writer
- **校验时间**: 2026-09-13 03:52:37
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2020-07-14 (2251 天前); 公开仓库数: 9; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 672; 包名: dsh-plugin-word-docx-writer; 描述: Word (.docx) 生成与排版 Agent Skill（中文长文档通用：实验报告、课程设计报告、毕业论文等），同时作为 skills-only 的 DSH — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CHANGELOG.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README_EN.md:7 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:6 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - scripts/word_style.py:46 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, schemas.openxmlformats.org, semver.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1008, 注释行: 37 (3.7%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | word-docx-writer |
| 校验 URL | https://github.com/RempleXI/word-docx-writer |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
