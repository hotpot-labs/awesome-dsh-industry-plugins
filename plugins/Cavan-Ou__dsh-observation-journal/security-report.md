...
- **校验对象**: https://github.com/Cavan-Ou/dsh-observation-journal
- **校验时间**: 2026-08-23 14:30:01
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 563; 包名: dsh-observation-journal; 描述: DSH 插件：零侵入运行事实遥测——纯观察者（零工具注册/零 LLM 调用），session 结束时自动把任务/模型/工具/失败/状态/时长按一行一会话写入观测 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 23 处, 函数: 86 个, 比例: 26.7% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - tests/test.mjs:133 → 读取环境变量配置 (8 处)<br>  - lib/index.js:82 → 读取环境变量配置 (3 处) |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (5 处)<br>  - README.zh.md:5 → 探测到 HTTP(S) 网络请求 (5 处)<br>涉及的域名: github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - REPORT.md:69 → 敏感凭证读取 (1 处)<br>  - README.md:62 → 敏感凭证读取 (2 处)<br>  - tests/test.mjs:133 → 读取环境变量 API Key/凭证 (8 处)<br>  - tests/test.mjs:342 → 敏感凭证读取 (6 处)<br>  - lib/index.js:82 → 读取环境变量 API Key/凭证 (3 处)<br>  ... 共 6 处 — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - tests/test.mjs:504 → 定时任务 (1 处)<br>  - tests/test.mjs:339 → 临时文件操作 (6 处)<br>  - lib/index.js:460 → 定时任务 (3 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1072, 注释行: 82 (7.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-observation-journal |
| 校验 URL | https://github.com/Cavan-Ou/dsh-observation-journal |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 13/14 (约 93%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
