...
- **校验对象**: https://github.com/Stellum-Waq/dsh-pet-ronaldo
- **校验时间**: 2026-09-13 03:52:46
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-09-14 (728 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 739; 包名: dsh-ronaldo-pet; 描述: C罗桌宠 — DeepSeek Harness（DSH）桌面宠物 bundle 插件。葡萄牙 7 号 chibi 吉祥物住在 Web 界面右下角，随 Agent — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (3): v1.2.1, v1.2.0, v1.1.0; package.json version: 1.2.1 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 65 处, 函数: 246 个, 比例: 26.4% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:10 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - client/client.js:49 → fetch 网络请求 (2 处)<br>  - src/host.js:323 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - assets/siu.mp3:3 → 探测到 HTTP(S) 网络请求 (9 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - demo/index.html:69 → 定时任务 (1 处)<br>  - client/client.js:128 → 定时任务 (4 处)<br>  - scripts/validate.mjs:110 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1823, 注释行: 91 (5.0%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-pet-ronaldo |
| 校验 URL | https://github.com/Stellum-Waq/dsh-pet-ronaldo |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
