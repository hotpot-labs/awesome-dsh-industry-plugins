...
- **校验对象**: https://github.com/NickyWooden/dsh-halo
- **校验时间**: 2026-09-25 04:01:17
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2017-08-01 (3341 天前); 公开仓库数: 10; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 356; 包名: dsh-halo; 描述: One-click publish of DSH conversations as Markdown articles on a Halo CMS blog:  — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["slots", "setti; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 36 处, 函数: 91 个, 比例: 39.6% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/index.js:44 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:46 → 读取用户主目录 (5 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - LICENSE:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:13 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package.json:12 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/client.js:31 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/client.js:215 → fetch 网络请求 (1 处)<br>涉及的域名: blog.example.com, github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - LICENSE:149 → 挖矿相关 (1 处) |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - package.json:4 → 敏感凭证读取 (1 处)<br>  - lib/client.js:7 → 敏感凭证读取 (19 处)<br>  - lib/index.js:44 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/index.js:9 → 敏感凭证读取 (29 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1132, 注释行: 125 (11.0%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:34 → 硬编码敏感信息 (1 处) — 需确认是否加密存储 |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - lib/index.js:96 → 修改系统目录 (2 处) |
...
| :--- | :--- |
| 插件名称 | dsh-halo |
| 校验 URL | https://github.com/NickyWooden/dsh-halo |
| 必查项通过率 | 20/33 (61%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
