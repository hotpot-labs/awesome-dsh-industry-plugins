...
- **校验对象**: https://github.com/SciF-Lin/dsh-browsercontrol-mcp
- **校验时间**: 2026-09-25 04:01:36
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-11-27 (301 天前); 公开仓库数: 4; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 574; 包名: dsh-browsercontrol-mcp; 描述: Real browser control for DeepSeek Harness: mounts Playwright MCP as native tools — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: ^4.0.2; @deepseek-ai/dsh-mcp-client: >=0.1.0-rc.2 <0.2.0-0 \|\| >=0.1.1-rc.1 <0.2.0-0 \|\| >=0.1.2-alpha.2 <0.2.0-0 \|\| >=0.1.3-alpha.2 <0.2.0-0 \|\| >=0.1.5-alpha.1 <0.2.0-0 \|\| >=0.1.6-alpha.1 <0.2.0-0 \|\| >=0.1.7-alpha.1 <0.2.0-0; engines: {"node": "^22.19.0 \|\| >=24.0.0"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-mcp-client; Cordis 配置文件: cordis.patch.yml |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 7 处, 函数: 81 个, 比例: 8.6% — try/catch 覆盖比例较低 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/index.js:125 → 读取环境变量配置 (6 处)<br>  - test/plugin.test.mjs:156 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package-lock.json:24 → 探测到 HTTP(S) 网络请求 (158 处)<br>  - README.md:54 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.en.md:48 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/index.js:58 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - test/plugin.test.mjs:78 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: chromewebstore.google.com, github.com, localhost, opencollective.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - test/plugin.test.mjs:42 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - README.en.md:6 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 621, 注释行: 85 (13.7%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/index.js:125 → 从环境变量获取敏感信息 (2 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-browsercontrol-mcp |
| 校验 URL | https://github.com/SciF-Lin/dsh-browsercontrol-mcp |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
