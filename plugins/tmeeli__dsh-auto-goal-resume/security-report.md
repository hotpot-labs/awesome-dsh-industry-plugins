...
- **校验对象**: https://github.com/tmeeli/dsh-auto-goal-resume
- **校验时间**: 2026-09-15 03:58:28
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-05-19 (2675 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 686; 包名: auto-goal-resume; 描述: DeepSeek Harness 插件:重启后自动续跑有活跃目标(goal)的会话,无需人工说"继续"。 — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}}; Cordis 配置文件: cordis.patch.yml<br>  - install.sh:54 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 10 处, 函数: 11 个, 比例: 90.9% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - install.sh:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:26 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:102 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: gitee.com, www.npmjs.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:205 → 临时文件操作 (4 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 198, 注释行: 36 (18.2%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - install.sh:1 → 修改系统目录 (1 处)<br>  - install.sh:126 → 创建开机自启/系统服务 (1 处)<br>  - README.md:117 → 创建开机自启/系统服务 (3 处) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 29 天; 提交总数: 10 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-auto-goal-resume |
| 校验 URL | https://github.com/tmeeli/dsh-auto-goal-resume |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
