...
- **校验对象**: https://github.com/dsh-publish/dsh-pair-quick
- **校验时间**: 2026-09-16 03:59:18
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 66; 包名: dsh-pair-quick; 描述: 设置页「快速配对」：一键铸造 dsh 远程访问配对链接并显示二维码（依赖 remote-web-ui 的 /api/pair/issue）。 — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | @deepseek-ai/cordis: >=4.0.0 <5 \|\| >=4.0.0-rc.1 <4.0.0; @deepseek-ai/dsh-host-webserver: >=0.1.1-rc.2 <0.1.2 \|\| >=0.1.2-rc.1 <0.2.0 |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"engines": {"dsh": ">=0.1.0-rc.1"}, "bundle": {"patch": "./cordis.patch.yml"}, "client": {"inject":; Cordis/DSH 依赖: @deepseek-ai/cordis, @deepseek-ai/dsh-host-webserver; Cordis 配置文件: cordis.patch.yml |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/index.js:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - lib/index.js:31 → fetch 网络请求 (1 处)<br>  - lib/client.js:18 → fetch 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 176, 注释行: 8 (4.5%) |
...
| :--- | :--- |
| 插件名称 | dsh-pair-quick |
| 校验 URL | https://github.com/dsh-publish/dsh-pair-quick |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
