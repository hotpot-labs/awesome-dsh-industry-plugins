...
- **校验对象**: https://github.com/Isanti2016/dsh-quicksight
- **校验时间**: 2026-08-23 16:31:50
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2016-04-13 (3784 天前); 公开仓库数: 47; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 538; 包名: dsh-quicksight; 描述: Two-tier image reading for text-only models in DeepSeek Harness: fast local OCR  — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 5 处, 函数: 21 个, 比例: 23.8% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - .gitignore:3 → 读取环境变量配置 (2 处)<br>  - dsh/index.js:53 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - dsh/index.js:32 → 读取用户主目录 (1 处)<br>  - test/plugin-smoke.test.mjs:10 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - SECURITY.md:20 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:25 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:44 → 探测到 HTTP(S) 网络请求 (8 处)<br>  - dsh/index.js:191 → fetch 网络请求 (1 处)<br>涉及的域名: ..., github.com, integrate.api.nvidia.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:3 → 敏感凭证读取 (2 处)<br>  - README.md:77 → API Key / Token 读取 (1 处)<br>  - dsh/index.js:53 → 读取环境变量 API Key/凭证 (3 处)<br>  - dsh/index.js:12 → 敏感凭证读取 (1 处)<br>  - docs/ARCHITECTURE.md:44 → 敏感凭证读取 (1 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/verify-package.mjs:18 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - docs/ARCHITECTURE.md:43 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 322, 注释行: 34 (10.6%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 7 天; 提交总数: 10 |
...
| :--- | :--- |
| 插件名称 | dsh-quicksight |
| 校验 URL | https://github.com/Isanti2016/dsh-quicksight |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
