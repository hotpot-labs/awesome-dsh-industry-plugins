...
- **校验对象**: https://github.com/AKS1st/dsh-archived-conversations
- **校验时间**: 2026-08-23 14:22:22
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 189; 包名: dsh-archived-conversations; 描述: Show archived conversations in the DSH Web settings page with read-only message  — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 5 处, 函数: 91 个, 比例: 5.5% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:22 → 探测到 HTTP(S) 网络请求 (85 处)<br>  - src/index.ts:163 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/client/index.js:78 → fetch 网络请求 (1 处)<br>  - lib/client.js:80 → fetch 网络请求 (1 处)<br>涉及的域名: dsh.local, github.com, opencollective.com, registry.npmmirror.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1159, 注释行: 270 (23.3%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 6 天; 提交总数: 9 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-archived-conversations |
| 校验 URL | https://github.com/AKS1st/dsh-archived-conversations |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
