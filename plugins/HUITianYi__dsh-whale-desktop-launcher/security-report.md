...
- **校验对象**: https://github.com/HUITianYi/dsh-whale-desktop-launcher
- **校验时间**: 2026-08-23 16:29:11
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-08-04 (384 天前); 公开仓库数: 11; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 364; 包名: dsh-whale-desktop-launcher; 描述: Windows desktop launcher for DeepSeek Harness with a whale-girl icon and a clean — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 1 处, 函数: 14 个, 比例: 7.1% — try/catch 覆盖比例较低 |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/install.js:40 → 读取用户主目录 (2 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.zh-CN.md:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package.json:48 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - ASSET_LICENSE.md:7 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - NOTICE:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, creativecommons.org, github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ❌ 不通过 | 检测到恶意网络模式:<br>  - assets/whale-girl.png:4168 → DDoS 攻击 (1 处) |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 254, 注释行: 6 (2.4%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-whale-desktop-launcher |
| 校验 URL | https://github.com/HUITianYi/dsh-whale-desktop-launcher |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
