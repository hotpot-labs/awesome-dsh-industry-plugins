...
- **校验对象**: https://github.com/jackyytche/dsh-hindsight-memory
- **校验时间**: 2026-09-04 03:59:25
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-05-17 (109 天前); 公开仓库数: 3; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 339; 包名: dsh-hindsight-memory; 描述: Hindsight long-term memory for DeepSeek Harness: auto-recall context injection b — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 16 处, 函数: 120 个, 比例: 13.3% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:23 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - cordis.patch.yml:12 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - tests/plugin.test.mjs:29 → 探测到 HTTP(S) 网络请求 (4 处)<br>涉及的域名: 10.0.0.9, 127.0.0.1, github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - CHANGELOG.md:17 → 会话历史读取 (1 处)<br>  - README.md:29 → API Key / Token 读取 (2 处)<br>  - tests/plugin.test.mjs:145 → 会话历史读取 (1 处)<br>  - lib/index.js:40 → API Key / Token 读取 (4 处)<br>  - lib/client.js:61 → API Key / Token 读取 (7 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - tests/plugin.test.mjs:248 → 定时任务 (5 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 2. repo: 1 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1537, 注释行: 175 (11.4%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-hindsight-memory |
| 校验 URL | https://github.com/jackyytche/dsh-hindsight-memory |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
