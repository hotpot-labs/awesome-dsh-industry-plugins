...
- **校验对象**: https://github.com/enteguo/dsh-plugin-quick-chat
- **校验时间**: 2026-09-12 03:50:18
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2015-08-22 (4038 天前); 公开仓库数: 22; 粉丝数: 4 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 83; 包名: dsh-plugin-quick-chat; 描述: 快速会话：侧边栏「新会话」下方的即时对话窗口，可选带主会话上下文，保留历史可继续、删除、重命名。 — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["@deepseek-ai/d; Cordis 配置文件: cordis.patch.yml<br>  - lib/client.js:622 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:20 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package.json:24 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:9 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/index.js:192 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - lib/client.js:230 → fetch 网络请求 (2 处)<br>涉及的域名: github.com, img.shields.io, quick-chat.local<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:42 → 文件写入 (1 处)<br>  - lib/index.js:30 → 文件读取 (1 处)<br>  - test/smoke.mjs:80 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1098, 注释行: 28 (2.6%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-quick-chat |
| 校验 URL | https://github.com/enteguo/dsh-plugin-quick-chat |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
