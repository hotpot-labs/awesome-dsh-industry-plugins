...
- **校验对象**: https://github.com/kgwdonkey/dsh-header-controls
- **校验时间**: 2026-09-08 03:43:55
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ⚠️  需人工复核 | 账号注册: 2026-08-16 (22 天前); 公开仓库数: 2; 粉丝数: 0; ⚠️ 账号注册不足 30 天 (22 天) |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 268; 包名: dsh-header-controls; 描述: 会话右上角电源控制：Session log 左侧的「重启 / 注销」胶囊按钮，与官方版式一致（静态 web 插件） — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 21 处, 函数: 50 个, 比例: 42.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/index.js:24 → 读取环境变量配置 (1 处)<br>  - lib/relaunch-helper.cjs:17 → 读取环境变量配置 (7 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/index.js:24 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ❌ 不通过 | 检测到命令注入风险:<br>  - lib/relaunch-helper.cjs:116 → 命令字符串拼接变量 (注入风险) (1 处) |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:15 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - lib/client.js:36 → fetch 网络请求 (2 处)<br>  - lib/relaunch-helper.cjs:138 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, github.com, img.shields.io<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/index.js:24 → 读取环境变量 API Key/凭证 (1 处)<br>  - lib/relaunch-helper.cjs:17 → 读取环境变量 API Key/凭证 (7 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/index.js:57 → 定时任务 (1 处)<br>  - lib/client.js:84 → 定时任务 (5 处)<br>  - lib/relaunch-helper.cjs:79 → 定时任务 (3 处)<br>  - lib/relaunch-helper.cjs:23 → 文件写入 (3 处)<br>  - lib/relaunch-helper.cjs:36 → 文件读取 (2 处)<br>  ... 共 6 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 415, 注释行: 30 (7.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-header-controls |
| 校验 URL | https://github.com/kgwdonkey/dsh-header-controls |
| 必查项通过率 | 20/33 (61%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
