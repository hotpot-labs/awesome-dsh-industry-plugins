...
- **校验对象**: https://github.com/988hj7tczd-oss/dsh-math-olympiad
- **校验时间**: 2026-08-30 04:21:04
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 556; 包名: dsh-math-olympiad; 描述: DSH skill bundle: competition math (IMO/Putnam/USAMO/AIME) solved with a pure-re — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 2.1 | 明确标注支持的 DSH 版本范围 | 必查 | ✅ 通过 | engines: {"node": ">=22.18.0"} |
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ✅ 通过 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "compatibility": {"node": ">=22.18.0", "dsh": ">=0.1.0"}; Cordis 配置文件: cordis.patch.yml, cordis.yml |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - tests/smoke.e2e.ts:329 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:65 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/store-evidence.md:9 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 585, 注释行: 96 (16.4%) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 6.1 | 无需 root 权限 | 必查 | ❌ 不通过 |   - skills/dsh-math-olympiad/scripts/check_latex.sh:24 → 要求 root/sudo 权限 |
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - tests/smoke.e2e.ts:321 → 修改系统目录 (3 处)<br>  - skills/dsh-math-olympiad/scripts/check_latex.sh:1 → 修改系统目录 (1 处)<br>  - skills/dsh-math-olympiad/scripts/compile_pdf.sh:1 → 修改系统目录 (1 处) |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 7.1 | 近 3 个月内有更新 | 必查 | ✅ 通过 | 最近提交距今: 5 天; 提交总数: 3 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-math-olympiad |
| 校验 URL | https://github.com/988hj7tczd-oss/dsh-math-olympiad |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
