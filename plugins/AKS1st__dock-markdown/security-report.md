...
- **校验对象**: https://github.com/AKS1st/dock-markdown
- **校验时间**: 2026-08-23 14:22:18
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 260; 包名: dock-markdown; 描述: Markdown viewer for the dock file explorer: renders .md/.markdown/.mdx files wit — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 69 处, 函数: 335 个, 比例: 20.6% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - src/index.ts:197 → 读取环境变量配置 (1 处)<br>  - lib/index.js:179 → 读取环境变量配置 (1 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.en.md:26 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package.json:8 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package-lock.json:48 → 探测到 HTTP(S) 网络请求 (96 处)<br>  - README.md:26 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - src/index.ts:134 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: bugs.chromium.org, dsh.internal, github.com, opencollective.com, registry.npmmirror.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client/MarkdownView.tsx:158 → 定时任务 (1 处)<br>  - lib/client.js:4044 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 8. repo: 0 deps + 8 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 6010, 注释行: 1049 (17.5%) |
...
| :--- | :--- |
| 插件名称 | dock-markdown |
| 校验 URL | https://github.com/AKS1st/dock-markdown |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
