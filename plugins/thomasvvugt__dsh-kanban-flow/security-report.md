...
- **校验对象**: https://github.com/thomasvvugt/dsh-kanban-flow
- **校验时间**: 2026-08-28 10:31:59
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2019-10-15 (2508 天前); 公开仓库数: 15; 粉丝数: 19 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 806; 包名: dsh-kanban-flow; 描述: Agent-driven kanban board plugin for DeepSeek Harness: one board per workspace,  — README 包含功能/使用说明 |
...
| 2.2 | 遵循 Cordis 插件开发规范 | 必查 | ⚠️  需人工复核 | dsh 配置: {"bundle": {"patch": "./cordis.patch.yml"}, "client": {"platform": "web", "inject": ["slots"]}}; Cordis 配置文件: cordis.patch.yml<br>  - index.js:269 → Monkey patch / prototype pollution — 发现潜在 hack 模式, 需人工确认 |
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 42 处, 函数: 716 个, 比例: 5.9% — try/catch 覆盖比例较低 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:47 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - package-lock.json:31 → 探测到 HTTP(S) 网络请求 (135 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - src/client/Controllers.tsx:479 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - src/client/lib/api.ts:9 → fetch 网络请求 (1 处)<br>涉及的域名: github.com, img.shields.io, opencollective.com, registry.npmjs.org, tidelift.com, www.w3.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - .github/workflows/publish.yml:2 → 敏感凭证读取 (4 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - build.mjs:53 → 进程控制 (1 处)<br>  - index.js:467 → 定时任务 (1 处)<br>  - src/client/KanbanBoard.tsx:237 → 定时任务 (2 处)<br>  - src/client/Controllers.tsx:102 → 定时任务 (4 处)<br>  - lib/client.js:479 → 定时任务 (12 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 11. repo: 0 deps + 11 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 2479, 注释行: 198 (8.0%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - src/client/lib/store.ts:24 → 明文写入敏感信息 (2 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-kanban-flow |
| 校验 URL | https://github.com/thomasvvugt/dsh-kanban-flow |
| 必查项通过率 | 23/33 (70%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
