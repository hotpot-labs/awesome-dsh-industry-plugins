...
- **校验对象**: https://github.com/heiheiha798/dsh-plugin-response-window
- **校验时间**: 2026-08-27 08:15:34
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2023-08-26 (1096 天前); 公开仓库数: 35; 粉丝数: 12 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 359; 包名: dsh-plugin-response-window; 描述: DeepSeek Harness (DSH) web plugin: per-turn response window. Each turn's tool ca — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 15 处, 函数: 83 个, 比例: 18.1% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:45 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:35 → 探测到 HTTP(S) 网络请求 (2 处)<br>  - test/e2e.py:5 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:641 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 1 deps + 0 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 949, 注释行: 145 (15.3%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - test/e2e.py:1 → 修改系统目录 (1 处) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-plugin-response-window |
| 校验 URL | https://github.com/heiheiha798/dsh-plugin-response-window |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
