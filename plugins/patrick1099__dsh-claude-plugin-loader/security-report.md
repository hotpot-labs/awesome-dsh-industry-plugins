...
- **校验对象**: https://github.com/patrick1099/dsh-claude-plugin-loader
- **校验时间**: 2026-08-28 10:30:19
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-11-23 (277 天前); 公开仓库数: 13; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ⚠️  需人工复核 | 未找到 README, 无法自动比对 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 19 处, 函数: 50 个, 比例: 38.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - dsh-claude-plugin-loader.mjs:372 → 读取环境变量配置 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - dsh-claude-plugin-loader.mjs:443 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ❌ 不通过 | 检测到命令执行:<br>  - dsh-claude-plugin-loader.mjs:375 → 进程执行 (1 处) |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:54 → 探测到 HTTP(S) 网络请求 (3 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:6 → 敏感凭证读取 (1 处)<br>  - dsh-claude-plugin-loader.mjs:372 → 读取环境变量 API Key/凭证 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - GUIDE.md:191 → 临时文件操作 (1 处)<br>  - dsh-claude-plugin-loader.mjs:347 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 701, 注释行: 55 (7.8%) |
...
| 7.5 | 完整文档 | 推荐 | ⚠️  需人工复核 | 有文档但缺少: README |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-claude-plugin-loader |
| 校验 URL | https://github.com/patrick1099/dsh-claude-plugin-loader |
| 必查项通过率 | 20/33 (61%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
