...
- **校验对象**: https://github.com/RealHacker/dsh-theme-colorizer
- **校验时间**: 2026-08-30 04:26:35
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2013-06-20 (4818 天前); 公开仓库数: 36; 粉丝数: 174 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 2039; 包名: dsh-theme-colorizer; 描述: DSH plugin: adds customizable color themes to the DeepSeek Web UI, selectable in — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ⚠️  需人工复核 | try/catch: 6 处, 函数: 85 个, 比例: 7.1% — try/catch 覆盖比例较低 |
...
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - tsconfig.json:2 → 深层目录穿越 (9 处)<br>  - lib/client.js:9 → 深层目录穿越 (1 处)<br>  - lib/tsconfig.tsbuildinfo:1 → 深层目录穿越 (476 处) — 需人工判断是否有业务必要性 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.md:188 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/tsconfig.tsbuildinfo:1 → API Key / Token 读取 (1 处)<br>  - lib/tsconfig.tsbuildinfo:1 → 敏感凭证读取 (3 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1515, 注释行: 463 (30.6%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - src/client/index.ts:87 → 明文写入敏感信息 (1 处)<br>  - lib/client.js:501 → 明文写入敏感信息 (1 处)<br>  - lib/types/client/index.js:43 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-theme-colorizer |
| 校验 URL | https://github.com/RealHacker/dsh-theme-colorizer |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
