...
- **校验对象**: https://github.com/173787247/dsh-wsl-k8s
- **校验时间**: 2026-09-24 03:45:28
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 143; 包名: dsh-wsl-k8s; 描述: DeepSeek Harness WSL plugin: read-only kubectl helpers (get/describe/logs trunca — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 8 处, 函数: 33 个, 比例: 24.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - README.zh.md:13 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:45 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.en.md:41 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - package.json:6 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - docs/awesome-entry.yml:2 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/kubectl.js:64 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 269, 注释行: 2 (0.7%) |
...
| :--- | :--- |
| 插件名称 | dsh-wsl-k8s |
| 校验 URL | https://github.com/173787247/dsh-wsl-k8s |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
