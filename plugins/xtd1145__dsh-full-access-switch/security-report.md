...
- **校验对象**: https://github.com/xtd1145/dsh-full-access-switch
- **校验时间**: 2026-08-29 06:15:38
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2025-07-10 (414 天前); 公开仓库数: 5; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 268; 包名: dsh-full-access-switch; 描述: DSH Full access 一次性开关：新工作区/新对话默认 full access，不再逐个确认（可安装 bundle + 补丁工具） — README 包含功能/使用说明 |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - patches.json:12 → 读取浏览器 Cookie/本地存储 (4 处)<br>  - patches/dsh-client-ui-conversation.client.js.diff:16 → 读取浏览器 Cookie/本地存储 (1 处)<br>  - patches/dsh-client-ui-permission-presets.client.js.diff:16 → 读取浏览器 Cookie/本地存储 (3 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - cordis.patch.yml:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:28 → 探测到 HTTP(S) 网络请求 (7 处)<br>  - docs/FAQ.md:5 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: awesome-dsh-plugin.com, dsh.market, dshbase.com, github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - scripts/verify-patches.mjs:57 → 进程控制 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 58, 注释行: 5 (8.6%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - patches.json:12 → 明文写入敏感信息 (1 处)<br>  - patches/dsh-client-ui-permission-presets.client.js.diff:24 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-full-access-switch |
| 校验 URL | https://github.com/xtd1145/dsh-full-access-switch |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
