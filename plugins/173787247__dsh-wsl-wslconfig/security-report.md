...
- **校验对象**: https://github.com/173787247/dsh-wsl-wslconfig
- **校验时间**: 2026-09-02 03:32:31
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 53; 包名: dsh-wsl-wslconfig; 描述: Read-only advice for .wslconfig memory/mirrored networking. — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 2 处, 函数: 20 个, 比例: 10.0% |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/wsl-host.js:20 → 读取环境变量配置 (2 处)<br>  - lib/wslconfig.js:26 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ⚠️  需人工复核 | 检测到全局文件访问:<br>  - lib/wslconfig.js:33 → 读取用户主目录 (1 处) — 需人工判断是否有业务必要性 |
...
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - lib/wslconfig.js:26 → 读取环境变量 API Key/凭证 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 160, 注释行: 2 (1.2%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-wsl-wslconfig |
| 校验 URL | https://github.com/173787247/dsh-wsl-wslconfig |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
