...
- **校验对象**: https://github.com/CAI-MH/dsh-agent-bridge
- **校验时间**: 2026-09-19 03:45:21
...
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 93; 包名: dsh-agent-bridge; 描述: 飞书「算法组」机器人 → DSH 会话桥：在指定工作区开真会话，用 DSH 现有模型+插件执行任务并回传结论 — DSH bundle。 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 10 处, 函数: 18 个, 比例: 55.6% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - index.js:187 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: 127.0.0.1<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - index.js:226 → 开启监听端口 (1 处)<br>  - index.js:94 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 241, 注释行: 14 (5.8%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - index.js:65 → 明文写入敏感信息 (1 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-agent-bridge |
| 校验 URL | https://github.com/CAI-MH/dsh-agent-bridge |
| 必查项通过率 | 24/33 (73%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
