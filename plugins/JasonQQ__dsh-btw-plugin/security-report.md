...
- **校验对象**: https://github.com/JasonQQ/dsh-btw-plugin
- **校验时间**: 2026-08-27 08:11:50
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2015-01-15 (4242 天前); 公开仓库数: 46; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 362; 包名: @jasonqq/dsh-btw-plugin; 描述: Codex-style /btw for DeepSeek Harness: ask a side question answered by a convers — README 包含功能/使用说明 |
| 1.5 | 不违反法律法规 | 必查 | ✅ 通过 | 未检测到明显的违法违规模式 |
| 1.6 | 有明确的版本号与正式 Release | 推荐 | ✅ 通过 | Git tags (2): v0.1.1, v0.1.0; package.json version: 0.1.0; GitHub Releases (1): v0.1.0 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 6 处, 函数: 57 个, 比例: 10.5% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: test/index.test.js; 发现清理逻辑: lib/index.js |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:30 → 探测到 HTTP(S) 网络请求 (4 处)<br>  - package-lock.json:20 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.zh.md:3 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - .github/workflows/node.js.yml:2 → 探测到 HTTP(S) 网络请求 (2 处)<br>涉及的域名: github.com, npm.pkg.github.com, registry.npmjs.org<br>需人工确认每个网络请求的用途是否明确 |
| 3.6 | 无恶意网络逻辑 | 必查 | ✅ 通过 | 未检测到挖矿/远控/代理等恶意网络模式 |
| 3.7 | 不读取敏感配置 | 必查 | ⚠️  需人工复核 | 检测到敏感配置读取:<br>  - SECURITY.md:3 → 敏感凭证读取 (2 处)<br>  - .github/workflows/npm-publish-github-packages.yml:36 → 敏感凭证读取 (1 处)<br>  - .github/workflows/release.yml:33 → 敏感凭证读取 (2 处) — 需人工判断是否读取的是当前会话上下文 |
...
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - .github/workflows/release.yml:39 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 总依赖数: 1. repo: 0 deps + 1 devDeps |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 534, 注释行: 55 (10.3%) |
...
| :--- | :--- |
| 插件名称 | dsh-btw-plugin |
| 校验 URL | https://github.com/JasonQQ/dsh-btw-plugin |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
