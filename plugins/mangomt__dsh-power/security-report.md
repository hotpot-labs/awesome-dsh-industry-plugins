...
- **校验对象**: https://github.com/mangomt/dsh-power
- **校验时间**: 2026-08-27 08:17:19
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-03-03 (3098 天前); 公开仓库数: 2; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 581; 包名: dsh-power; 描述: Floating power dock for the DeepSeek Harness web GUI: one-click restart or grace — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 10 处, 函数: 66 个, 比例: 15.2% |
| 2.4 | 卸载后完整释放资源 | 必查 | ✅ 通过 | 发现清理逻辑: lib/shutdown.js; 发现清理逻辑: lib/restart.js; 发现清理逻辑: lib/routes.js |
...
| 序号 | 检查项 | 类型 | 结果 | 详情 |
| :--- | :--- | :---: | :---: | :--- |
| 3.1 | 文件系统权限最小化 | 必查 | ❌ 不通过 | 检测到敏感路径访问:<br>  - lib/restart.js:140 → 读取环境变量配置 (2 处) |
| 3.2 | 无全局文件读写 | 必查 | ✅ 通过 | 未检测到全局文件访问模式 |
| 3.3 | 无无限制系统命令执行 | 必查 | ✅ 通过 | 未检测到命令执行调用 |
| 3.4 | 无命令注入风险 | 必查 | ✅ 通过 | 未检测到命令注入模式 |
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:7 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - client/client.js:204 → fetch 网络请求 (2 处)<br>涉及的域名: github.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - README.md:52 → 进程控制 (1 处)<br>  - README.zh.md:52 → 进程控制 (1 处)<br>  - client/client.js:188 → 定时任务 (4 处)<br>  - lib/shutdown.js:21 → 定时任务 (2 处)<br>  - lib/shutdown.js:20 → 进程控制 (2 处)<br>  ... 共 7 处 — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 717, 注释行: 183 (25.5%) |
...
| 6.2 | 不修改系统配置 | 必查 | ❌ 不通过 | 检测到系统配置修改:<br>  - README.md:37 → 创建开机自启/系统服务 (1 处) |
...
| :--- | :--- |
| 插件名称 | dsh-power |
| 校验 URL | https://github.com/mangomt/dsh-power |
| 必查项通过率 | 25/33 (76%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
