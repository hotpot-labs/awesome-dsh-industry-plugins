...
- **校验对象**: https://github.com/loyalchiiina/dsh-todo-float-ball
- **校验时间**: 2026-09-09 03:46:18
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2026-08-10 (30 天前); 公开仓库数: 6; 粉丝数: 1 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 893; 包名: dsh-todo-float-ball; 描述: DSH todo 悬浮球：常驻可拖动悬浮球实时显示当前会话 todo_write 任务清单进度（总数/完成数/进行中），Shadow DOM 样式隔离，点击折叠 — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 80 处, 函数: 84 个, 比例: 95.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CHANGELOG.md:380 → WebSocket 连接 (2 处)<br>  - CHANGELOG.md:380 → 浏览器网络 API (2 处)<br>  - package.json:28 → 探测到 HTTP(S) 网络请求 (3 处)<br>  - README.md:52 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: 127.0.0.1, github.com, keepachangelog.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - lib/client.js:215 → 定时任务 (5 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ⚠️  需人工复核 | 检测到潜在文件窃取/上传模式:<br>  - README.md:31 → 云存储上传 (1 处) — 需人工判断是否有恶意意图 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1153, 注释行: 137 (11.9%) |
...
| 5.3 | 敏感信息加密存储 | 必查 | ⚠️  需人工复核 | 检测到敏感信息存储:<br>  - lib/client.js:63 → 明文写入敏感信息 (7 处) — 需确认是否加密存储 |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-todo-float-ball |
| 校验 URL | https://github.com/loyalchiiina/dsh-todo-float-ball |
| 必查项通过率 | 27/33 (82%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🟡 灰名单 (需人工复核) |
| 主要风险说明 | 详见各检查项结果 |
