...
- **校验对象**: https://github.com/DDDMUC/dsh-delete-turn
- **校验时间**: 2026-09-22 03:56:43
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2024-07-11 (802 天前); 公开仓库数: 14; 粉丝数: 2 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 1238; 包名: dsh-delete-turn; 描述: Per-message delete for DeepSeek Harness: remove a user message, one reply step,  — README 包含功能/使用说明 |
...
| 2.3 | 具备异常捕获机制 | 必查 | ✅ 通过 | try/catch: 24 处, 函数: 158 个, 比例: 15.2% |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:30 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:12 → 探测到 HTTP(S) 网络请求 (6 处)<br>  - src/index.js:295 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - src/client.js:211 → fetch 网络请求 (2 处)<br>涉及的域名: github.com, localhost, raw.githubusercontent.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/index.js:120 → 定时任务 (2 处)<br>  - src/client.js:380 → 定时任务 (2 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 1863, 注释行: 222 (11.9%) |
...
| 项目 | 内容 |
| :--- | :--- |
| 插件名称 | dsh-delete-turn |
| 校验 URL | https://github.com/DDDMUC/dsh-delete-turn |
| 必查项通过率 | 28/33 (85%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
