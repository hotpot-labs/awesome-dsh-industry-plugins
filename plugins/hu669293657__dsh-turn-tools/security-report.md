...
- **校验对象**: https://github.com/hu669293657/dsh-turn-tools
- **校验时间**: 2026-09-02 03:37:43
...
| 1.2 | 发布账号非匿名一次性账号 | 必查 | ✅ 通过 | 账号注册: 2018-05-02 (3044 天前); 公开仓库数: 15; 粉丝数: 0 |
| 1.3 | 开源协议与 MIT 兼容 | 必查 | ✅ 通过 | 许可证: MIT (来自 LICENSE) |
| 1.4 | 核心功能与 README 描述一致 | 必查 | ✅ 通过 | README 字数: 364; 包名: dsh-turn-tools; 描述: DSH web plugin: per-turn deliverable buttons (open with the OS default app) plus — README 包含功能/使用说明 |
...
| 3.5 | 对外网络请求域名明确 | 必查 | ⚠️  需人工复核 | 检测到网络请求:<br>  - package.json:18 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - README.md:7 → 探测到 HTTP(S) 网络请求 (1 处)<br>  - CHANGELOG.md:4 → 探测到 HTTP(S) 网络请求 (1 处)<br>涉及的域名: github.com, keepachangelog.com<br>需人工确认每个网络请求的用途是否明确 |
...
| 4.5 | 无隐藏后门 | 必查 | ⚠️  需人工复核 | 检测到潜在后门/隐藏逻辑:<br>  - src/client/index.tsx:190 → 定时任务 (1 处)<br>  - lib/client.js:204 → 定时任务 (1 处) — 需人工判断是否有恶意意图 |
| 4.6 | 无文件窃取/静默上传 | 必查 | ✅ 通过 | 未检测到文件窃取/静默上传模式 |
| 4.7 | 依赖数量可控 | 推荐 | ✅ 通过 | 无第三方依赖 |
| 4.8 | 代码结构清晰 | 推荐 | ✅ 通过 | 总行数: 303, 注释行: 28 (9.2%) |
...
| :--- | :--- |
| 插件名称 | dsh-turn-tools |
| 校验 URL | https://github.com/hu669293657/dsh-turn-tools |
| 必查项通过率 | 26/33 (79%) |
| 推荐项满足率 | 14/14 (约 100%) |
| 最终分级 | 🔴 黑名单 |
| 主要风险说明 | 详见各检查项结果 |
